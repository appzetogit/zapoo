...
const calculateRestaurantCustomerDeliveryFee = ({
    ...
    if (!config?.isEnabled) return { customerDeliveryFee: 0 };
    ...
    return { customerDeliveryFee: roundCurrency(distanceKm * customerPerKmRate) };
};

// Test Scenarios
const runTests = () => {
    console.log('--- Starting Unit Tests for Delivery Charge Logic ---');

    const globalSlabs = [{ _id: 'global-1', minKm: 0, maxKm: 5, isActive: true }];
    const tierSlabs = [{ _id: 'tier-1', minKm: 0, maxKm: 5, isActive: true }];

    const restaurant = {
        deliveryPricingConfig: {
            isEnabled: true,
            orderValueSlabs: [{ _id: 'ov-1', minOrderValue: 0, maxOrderValue: 1000 }],
            customerDeliveryRates: [
                { distanceSlabId: 'tier-1', orderValueSlabId: 'ov-1', perKmRate: 10 },
                { distanceSlabId: 'global-1', orderValueSlabId: 'ov-1', perKmRate: 50 }, // Should NOT be used
            ]
        }
    };
    ...
    let result = calculateRestaurantCustomerDeliveryFee({ subtotal: 100, distanceKm: 2, restaurant, matchedDistanceSlab });
    console.log('Test 1 (Tier has slabs):', result.customerDeliveryFee === 20 ? '✅ PASSED' : '❌ FAILED' + ' (Got ' + result.customerDeliveryFee + ')');
    ...
    result = calculateRestaurantCustomerDeliveryFee({ subtotal: 100, distanceKm: 2, restaurant, matchedDistanceSlab });
    console.log('Test 2 (Tier has NO slabs):', result.customerDeliveryFee === 0 ? '✅ PASSED' : '❌ FAILED' + ' (Got ' + result.customerDeliveryFee + ')');

    // Scenario 3: No Tier assigned (My code: distanceSlabs = [])
    distanceSlabs = [];
    matchedDistanceSlab = findDistanceSlab(distanceSlabs, 2) || findBaseDistanceSlab(distanceSlabs);
    result = calculateRestaurantCustomerDeliveryFee({ subtotal: 100, distanceKm: 2, restaurant, matchedDistanceSlab });
    console.log('Test 3 (No Tier):', result.customerDeliveryFee === 0 ? '✅ PASSED' : '❌ FAILED');
