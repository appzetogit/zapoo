    ...
    try {
      ...
      if (apiKey) {
        ...
        try {
          ...
          try {
            response = await axios.get('https://api.olamaps.io/places/v1/reverse-geocode', {
              params: {
                lat: latNum,
                lng: lngNum,
                key: apiKey,
      ...
      if (!response) {
        try {
          try {
            ...
            if (googleApiKey) {
              try {
                fallbackResponse = await axios.get(`https://maps.googleapis.com/maps/api/geocode/json`, {
                  params: {
                ...
                if (fallbackResponse.data && fallbackResponse.data.results && fallbackResponse.data.results.length > 0) {
                  ...
                  if (!area) {
                    ...
                    const sublocality = addressComponents.find(c => {
                      const types = c.types || [];
                      return types.includes('sublocality_level_2') || types.includes('sublocality_level_3') || types.includes('neighborhood') || types.includes('political') && !types.includes('administrative_area_level_1') && !types.includes('locality');
                    });
                    if (sublocality && sublocality.long_name !== city && sublocality.long_name !== state && !sublocality.long_name.toLowerCase().includes('district')) {
            ...
            fallbackResponse = await axios.get(`https://api.bigdatacloud.net/data/reverse-geocode-client`, {
              params: {
                latitude: latNum,
                longitude: lngNum,
                localityLanguage: 'en'
            ...
            if (fallbackData.localityInfo?.administrative) {
              ...
              for (let i = 2; i < adminLevels.length && i < 5; i++) {
                ...
                if (level?.name && level.name !== fallbackData.principalSubdivision && level.name !== fallbackData.city && level.name !== fallbackData.locality) {
                  area = level.name;
                  break;
                }
              }
          ...
          if (firstResult.address_components && Array.isArray(firstResult.address_components)) {
            ...
            if (!area) {
              const sublocality = firstResult.address_components.find(c => {
                const types = c.types || [];
                return types.includes('sublocality_level_2') || types.includes('sublocality_level_3') || types.includes('political') && !types.includes('administrative_area_level_1') && !types.includes('locality') && !types.includes('country');
              });
              if (sublocality) {
            ...
            if (area && (area.toLowerCase().includes('district') || area.toLowerCase() === (state || "").toLowerCase() || area.toLowerCase() === (city || "").toLowerCase())) {
              area = "";
            }
            ...
            if (!area && formattedAddress) {
              ...
              if (parts.length >= 3) {
                ...
                if (potentialArea && potentialArea.toLowerCase() !== (cityPart || "").toLowerCase() && potentialArea.toLowerCase() !== (statePart || "").toLowerCase() && !potentialArea.toLowerCase().includes('district') && !potentialArea.toLowerCase().includes('city') && potentialArea.length > 2 && potentialArea.length < 50) {
                  area = potentialArea;
                ...
                if (city && firstPart.toLowerCase() !== city.toLowerCase() && firstPart.toLowerCase() !== (state || "").toLowerCase() && !firstPart.toLowerCase().includes('district') && !firstPart.toLowerCase().includes('city') && firstPart.length > 2 && firstPart.length < 50) {
                  area = firstPart;
                }
              }
            }
  ...
  try {
    ...
    if (apiKey) {
      try {
        ...
        const response = await axios.get('https://api.olamaps.io/places/v1/nearby', {
          params: {
            lat: latNum,
            lng: lngNum,
            radius: radiusNum,
