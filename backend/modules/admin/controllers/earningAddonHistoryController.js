...
export const getEarningAddonHistory = asyncHandler(async (req, res) => {
  try {
    ...
    let history = await EarningAddonHistory.find(searchQuery).populate('earningAddonId', 'title requiredOrders earningAmount startDate endDate').populate('deliveryPartnerId', 'name deliveryId phone email').populate('processedBy', 'name email').sort(sort).skip(skip).limit(parseInt(limit)).lean();
    ...
    if (search) {
      ...
      history = history.filter(item => {
        ...
        return deliveryName.toLowerCase().includes(searchLower) || deliveryId.toLowerCase().includes(searchLower) || phone.includes(search) || offerTitle.toLowerCase().includes(searchLower);
      });
    }
    ...
    const formattedHistory = history.map((item, index) => {
      ...
      return {
        ...
        incentive: item.earningAmount || 0,
    ...
    logger.error(`Error fetching earning addon history: ${error.message}`, {
      error: error.stack
    });
...
export const getEarningAddonHistoryById = asyncHandler(async (req, res) => {
  try {
    ...
    const history = await EarningAddonHistory.findById(id).populate('earningAddonId').populate('deliveryPartnerId', 'name deliveryId phone email profileImage').populate('processedBy', 'name email').lean();
    if (!history) {
    ...
    logger.error(`Error fetching history record: ${error.message}`, {
      error: error.stack
    });
...
export const creditEarningToWallet = asyncHandler(async (req, res) => {
  try {
    ...
    logger.error(`Error crediting earning: ${error.message}`, {
      error: error.stack
    });
...
export const cancelEarningAddonHistory = asyncHandler(async (req, res) => {
  try {
    ...
    logger.error(`Error cancelling earning: ${error.message}`, {
      error: error.stack
    });
