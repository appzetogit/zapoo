...
export const getDeliveryEarnings = asyncHandler(async (req, res) => {
  try {
    ...
    const deliveries = await Delivery.find(deliveryQuery).select('_id name phone email deliveryId status').lean();
    ...
    for (const wallet of wallets) {
      ...
      transactions = transactions.filter(t => t.type === 'payment' && t.status === 'Completed');
      ...
      const orderIds = transactions.filter(t => t.orderId).map(t => {
      ...
      if (orderIds.length > 0) {
        try {
          ...
          }).select('orderId status createdAt deliveredAt pricing.total pricing.deliveryFee restaurantName address').lean();
        } catch (orderError) {
