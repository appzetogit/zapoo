...
export const getAllReviews = asyncHandler(async (req, res) => {
  try {
    ...
    const reviews = await Order.find(query).populate('userId', 'name phone email').populate('restaurantId', 'name').populate('deliveryPartnerId', 'name phone').select('orderId restaurantId restaurantName userId review deliveredAt createdAt items deliveryPartnerId').sort(sortOptions).skip(skip).limit(limitNum).lean();
...
export const getReviewByOrderId = asyncHandler(async (req, res) => {
  try {
    ...
    }).populate('userId', 'name phone email').populate('restaurantId', 'name').select('orderId restaurantId restaurantName userId review deliveredAt createdAt').lean();
    if (!order) {
...
export const getReviewsByRestaurant = asyncHandler(async (req, res) => {
  try {
    ...
    const reviews = await Order.find(query).populate('userId', 'name phone email').select('orderId userId review deliveredAt createdAt').sort(sortOptions).skip(skip).limit(limitNum).lean();
    const totalReviews = await Order.countDocuments(query);
    const avgRatingResult = await Order.aggregate([{
      $match: query
    }, {
...
export const getDeliverymanReviews = asyncHandler(async (req, res) => {
  try {
    ...
    const reviews = await Order.find(query).populate('deliveryPartnerId', 'name phone').populate('userId', 'name phone email').select('orderId deliveryPartnerId userId review deliveredAt createdAt').sort(sortOptions).skip(skip).limit(limitNum).lean();
    // Get total count
    const totalReviews = await Order.countDocuments(query);
