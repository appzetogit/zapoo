...
export const getPublicEnvVariables = asyncHandler(async (req, res) => {
  try {
    ...
    logger.error(`Error fetching public environment variables: ${error.message}`, {
      stack: error.stack
    });
...
export const saveEnvVariables = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (error.errors) {
      const validationErrors = Object.keys(error.errors).map(key => `${key}: ${error.errors[key].message}`).join(', ');
