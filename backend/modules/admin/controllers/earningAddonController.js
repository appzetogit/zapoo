...
export const createEarningAddon = asyncHandler(async (req, res) => {
  try {
    ...
    logger.error(`Error creating earning addon: ${error.message}`, {
      error: error.stack
    });
...
export const getEarningAddons = asyncHandler(async (req, res) => {
  try {
    ...
    const [earningAddons, total] = await Promise.all([EarningAddon.find(query).populate('createdBy', 'name email').sort(sort).skip(skip).limit(parseInt(limit)).lean(), EarningAddon.countDocuments(query)]);
    ...
    const now = new Date();
    const addonsWithValidity = await Promise.all(earningAddons.map(async addon => {
      try {
        // Convert addon._id to ObjectId (lean() returns plain objects)
        const addonId = mongoose.Types.ObjectId.isValid(addon._id) ? new mongoose.Types.ObjectId(addon._id.toString()) : addon._id;
        ...
        if (actualRedemptions !== (addon.currentRedemptions || 0)) {
          try {
            await EarningAddon.updateOne({
              ...
              $set: {
                currentRedemptions: actualRedemptions
              }
            });
          } catch (updateError) {
        ...
        const isValid = addon.status === 'active' && now >= new Date(addon.startDate) && now <= new Date(addon.endDate) && (addon.maxRedemptions === null || actualRedemptions < addon.maxRedemptions);
        ...
        return {
          ...addon,
          currentRedemptions: finalRedemptions,
          // Use actual count from history
          isValid
        };
      } catch (error) {
...
export const getEarningAddonById = asyncHandler(async (req, res) => {
  try {
    ...
    const earningAddon = await EarningAddon.findById(id).populate('createdBy', 'name email').populate('applicableZones', 'name').lean();
    if (!earningAddon) {
    ...
    // Check validity
    const now = new Date();
    const isValid = earningAddon.status === 'active' && now >= new Date(earningAddon.startDate) && now <= new Date(earningAddon.endDate) && (earningAddon.maxRedemptions === null || earningAddon.currentRedemptions < earningAddon.maxRedemptions);

    // Get completion statistics
    const [totalCompletions, pendingCompletions, creditedCompletions] = await Promise.all([EarningAddonHistory.countDocuments({
      earningAddonId: id
    ...
    logger.error(`Error fetching earning addon: ${error.message}`, {
      error: error.stack
    });
...
export const updateEarningAddon = asyncHandler(async (req, res) => {
  try {
    ...
    logger.error(`Error updating earning addon: ${error.message}`, {
      error: error.stack
    });
...
export const deleteEarningAddon = asyncHandler(async (req, res) => {
  try {
    ...
    const completionsCount = await EarningAddonHistory.countDocuments({
      earningAddonId: id
    });
    ...
    logger.error(`Error deleting earning addon: ${error.message}`, {
      error: error.stack
    });
...
export const toggleEarningAddonStatus = asyncHandler(async (req, res) => {
  try {
    ...
    logger.error(`Error updating earning addon status: ${error.message}`, {
      error: error.stack
    });
...
export const checkEarningAddonCompletions = asyncHandler(async (req, res) => {
  try {
    ...
    for (const addon of activeAddons) {
      ...
      if (completedOrders >= addon.requiredOrders) {
        ...
        const orders = await Order.find({
          ...
          deliveredAt: 1
        }).limit(addon.requiredOrders).select('orderId').lean();
    ...
    logger.error(`Error checking earning addon completions: ${error.message}`, {
      error: error.stack
    });
