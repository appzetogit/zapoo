...
function isPointInZone(lat, lng, zoneCoordinates) {
  ...
  for (let i = 0, j = zoneCoordinates.length - 1; i < zoneCoordinates.length; j = i++) {
    ...
    const xi = typeof coordI === 'object' ? coordI.latitude || coordI.lat : Array.isArray(coordI) ? coordI[0] : null;
    const yi = typeof coordI === 'object' ? coordI.longitude || coordI.lng : Array.isArray(coordI) ? coordI[1] : null;
    const xj = typeof coordJ === 'object' ? coordJ.latitude || coordJ.lat : Array.isArray(coordJ) ? coordJ[0] : null;
    const yj = typeof coordJ === 'object' ? coordJ.longitude || coordJ.lng : Array.isArray(coordJ) ? coordJ[1] : null;
...
export async function findNearestRestaurant(deliveryLat, deliveryLng, orderItems = []) {
  try {
    ...
    if (!deliveryLat || !deliveryLng || typeof deliveryLat !== 'number' || typeof deliveryLng !== 'number' || deliveryLat < -90 || deliveryLat > 90 || deliveryLng < -180 || deliveryLng > 180) {
...
export async function assignOrderToNearestRestaurant(orderData) {
  try {
    const deliveryLocation = orderData.address?.location?.coordinates || [orderData.address?.location?.longitude || 0, orderData.address?.location?.latitude || 0];
    ...
    const nearestRestaurant = await findNearestRestaurant(deliveryLat, deliveryLng, orderData.items || []);
