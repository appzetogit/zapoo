...
class EmailService {
  ...
  async initializeTransporter() {
    ...
    const hasSMTPConfig = (smtpCreds.user || process.env.SMTP_USER) && (smtpCreds.pass || process.env.SMTP_PASS) && (smtpCreds.host || process.env.SMTP_HOST);
  ...
  async sendWelcomeEmail(email, name) {
    ...
    const html = templates.welcomeUser({
      companyName,
      name,
      dashboardUrl
    });
  ...
  async sendBookingConfirmation(email, booking) {
    ...
    const html = templates.bookingConfirmation({
      ...
      userName: "Guest",
      // Ideally get from user obj if available
      bookingUrl
    });
    return this.sendEmail({
      to: email,
