  ...
  async initializeCredentials() {
    ...
    if (process.env.NODE_ENV === "development") {
      if (!this.apiKey || !this.senderId) {
        ...
        console.warn("   Please check SMSINDIAHUB_API_KEY and SMSINDIAHUB_SENDER_ID in .env file");
      } else {}
    }
  }
  ...
  async sendOTP(phone, otp, purpose = 'register') {
    try {
      ...
      if (!apiKey || !senderId) {
        ...
        console.error("   SMSINDIAHUB_API_KEY:", apiKey ? "✓ Set" : "✗ Missing");
        console.error("   SMSINDIAHUB_SENDER_ID:", senderId ? "✓ Set" : "✗ Missing");
        throw new Error("SMSIndia Hub not configured. Please check your environment variables SMSINDIAHUB_API_KEY and SMSINDIAHUB_SENDER_ID in .env file.");
      }
      ...
      const params = new URLSearchParams({
        ...
        msg: message,
        fl: "0",
        // Flash message flag (0 = normal SMS)
        dc: "0",
        // Delivery confirmation (0 = no confirmation)
      ...
      if (parsedResponse && typeof parsedResponse === "object") {
        if (parsedResponse.ErrorCode === "000" && parsedResponse.ErrorMessage === "Done") {
          const messageId = parsedResponse.MessageData && parsedResponse.MessageData[0] ? parsedResponse.MessageData[0].MessageId : `sms_${Date.now()}`;
          return {
            success: true,
            messageId: messageId,
  ...
  async getBalance() {
    try {
      ...
      if (!apiKey) {
        console.error("❌ SMSIndia Hub Configuration Error:");
        console.error("   SMSINDIAHUB_API_KEY:", apiKey ? "✓ Set" : "✗ Missing");
        throw new Error("SMSIndia Hub not configured. Please check your environment variable SMSINDIAHUB_API_KEY in .env file.");
      }
