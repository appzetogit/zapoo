...
export const resetPassword = asyncHandler(async (req, res) => {
  ...
  const user = await User.findOne(findQuery).select("+password");
  if (!user) {
    return errorResponse(res, 404, role ? `No ${role} account found with this email.` : "User not found");
  }
...
const finalizeGoogleLogin = async ({
  ...
  if (!email) {
    ...
    return errorResponse(res, 400, `${sourceLabel} login failed: email not found.`);
  }
  ...
  if (!emailRegex.test(email)) {
    ...
    return errorResponse(res, 400, `Invalid email format received from ${sourceLabel}.`);
  }
  ...
  if (user) {
    ...
    try {
      ...
      if (createError.code === 11000) {
        logger.warn("Duplicate key error during Google user creation, retrying role-scoped find", {
          email,
          role: userRole
        });
        ...
        logger.error(`${sourceLabel} login failed while creating user`, {
  ...
  return successResponse(res, 200, `${sourceLabel} authentication successful`, {
    accessToken: tokens.accessToken,
    user: {
      id: user._id,
      name: user.name,
 ...
 * POST /api/auth/google/native-login
 */
  ...
  try {
    ...
    if (accessToken) {
      ...
      googleUser = {
        ...
        name: payload?.name || payload?.display_name || payload?.given_name || "Google User",
...
export const googleAuth = asyncHandler(async (req, res) => {
  ...
  try {
    const {
      authUrl,
      state
    } = googleAuthService.getAuthUrl(userRole);
...
export const googleCallback = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const redirectPath = userRole === "restaurant" ? "/restaurant/auth/google-callback" : userRole === "delivery" ? "/delivery/auth/google-callback" : "/user/auth/google-callback";
    const userData = {
      id: user._id,
      name: user.name,
      email: user.email,
