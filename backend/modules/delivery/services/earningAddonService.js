    ...
    for (const offer of activeOffers) {
      try {
        ...
        const orderCount = await Order.countDocuments({
          ...
          deliveredAt: {
            $gte: countFromDate,
            // Count from offer creation/start date
            $lte: endDate
          }
        });
        ...
        if (orderCount >= offer.requiredOrders) {
          ...
          if (wallet) {
            const existingBonus = wallet.transactions.find(t => {
              ...
              if (metadata) {
                const earningAddonId = metadata instanceof Map ? metadata.get('earningAddonId') : metadata.earningAddonId;
          ...
          if (mongoose.Types.ObjectId.isValid(offer._id)) {
            offerId = typeof offer._id === 'string' ? new mongoose.Types.ObjectId(offer._id) : offer._id;
          }
          ...
          const history = await EarningAddonHistory.create({
            ...
            completedAt: now,
            status: 'pending',
            // Status is 'pending' - admin will credit manually
          ...
          return {
            ...
            status: 'pending',
            // Status is pending, not credited yet
...
export const getDeliveryBoyOfferProgress = async deliveryId => {
  try {
    ...
    const offersWithProgress = await Promise.all(activeOffers.map(async offer => {
      // Use the later of: offer creation date or offer start date
      const offerStartDate = new Date(offer.startDate);
      const offerCreatedAt = offer.createdAt ? new Date(offer.createdAt) : offerStartDate;
      // Count orders from when offer was created (or start date, whichever is later)
      ...
      const isValid = offer.status === 'active' && now >= startDate && now <= endDate && (offer.maxRedemptions === null || offer.currentRedemptions < offer.maxRedemptions);
      return {
        ...offer,
        isValid,
