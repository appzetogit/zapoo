  ...
  try {
    ...
    for (const point of points) {
      ...
      for (const [cachedKey, cached] of snapToRoadCache.entries()) {
        ...
        const distance = calculateDistanceQuick(point, {
          lat: cachedLat,
          lng: cachedLng
        });
      ...
      try {
        const response = await axios.get(`https://roads.googleapis.com/v1/snapToRoads`, {
          params: {
            path,
            interpolate: true,
            // Interpolate points between snapped points
            key: apiKey
          },
...
export function smoothLocation(riderId, location) {
  ...
  const avgSpeed = speeds.length > 0 ? Math.max(10, Math.min(45, speeds.reduce((sum, s) => sum + s, 0) / speeds.length)) : 20; // Default 20 km/h
...
export async function calculateRouteProgress(orderId, currentLocation) {
  ...
  const progress = route.totalDistance > 0 ? Math.min(1, Math.max(0, distanceCovered / route.totalDistance)) : 0;
...
export async function processLocationUpdate(riderId, orderId, rawLocation, routeInfo) {
  try {
    ...
    const snappedPoints = await snapToRoad([{
      lat: rawLocation.lat,
      lng: rawLocation.lng
    }], riderId);
    ...
    if (!routePolylines.has(orderId) && routeInfo) {
      const route = await generateRoutePolyline(smoothedLocation, routeInfo.restaurant, routeInfo.customer);
      if (route) {
