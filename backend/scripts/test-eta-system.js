...
const TEST_CONFIG = {
  restaurantId: null,
  // Will be set from first restaurant
  userId: null,
  // Will be set from first user
  deliveryPartnerId: null,
  // Will be set from first delivery partner
  ...
  try {
    ...
    const order = new Order({
      ...
      address: {
        ...
        location: {
          type: 'Point',
          coordinates: [restaurant.location.longitude + 0.01, restaurant.location.latitude + 0.01]
        }
      },
      pricing: {
        subtotal: 100,
...
async function testRestaurantAccepts() {
  ...
  try {
    ...
    const etaLog = await ETALog.findOne({
      orderId: order._id
    }).sort({
      calculatedAt: -1
    });
