...
const createCustomAdmin = async () => {
    try {
        ...
        if (existingAdmin) {
            console.log("⚠️  Admin already exists with this email:", adminCredentials.email);
            // Optional: Update password if needed
            existingAdmin.password = adminCredentials.password;
            await existingAdmin.save();
            console.log("✅ Admin password updated to provided credential.");
            process.exit(0);
        }

        const newAdmin = await Admin.create({
            name: "Ajay Panchal", // Inferring name from email, can be generic
            email: adminCredentials.email,
            password: adminCredentials.password,
            role: "admin",
            isActive: true,
            phoneVerified: true, // Assuming verified since credentials are provided
            permissions: [
                'dashboard_view',
                'admin_manage',
                'restaurant_manage',
