...
const verifyPricing = async () => {
    try {
        ...
        await Zone.deleteMany({ name: /^Test Zone Tier/ });
        await Restaurant.deleteMany({ name: /^Test Restaurant Tier/ });
        console.log("Cleaned up old test data.");

        // 3. Find or Create a Restaurant for EACH Tier
        for (const tier of tiers) {
            console.log(`\n--- Testing Tier ${tier.rank} (${tier.name}) ---`);

            // Find a zone with this tier
            let zone = await Zone.findOne({ tierId: tier._id });
            if (!zone) {
                console.log(`No zone found for Tier ${tier.rank}. Creating one...`);
                ...
                const coords = [
                    [0, 0],
                    [0, sideLengthDeg],
                    [sideLengthDeg, sideLengthDeg],
                    [sideLengthDeg, 0],
                    [0, 0]
                ].map(p => ({ latitude: p[1], longitude: p[0] })); // Lat is Y, Long is X
            ...
            let restaurant = await Restaurant.findOne({ zoneId: zone._id });
            if (!restaurant) {
                console.log(`No restaurant found in Zone ${zone.name}. Creating one...`);
            ...
            if (res.statusCode === 200 && res.data.success) {
                ...
                fetchedPlans.forEach(plan => {
                    ...
                    if (plan.price === expectedPrice) {
                        console.log(`✅ ${plan.name}: Price ${plan.price} matches Tier ${tier.rank} price.`);
                    } else {
