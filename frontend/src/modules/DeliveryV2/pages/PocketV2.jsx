...
export const PocketV2 = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const [profileResult, earningsResult, walletResult] = await Promise.allSettled([
        ...
        if (walletResult.status === 'rejected') {
          console.warn('[PocketV2] wallet fetch failed, using safe defaults', walletResult.reason);
        }
      } catch (err) {
