...
import { useDeliveryNotificationContext } from '@/modules/DeliveryV2/components/DeliveryNotificationProvider';
...
import { 
  ...
  Contact, Package, Phone, Loader2, ArrowLeft
} from 'lucide-react';
...
export default function DeliveryHomeV2({ tab = 'feed' }) {
  ...
  const {
    ...
    emitLocation,
    incomingOrder,
    isModalMinimized: isOfferModalMinimized,
    setIsModalMinimized: setOfferModalMinimized,
      ...
      try {
        ...
        if (serverData) {
          ...
          const cusLoc =
            ...
            getLoc(serverData.deliveryAddress, ['latitude', 'lat'], ['longitude', 'lng']) ||
            getLoc(serverData.customerLocation, ['latitude', 'lat'], ['longitude', 'lng']) ||
  ...
  return (
    <div className="relative h-screen w-full bg-white text-gray-900 overflow-hidden flex flex-col">
      ...
      {(currentTab === 'pocket' || currentTab === 'profile') && (
        <div className="absolute top-0 inset-x-0 z-[120] safe-top bg-[#121212] border-b border-white/10 backdrop-blur-2xl">
          <div className="px-6 py-3 flex items-center justify-between">
            <div className="flex items-center gap-4">
              <button
                onClick={() => navigate('/food/delivery/feed')}
                className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center text-white border border-white/10 active:scale-90 transition-all"
              >
                <ArrowLeft className="w-5 h-5" />
              </button>
              <div>
                <h1 className="text-xl font-black text-white uppercase tracking-tighter">
                  {currentTab === 'pocket' ? 'Pocket' : 'Profile'}
                </h1>
                <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mt-0.5">
                  {currentTab === 'pocket' ? 'Your payout milestones' : 'Your delivery milestones'}
                </p>
              </div>
            </div>
            <div className="w-10 h-10 rounded-xl bg-green-500/10 flex items-center justify-center text-[#10B981] border border-green-500/20">
              {currentTab === 'pocket' ? <Wallet className="w-5 h-5" /> : <UserIcon className="w-5 h-5" />}
            </div>
          </div>
        </div>
      )}
      ...
      <div
        ...
        className={`flex-1 relative overflow-y-auto ${currentTab === 'feed' ? 'pt-[120px]' : currentTab === 'history' ? 'pt-0' : 'pt-[61px]'} no-scrollbar`}
      >
         {currentTab === 'feed' ? (
      ...
      {(isModalMinimized || isOfferModalMinimized) && (activeOrder || incomingOrder || showVerification) && (
        <motion.div 
