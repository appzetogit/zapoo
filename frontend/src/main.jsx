...
(async () => {
  ...
  try {
    ...
    if (googleMapsApiKey) {
      ...
      script.src = `https://maps.googleapis.com/maps/api/js?key=${googleMapsApiKey}&libraries=places,geometry,drawing,geocoding`;
      script.async = true;
      script.defer = true;
      script.onload = () => {
        window.__googleMapsLoaded = true;
...
console.error = (...args) => {
  ...
  // Suppress browser extension errors
  if (typeof args[0] === 'string' && (args[0].includes('chrome-extension://') || args[0].includes('_$initialUrl') || args[0].includes('_$onReInit') || args[0].includes('_$bindListeners'))) {
    return; // Suppress browser extension errors
  }

  // Suppress geolocation errors (non-critical, will retry or use fallback)
  if (errorStr.includes('Timeout expired') || errorStr.includes('GeolocationPositionError') || errorStr.includes('Geolocation error') || errorStr.includes('User denied Geolocation') || errorStr.includes('permission denied') || errorStr.includes('code: 3') && errorStr.includes('location') || errorStr.includes('code: 1') && errorStr.includes('location')) {
  ...
  if (errorStr.includes('🌐 Network Error') || errorStr.includes('Network Error - Backend server may not be running') || errorStr.includes('ERR_NETWORK') && errorStr.includes('AxiosError') || errorStr.includes('💡 API Base URL:') || errorStr.includes('💡 Backend URL:') || errorStr.includes('💡 Start backend with:') || errorStr.includes('💡 Check backend health:') || errorStr.includes('💡 Make sure backend server is running:') || errorStr.includes('❌ Backend not accessible at:') || errorStr.includes('💡 Start backend:')) {
  ...
  if (errorStr.includes('timeout of') || errorStr.includes('ECONNABORTED') || errorStr.includes('AxiosError') && errorStr.includes('timeout')) {
  ...
  if (errorStr.includes('OTP Verification Error:') || errorStr.includes('AxiosError') && errorStr.includes('Request failed with status code 403') && errorStr.includes('verify-otp')) {
  ...
  if (errorStr.includes('Restaurant Socket connection error') || errorStr.includes('xhr poll error') || typeof args[0] === 'object' && args[0]?.type === 'TransportError' && args[0]?.message?.includes('xhr poll error')) {
    return;
  }
...
window.addEventListener('unhandledrejection', event => {
  ...
  if (errorMsg.includes('Timeout expired') || errorMsg.includes('User denied Geolocation') || errorMsg.includes('permission denied') || errorName === 'GeolocationPositionError' || error?.code === 3 && errorMsg.includes('timeout') || error?.code === 1 && (errorMsg.includes('location') || errorMsg.includes('geolocation'))) {
  ...
  if (errorStr.includes('Error processing refund') || errorName === 'AxiosError' && errorMsg.includes('refund')) {
