...
export default function Orders() {
  ...
  useEffect(() => {
    ...
    const deliveredOrders = orders.filter(order => {
      ...
      const isDelivered = originalStatus === 'delivered' || originalStatus === 'completed' || originalStatus.toLowerCase() === 'delivered' || originalStatus.toLowerCase() === 'completed' || transformedStatus === 'delivered' || transformedStatus === 'completed' || transformedStatus.toLowerCase() === 'delivered' || transformedStatus.toLowerCase() === 'completed';

      // Check if order has rating - check multiple places where rating might be stored
      const hasRating = order.rating !== null && order.rating !== undefined && order.rating !== '' || order.review?.rating !== null && order.review?.rating !== undefined || order.review !== null && order.review !== undefined;
        ...
        if (ordersData.length > 0) {
          ...
          const transformedOrders = ordersData.map(order => {
            ...
            return {
              ...
              // Check both rating and review.rating
  ...
  const filteredOrders = orders.filter(order => {
    ...
    const itemsMatch = order.items.some(item => (item.name || item.foodName || '').toLowerCase().includes(query));
        ...
        return <div key={order.id} className="relative bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
              ...
              return <div key={item._id || item.id || item.itemId || idx} className="flex items-start gap-3">
                ...
                <div className="px-4 py-3 flex items-center justify-between">
                  ...
                  {isRestaurantCancelled ? <div className="flex flex-col gap-1">
          ...
          <div className="w-full max-w-md rounded-3xl bg-white shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
            ...
            <div className="px-6 py-6">
              ...
              <div className="mb-6">
                ...
                return <button key={num} type="button" onClick={() => setSelectedRating(num)} className="p-2 transition-transform hover:scale-125 active:scale-95">
                        <Star className={`w-10 h-10 transition-all ${isActive ? "text-yellow-400 fill-yellow-400 drop-shadow-lg" : "text-gray-300 hover:text-yellow-200"}`} />
                      </button>;
              })}
                </div>
