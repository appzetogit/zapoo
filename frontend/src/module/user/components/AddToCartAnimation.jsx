...
export default function AddToCartAnimation({
    ...
    if (lastAddEvent && lastAddEvent.sourcePosition && linkRef.current) {
      ...
      setTimeout(() => {
        if (flyingThumbnailRef.current && linkRef.current) {
          ...
          .to(thumbnail, {
            x: deltaX * 0.98,
            // Slight X overshoot for bounce
            y: deltaY,
            // No overshoot on Y to prevent going below pill
            rotation: 360,
            scale: 1.1,
            duration: 0.4,
            ease: 'power2.inOut'
  ...
  return createPortal(<>
      ...
      objectFit: 'cover'
          ...
          {removedProduct.product?.imageUrl ? <img src={removedProduct.product.imageUrl} alt={removedProduct.product.name} className="w-full h-full object-cover rounded-full" /> : <div className="w-full h-full flex items-center justify-center bg-neutral-200 text-neutral-400 text-xs font-semibold rounded-full">
              {removedProduct.product?.name?.charAt(0).toUpperCase() || '?'}
            </div>}
        </div>}

      {/* Flying product thumbnail - going to cart */}
      {flyingProduct && <div ref={flyingThumbnailRef} className="w-8 h-8 rounded-full border-2 border-white overflow-hidden bg-white flex-shrink-0 shadow-lg" style={{
      borderRadius: '50%',
      objectFit: 'cover'
    }}>
          {flyingProduct.product.imageUrl ? <img src={flyingProduct.product.imageUrl} alt={flyingProduct.product.name} className="w-full h-full object-cover rounded-full" /> : <div className="w-full h-full flex items-center justify-center bg-neutral-200 text-neutral-400 text-xs font-semibold rounded-full">
        ...
        pointerEvents: isScrollTemporarilyHidden ? 'none' : 'auto'
          ...
          navigate(linkTo);
              ...
              <div className="flex items-center -space-x-4">
                {thumbnailItems.map((item, idx) => <motion.div key={item.product.id} initial={{
              scale: 0,
              opacity: 0
            }} animate={{
              scale: 1,
              ...
              damping: 25
                    ...
                    {item.product.imageUrl ? <img src={item.product.imageUrl} alt={item.product.name} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center bg-neutral-200 text-neutral-400 text-xs font-semibold">
            ...
            duration: 0.3
              ...
              {/* Right: Arrow icon */}
              <motion.div className="ml-auto bg-white/25 rounded-full p-1 backdrop-blur-sm" initial={{
            opacity: 0,
            x: 10
          }} animate={{
            opacity: 1,
