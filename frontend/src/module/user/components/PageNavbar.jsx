...
export default function PageNavbar({
  ...
  const locationDisplay = (() => {
    ...
    if (location?.mainTitle && location.mainTitle.trim() !== "" && location.mainTitle !== "Location Found") {
      ...
      if (location?.area && location.area.trim() !== "" && location.area.toLowerCase() !== location.mainTitle.toLowerCase() && location.area.toLowerCase() !== location?.city?.toLowerCase()) {
    ...
    if (!mainLocation && location?.formattedAddress && !isCoordinates(location.formattedAddress) && location.formattedAddress !== "Select location") {
      ...
      if (formattedParts.length >= 4) {
        ...
        let cityIndex = -1;
        if (location?.city) {
          cityIndex = formattedParts.findIndex(part => part.toLowerCase() === location.city.toLowerCase() || part.toLowerCase().includes(location.city.toLowerCase()));
        }

        // If city not found, look for state (city is usually before state)
        if (cityIndex === -1 && location?.state) {
          const stateIndex = formattedParts.findIndex(part => part.toLowerCase().includes("madhya") || part.toLowerCase().includes("pradesh") || part.toLowerCase().includes(location.state.toLowerCase()));
          if (stateIndex > 0) {
    ...
    if (!mainLocation && location?.address && !isCoordinates(location.address) && location.address !== "Select location") {
      ...
      const cityInAddress = addressParts.some(part => part.toLowerCase() === location?.city?.toLowerCase() || part.toLowerCase() === "indore" || part.toLowerCase() === "bhopal");
      ...
      if (!cityInAddress && addressParts.length > 0) {
        ...
        let cityIndex = filteredParts.findIndex(part => part.toLowerCase() === location?.city?.toLowerCase() || part.toLowerCase() === "indore" || part.toLowerCase() === "bhopal");
        if (cityIndex > 0) {
    ...
    if (!mainLocation && location?.formattedAddress && !isCoordinates(location.formattedAddress) && location.formattedAddress !== "Select location") {
      ...
      if (location?.city) {
        cityIndex = filteredParts.findIndex(part => part.toLowerCase() === location.city.toLowerCase());
      }
      ...
      if (cityIndex === -1) {
        ...
        cityIndex = filteredParts.findIndex(part => commonCities.some(city => part.toLowerCase() === city.toLowerCase()));
      }

      // Method 3: Check if part contains state name (usually comes after city)
      if (cityIndex === -1 && location?.state) {
        const stateIndex = filteredParts.findIndex(part => part.toLowerCase().includes(location.state.toLowerCase()) || part.toLowerCase().includes("madhya") || part.toLowerCase().includes("pradesh"));
        if (stateIndex > 0) {
      ...
      if (cityIndex === -1) {
        const stateIndex = filteredParts.findIndex(part => part.toLowerCase().includes("madhya") || part.toLowerCase().includes("pradesh") || part.toLowerCase().includes("maharashtra") || part.toLowerCase().includes("gujarat"));
        if (stateIndex > 0) {
          cityIndex = stateIndex - 1;
        }
      }
      if (cityIndex > 0) {
        ...
        const stateIndex = filteredParts.findIndex(part => part.toLowerCase().includes("madhya") || part.toLowerCase().includes("pradesh") || location?.state && part.toLowerCase().includes(location.state.toLowerCase()));
        if (stateIndex > 0) {
    ...
    if (!mainLocation && location?.address && !isCoordinates(location.address) && location.address !== "Select location") {
      ...
      if (filteredParts.length === 0 || filteredParts.length === 1 && (filteredParts[0].toLowerCase() === "select location" || filteredParts[0].toLowerCase() === "current location")) {} else {
        ...
        if (cityIndex === -1) {
          ...
          cityIndex = filteredParts.findIndex(part => commonCities.some(city => part.toLowerCase().includes(city.toLowerCase())));
        }
        if (cityIndex === -1 && location?.state) {
          const stateIndex = filteredParts.findIndex(part => part.toLowerCase().includes(location.state.toLowerCase()));
          if (stateIndex > 0) {
            cityIndex = stateIndex - 1;
          }
        }
        if (cityIndex > 0) {
          ...
          const firstPart = filteredParts[0];
          if (!isCoordinates(firstPart) && firstPart.length > 2 && firstPart.toLowerCase() !== "select location" && firstPart.toLowerCase() !== "current location") {
            mainLocation = firstPart;
          }
        }
      }
    ...
    if (!mainLocation) {
      ...
      if (addressToUse && !isCoordinates(addressToUse) && addressToUse !== "Select location" && addressToUse.trim() !== "") {
    ...
    if (!mainLocation && location?.area && location.area.trim() !== "" && !isCoordinates(location.area)) {
      ...
      if (location?.city && location.city.trim() !== "" && location.city !== "Unknown City" && location.area.toLowerCase() !== location.city.toLowerCase()) {
    ...
    else if (!mainLocation && location?.city && location.city.trim() !== "" && location.city !== "Unknown City" && location.city !== "Current Location" && location.city !== "Select location") {
    ...
    if (mainLocation && (mainLocation.toLowerCase() === location?.city?.toLowerCase() || mainLocation === "Indore")) {
      ...
      if (location?.area && location.area.trim() !== "" && location.area.toLowerCase() !== location?.city?.toLowerCase() && !isCoordinates(location.area)) {
        mainLocation = `${location.area}, ${location.city}`;
        ...
        let cityIndex = -1;
        if (location?.city) {
          cityIndex = parts.findIndex(part => part.toLowerCase() === location.city.toLowerCase() || part.toLowerCase().includes(location.city.toLowerCase()));
        }
        ...
        if (cityIndex > 0) {
          ...
          if (areaParts.length > 0) {
            ...
            if (extractedArea && extractedArea.toLowerCase() !== location.city.toLowerCase() && !extractedArea.match(/^\d+/) && extractedArea.length > 2 && !extractedArea.toLowerCase().includes("madhya") && !extractedArea.toLowerCase().includes("pradesh")) {
          ...
          if (filteredParts.length >= 1) {
            ...
            if (extractedArea && extractedArea.toLowerCase() !== location.city.toLowerCase() && extractedArea.length > 2 && !extractedArea.match(/^\d+/)) {
              mainLocation = `${extractedArea}, ${location.city}`;
            }
          }
        }
      ...
      // If area field is empty, try to extract from formattedAddress one more time
        ...
        if (parts.length >= 3) {
          ...
          for (let i = parts.length - 3; i >= 0; i--) {
            const part = parts[i];
            if (part && part.toLowerCase() !== location.city.toLowerCase() && !part.match(/^\d+/) && part.length > 2 && !part.toLowerCase().includes("madhya") && !part.toLowerCase().includes("pradesh") && part.toLowerCase() !== "india") {
      ...
      if (location?.formattedAddress) {
        ...
        if (parts.length >= 4) {
          ...
          if (!subLocation && parts.length >= 6) {
            ...
            if (cityPart && statePart && !cityPart.match(/^\d+$/) && !statePart.match(/^\d+$/) && cityPart.length > 1 && statePart.length > 1) {
              subLocation = `${cityPart}, ${statePart}`;
            } else {}
          }
        }
