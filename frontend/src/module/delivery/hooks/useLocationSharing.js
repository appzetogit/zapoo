    ...
    if (navigator.geolocation) {
      watchIdRef.current = navigator.geolocation.watchPosition(position => {
        ...
        if (lastLocationRef.lat !== null && lastLocationRef.lng !== null) {
          const distance = calculateDistance(lastLocationRef.lat, lastLocationRef.lng, latitude, longitude);
        ...
        if (socketRef.current && socketRef.current.connected) {
          socketRef.current.emit('update-location', {
            ...
            lng: longitude,
            heading: heading || 0,
            // heading may not be available on all devices
            timestamp: now
          });
        }
      }, error => {
        ...
        // Force GPS for accuracy
        maximumAge: 0,
        // No cache, always get fresh location
