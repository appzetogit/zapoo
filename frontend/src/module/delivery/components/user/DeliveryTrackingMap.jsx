...
const DeliveryTrackingMap = ({
  ...
  useEffect(() => {
    ...
    const applyIncomingLocation = (data) => {
    ...
    socketRef.current.on('connect', () => {
      trackingIds.forEach(id => {
        ...
        const liveEvent = `location-receive-${id}`;
        const currentEvent = `current-location-${id}`;
        const liveHandler = (data) => applyIncomingLocation(data);
        const currentHandler = (data) => applyIncomingLocation(data);
        socketRef.current.on(liveEvent, liveHandler);
        socketRef.current.on(currentEvent, currentHandler);
        locationHandlers.push([liveEvent, liveHandler], [currentEvent, currentHandler]);
      });
    });

    // Backward compatibility fallback if any emitter still sends generic event.
    ...
    return () => {
      ...
      if (socketRef.current) {
        locationHandlers.forEach(([eventName, handler]) => {
