...
function BottomPopup({
  ...
  return <div className="fixed inset-0 z-[60] flex items-end" onClick={closeOnBackdropClick ? onClose : undefined}>
      ...
      stiffness: 300,
      damping: 30
    }} className="relative w-full bg-white rounded-t-2xl shadow-xl p-4" style={{
      maxHeight,
      overflow: "auto"
    }} onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-lg font-semibold">{title}</h2>
          {showCloseButton && <button onClick={onClose} className="text-gray-500 hover:text-gray-700 text-sm px-3 py-1 rounded-md">
              Close
            </button>}
        </div>
        {children}
  ...
  useEffect(() => {
    const fetchProfileImage = async () => {
      try {
        ...
        if (error.code !== 'ECONNABORTED' && error.code !== 'ERR_NETWORK' && error.message !== 'Network Error' && !error.message?.includes('timeout')) {
  ...
  return <>
      <div className={`bg-white px-4 py-3 flex items-center justify-between sticky top-0 z-50 border-b border-gray-200 ${className}`}>
        ...
        zIndex: 100
          ...
          <button onClick={handleToggle} onTouchStart={e => e.stopPropagation()} className="focus:outline-none relative cursor-pointer" type="button" role="switch" aria-checked={isOnline} style={{
          pointerEvents: "auto",
          zIndex: 100,
          WebkitTapHighlightColor: "transparent"
        }}>
            <div className={`relative w-20 h-8 rounded-full transition-colors duration-300 ${isOnline ? "bg-green-500" : "bg-gray-400"}`}>
              <span className={`text-[11px] font-bold text-white absolute top-1/2 -translate-y-1/2 whitespace-nowrap transition-all duration-300 ${isOnline ? "left-2" : "right-2"}`} style={{
              opacity: 1,
              zIndex: 2,
              pointerEvents: "none"
            }}>
              ...
              <motion.div className="absolute top-1 w-6 h-6 bg-white rounded-full shadow-lg" animate={{
              x: isOnline ? 48 : 2
            }} transition={{
              type: "spring",
              stiffness: 500,
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => setShowEmergencyPopup(true)} className="w-10 h-10 rounded-full bg-gray-200 flex items-center justify-center hover:bg-[#DC2626] transition-colors relative" title="Emergency">
            <svg className="w-5 h-5 text-[#DC2626]" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2.5}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
            </svg>
          </button>

          {/* Help */}
          <button onClick={() => setShowHelpPopup(true)} className="w-10 h-10 rounded-full bg-gray-200 flex items-center justify-center hover:bg-gray-300 transition-colors" title="Help">
          ...
          <button onClick={handleProfileClick} className="w-9 h-9 rounded-full overflow-hidden ring-2 ring-gray-300 flex items-center justify-center bg-gray-200" title="Profile">
            {profileImage && !imageError ? <img src={profileImage} alt="Profile" className="w-full h-full object-cover" onError={() => {
            setImageError(true);
      ...
      <BottomPopup isOpen={showHelpPopup} onClose={() => setShowHelpPopup(false)} title="How can we help?" showCloseButton={true} closeOnBackdropClick={true} maxHeight="70vh">
        <div className="py-2">
          {helpOptions.map(option => <button key={option.id} onClick={() => handleHelpOptionClick(option)} className="w-full flex items-center gap-4 py-4 hover:bg-gray-50 transition-colors border-b border-gray-100 last:border-b-0">
              {/* Icon */}
      ...
      <BottomPopup isOpen={showEmergencyPopup} onClose={() => setShowEmergencyPopup(false)} title="Emergency help" showCloseButton={true} closeOnBackdropClick={true} maxHeight="70vh">
        <div className="py-2">
          {emergencyOptions.map(option => <button key={option.id} onClick={() => {
          option.onClick?.();
          setShowEmergencyPopup(false);
              ...
              <div className="shrink-0 w-12 h-12 rounded-full bg-gray-100 flex items-center justify-center">
                {option.icon === "ambulance" && <Ambulance className="w-6 h-6 text-[#DC2626]" />}
                {option.icon === "accident" && <AlertTriangle className="w-6 h-6 text-[#DC2626]" />}
                {option.icon === "police" && <Shield className="w-6 h-6 text-blue-600" />}
                {option.icon === "insurance" && <ShieldCheck className="w-6 h-6 text-green-600" />}
              </div>
