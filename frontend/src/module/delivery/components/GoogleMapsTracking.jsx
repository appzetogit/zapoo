...
export default function GoogleMapsTracking({
  ...
  const path = [...allSellers, ...(deliveryLocation ? [deliveryLocation] : []), customerLocation].filter(loc => loc && (loc.lat !== 0 || loc.lng !== 0));
  ...
  const calculateAndDisplayRoute = useCallback((origin, destination, waypoints = []) => {
    ...
    if (!directionsRendererRef.current) {
      directionsRendererRef.current = new window.google.maps.DirectionsRenderer({
        ...
        // We'll use custom markers
        preserveViewport: true,
        // Preserve viewport - we'll center manually
        polylineOptions: {
          strokeColor: '#3b82f6',
          // Bright blue like Zomato/Swiggy
          strokeWeight: 6,
          strokeOpacity: 1.0,
          // Fully visible - plain solid line
  ...
  useEffect(() => {
    ...
    if (deliveryLocation.lat === lastDeliveryLocationRef.current.lat && deliveryLocation.lng === lastDeliveryLocationRef.current.lng) {
      return;
    }
  ...
  const containerClasses = isFullScreen ? "fixed inset-0 z-[100] bg-white w-screen h-screen flex flex-col" : "relative mx-4 mt-4 rounded-lg overflow-hidden shadow-sm";
  if (loadError) {
    return <div className={containerClasses + " bg-red-50 border border-red-200 p-4 text-center"}>
        <p className="text-red-800 text-sm">❌ Failed to load Google Maps</p>
      </div>;
  }
  if (!isLoaded) {
    return <div className={containerClasses + " bg-gray-100 p-8 text-center"}>
        <div className="animate-spin text-2xl">🗺️</div>
        <p className="text-gray-600 text-sm mt-2">Loading map...</p>
      </div>;
  }
  if (!apiKey) {
    return <div className={containerClasses + " bg-yellow-50 border border-yellow-200 p-4 text-center"}>
  ...
  return <div className={containerClasses}>
      ...
      <div className={`absolute ${isFullScreen ? 'left-6 top-6' : 'left-3 top-3'} flex flex-col gap-2 z-10`}>
        ...
        {routeInfo && isFullScreen && <motion.div initial={{
        opacity: 0,
        y: -20
      }} animate={{
        opacity: 1,
        y: 0
            ...
            {destinationName && <div className="text-xs text-blue-600 mt-1 font-medium truncate">
                to {destinationName}
              </div>}
          </motion.div>}
      </div>
      ...
      <div className={`absolute ${isFullScreen ? 'right-6 top-6' : 'right-3 top-3'} flex flex-col gap-2 z-10`}>
        <button onClick={toggleFullScreen} className="p-2 bg-white rounded-full shadow-md hover:bg-gray-50 transition-colors" title={isFullScreen ? "Exit Full Screen" : "Full Screen"}>
          {isFullScreen ? <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M8 3v3a2 2 0 0 1-2 2H3m18 0h-3a2 2 0 0 1-2-2V3m0 18v-3a2 2 0 0 1 2-2h3M3 16h3a2 2 0 0 1 2 2v3" /></svg> : <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M15 3h6v6M9 21H3v-6M21 3l-7 7M3 21l7-7" /></svg>}
        </button>
        <button onClick={handleRecenter} className="p-2 bg-white rounded-full shadow-md hover:bg-gray-50 transition-colors" title="Recenter">
      ...
      {routeError && <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-10">
      ...
      {isTracking && (!customerLocation || customerLocation.lat === 0) && <div className="absolute bottom-16 left-1/2 -translate-x-1/2 z-10 w-max max-w-[90%]">
      ...
      <GoogleMap mapContainerStyle={isFullScreen ? {
      width: '100%',
      height: '100%'
    } : mapContainerStyle} center={initialCenter} zoom={13} onLoad={onLoad} options={{
      zoomControl: false,
      ...
      // Limit max zoom when polyline is shown or during live tracking
      styles: [{
        featureType: "poi",
        elementType: "labels",
        stylers: [{
        ...
        {customerLocation && (customerLocation.lat !== 0 || customerLocation.lng !== 0) && <Marker position={customerLocation} icon={{
        ...
        {/* Seller Markers */}
        {allSellers.map((seller, index) => <Marker key={`seller-${index}`} position={seller} icon={showRoute && routeDestination?.lat === seller.lat ? {
        ...
        {animatedDeliveryLocation && <Marker position={animatedDeliveryLocation} icon={{
