...
export const calculateDeliveryBalances = state => {
  ...
  if (state.transactions && Array.isArray(state.transactions)) {
    const pendingFromTransactions = state.transactions.filter(t => t.type === 'withdrawal' && t.status === 'Pending').reduce((sum, t) => sum + (t.amount || 0), 0);
  ...
  if (state.transactions && Array.isArray(state.transactions)) {
    const earningsFromTransactions = state.transactions.filter(t => t.type === 'payment' && t.status === 'Completed') // Exclude bonus from earnings
