...
export default function ProfilePage() {
  ...
  return <div className="min-h-screen bg-gray-100 text-gray-900 font-poppins overflow-x-hidden">
    {/* Main Content */}
    ...
    <div ref={profileRef} className="mb-0">
      <div className="bg-white p-4 w-full shadow-sm">
        ...
        {/* Profile Information */}
        <div onClick={() => navigate("/delivery/profile/details")} className="flex items-start justify-between">
          ...
          <div className="relative shrink-0 ml-4">
            {profile?.profileImage?.url ? <img src={profile.profileImage.url} alt="Profile" className="w-20 h-20 md:w-24 md:h-24 rounded-full object-cover border-2 border-gray-200" onError={e => {
            ...
            }} /> : profile?.documents?.photo ? <img src={profile.documents.photo} alt="Profile" className="w-20 h-20 md:w-24 md:h-24 rounded-full object-cover border-2 border-gray-200" onError={e => {
              // Show default icon if image fails to load
              e.target.style.display = 'none';
              e.target.nextElementSibling?.classList.remove('hidden');
            }} /> : null}
            {!profile?.profileImage?.url && !profile?.documents?.photo && <div className="w-20 h-20 md:w-24 md:h-24 rounded-full bg-gray-300 flex items-center justify-center border-2 border-gray-200">
    ...
    <div className="px-4 py-6 pb-24 md:pb-6">
      ...
      <div ref={navButtonsRef} className="grid grid-cols-1 gap-3 mb-6">
        <button onClick={() => navigate("/delivery/trip-history")} className="bg-white rounded-lg p-4 flex flex-col items-center gap-1 hover:bg-gray-200 transition-colors">
      ...
      <div ref={sectionsRef} className="space-y-4">
        {/* Referral bonus */}
        <Card onClick={() => navigate("/delivery/refer-and-earn")} className="py-0 bg-white border-0 shadow-none cursor-pointer hover:bg-gray-200 transition-colors">
        ...
        <div>
          ...
          <div className="space-y-0">
            ...
            <Card onClick={() => navigate("/delivery/help/tickets")} className="bg-white py-0 border-0 shadow-none rounded-none first:rounded-t-lg last:rounded-b-lg cursor-pointer hover:bg-gray-200 transition-colors">
              <CardContent className="p-4 flex items-center justify-between">
        ...
        <div>
          ...
          <Card onClick={() => setShowAlertSoundPopup(true)} className="bg-white py-0 border-0 shadow-none rounded-lg cursor-pointer hover:bg-gray-200 transition-colors">
            <CardContent className="p-4 flex items-center justify-between">
              <div className="flex items-center gap-3">
...
>>>>>>> 6bbb4ca (Challenges flow implemented)
    ...
    {showAlertSoundPopup && <div className="fixed inset-0 bg-black/50 z-50 flex items-end justify-center">
      <div className="bg-white w-full max-w-md rounded-t-2xl shadow-2xl animate-in slide-in-from-bottom duration-300">
        ...
        <div className="flex items-center justify-between p-4 border-b border-gray-200">
          ...
          <button onClick={() => setShowAlertSoundPopup(false)} className="p-1 hover:bg-gray-100 rounded-full transition-colors">
        ...
        <div className="p-4">
          <div className="space-y-4">
            ...
            <label className="flex items-center justify-between p-3 cursor-pointer hover:bg-gray-50 rounded-lg transition-colors">
              ...
              <input type="radio" name="alertSound" value="original" checked={selectedAlertSound === 'original'} onChange={e => {
            ...
            <label className="flex items-center justify-between p-3 cursor-pointer hover:bg-gray-50 rounded-lg transition-colors">
              ...
              <input type="radio" name="alertSound" value="zomato_tone" checked={selectedAlertSound === 'zomato_tone'} onChange={e => {
                setSelectedAlertSound(e.target.value);
                localStorage.setItem('delivery_alert_sound', e.target.value);
                // Play preview sound
                try {
        ...
        <div className="p-4 border-t border-gray-200">
          <button onClick={() => setShowAlertSoundPopup(false)} className="w-full bg-[#DC2626] text-white py-3 rounded-lg font-semibold hover:bg-[#B91C1C] transition-colors">
            Ok
          </button>
        </div>
      </div>
