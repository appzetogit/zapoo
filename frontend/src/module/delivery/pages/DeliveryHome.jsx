...
import { fetchDeliveryWallet, calculatePeriodEarnings } from "../utils/deliveryWalletState";
...
const mockRestaurants = [{
  ...
  payment: "COD",
  amount: 76.62,
  // Payment amount (consistent with estimatedEarnings)
  items: 2,
  phone: "+911234567890",
...
export default function DeliveryHome() {
  ...
  const todayGig = bookedGigs.find(gig => gig.date === todayDateKey && gig.status === 'active') || bookedGigs.find(gig => gig.date === todayDateKey && gig.status === 'booked');
  ...
  const todayEarnings = hasStoreDataForToday && todayData ? todayData.earnings ?? calculatedEarnings : calculatedEarnings;
  ...
  const todayTrips = hasStoreDataForToday && todayData ? todayData.trips ?? calculatedTrips : calculatedTrips;
  ...
  useEffect(() => {
    ...
    if (calculatedEarnings !== undefined && calculatedTrips !== undefined && calculatedHours !== undefined && (lastUpdatedRef.current.earnings !== calculatedEarnings || lastUpdatedRef.current.trips !== calculatedTrips || lastUpdatedRef.current.hours !== calculatedHours)) {
  ...
  const playAlertSound = async () => {
    ...
    try {
      ...
      if (!soundFile) {
        console.error('❌ Sound file is undefined!', {
          selectedSound,
          soundFile
        });
        return null;
      ...
      try {
        ...
        if (!playError.message?.includes('user didn\'t interact') && !playError.name?.includes('NotAllowedError') && !playError.message?.includes('timeout')) {
    ...
    if (navigator.geolocation) {
      ...
      navigator.geolocation.getCurrentPosition(position => {
        ...
        if (latitude > 90 || latitude < -90 || longitude > 180 || longitude < -180) {
          console.error("❌ Coordinates out of valid range - possible swap:", {
            latitude,
            longitude
          });
        ...
        let heading = position.coords.heading !== null && position.coords.heading !== undefined ? position.coords.heading : null;
  ...
  useEffect(() => {
    ...
    const watchId = navigator.geolocation.watchPosition(position => {
        ...
        if (isOnlineRef.current && lastValidLocationRef.current) {
          ...
          if (timeSinceLastSend >= 30000) {
            ...
            if (lat >= -90 && lat <= 90 && lng >= -180 && lng <= 180) {
              deliveryAPI.updateLocation(lat, lng, true).then(() => {
                window.lastLocationSentTime = now;
              }).catch(error => {
                if (error.code !== 'ERR_NETWORK' && error.message !== 'Network Error') {
        ...
        if (!lastLocationRef.current) {
          ...
          if (window.deliveryMapInstance) {
            ...
            if (typeof lat === 'number' && typeof lng === 'number' && !isNaN(lat) && !isNaN(lng) && lat >= -90 && lat <= 90 && lng >= -180 && lng <= 180) {
              ...
              console.error('❌ Invalid coordinates for bike marker:', {
                lat,
                lng
              });
            }
      ...
      if (isOnlineRef.current && smoothedLocation) {
        ...
        const shouldSend = timeSinceLastSend >= 5000 || lastSentLocation && calculateDistance(lastSentLocation[0], lastSentLocation[1], smoothedLat, smoothedLng) > 0.05;
        if (shouldSend) {
          ...
          if (smoothedLat >= -90 && smoothedLat <= 90 && smoothedLng >= -180 && smoothedLng <= 180) {
            deliveryAPI.updateLocation(smoothedLat, smoothedLng, true).then(() => {
              window.lastLocationSentTime = now;
      ...
      if (response.data?.success && response.data.data) {
        ...
        if (order) {
          ...
          restaurantInfo = {
            ...
            restaurantMongoId: normalizeEntityId(order.restaurantId),
            customerUserId: normalizeEntityId(order.userId),
            name: restaurantName,
  ...
  const handlereachedPickupTouchEnd = e => {
    ...
    if (deltaX > threshold) {
      ...
      setTimeout(async () => {
        ...
        const deliveryPhase = selectedRestaurant?.deliveryPhase || selectedRestaurant?.deliveryState?.currentPhase || '';
        const deliveryStateStatus = selectedRestaurant?.deliveryState?.status || '';
        const isDelivered = orderStatus === 'delivered' || deliveryPhase === 'completed' || deliveryPhase === 'delivered' || deliveryStateStatus === 'delivered';
        if (isDelivered) {
        ...
        const isPastPickupPhase = orderStatus === 'out_for_delivery' || deliveryPhase === 'en_route_to_delivery' || deliveryPhase === 'picked_up' || deliveryStateStatus === 'order_confirmed' || deliveryStateStatus === 'reached_pickup' || deliveryPhase === 'at_pickup';
        ...
        if (orderId) {
          try {
            ...
            const errorMessage = error.response?.data?.message || (error.response?.status === 404 ? 'Order not found. Please refresh and try again.' : 'Failed to confirm reached pickup. Please try again.');
    ...
    try {
      ...
      if (window.flutter_inappwebview && typeof window.flutter_inappwebview.callHandler === 'function') {
        ...
        const result = await window.flutter_inappwebview.callHandler('openCamera', {
          source: 'camera',
          // 'camera' for camera, 'gallery' for file picker
          accept: 'image/*',
          multiple: false,
        ...
        if (result && result.success) {
          ...
          if (result.file) {
            ...
            try {
              ...
              file = new File([blob], result.fileName || `bill-image-${Date.now()}.jpg`, {
                type: mimeType
              });
            } catch (base64Error) {
      ...
      setTimeout(async () => {
        ...
        try {
          ...
          if (response.data?.success && response.data.data) {
            ...
            if (order && selectedRestaurant) {
              ...
              if (customerLat && customerLng) {
                const updatedRestaurant = {
                  ...selectedRestaurant,
                  restaurantMongoId: normalizeEntityId(order.restaurantId) || selectedRestaurant.restaurantMongoId || "",
                  customerUserId: normalizeEntityId(order.userId) || selectedRestaurant.customerUserId || "",
                  ...
                  phone: order.restaurantId?.phone || selectedRestaurant.phone || null,
                  customerLat,
                  customerLng
                };
            ...
            setShowOrderIdConfirmationPopup(false);
            toast.success('Order is out for delivery. Route to customer is on the map.', {
              duration: 4000
            });
          ...
          console.error('❌ Error confirming order ID:', {
            status,
            message: msg,
  ...
  useEffect(() => {
    if (newOrder) {
      ...
      if (!pickupDistance || pickupDistance === '0 km') {
        ...
        if (currentLocation && currentLocation.length === 2 && restaurantLat && restaurantLng && !isNaN(restaurantLat) && !isNaN(restaurantLng)) {
      ...
      const restaurantData = {
        ...
        restaurantMongoId: normalizeEntityId(newOrder.restaurantId || newOrder.restaurantMongoId),
        customerUserId: normalizeEntityId(newOrder.userId || newOrder.customerId),
  ...
  useEffect(() => {
    ...
    if (!selectedRestaurant?.address || selectedRestaurant.address === 'Restaurant address' || selectedRestaurant.address === 'Restaurant Address') {
  ...
  const emergencyOptions = [{
    ...
    phone: "108",
    // Indian emergency ambulance number
    icon: "ambulance"
  }, {
    id: "accident",
    ...
    try {
      ...
      if (response?.data?.success && response?.data?.data?.orders) {
        ...
        const pendingOrders = orders.filter(order => {
          ...
          if (order.deliveryState?.status === 'accepted' || order.deliveryState?.status === 'reached_pickup' || order.deliveryState?.status === 'order_confirmed' || deliveryPhase === 'en_route_to_pickup' || deliveryPhase === 'at_pickup' || deliveryPhase === 'en_route_to_delivery' || deliveryPhase === 'at_delivery') {
            return false;
          }
          return true;
        });
        if (pendingOrders.length > 0) {
          ...
          const restaurantData = {
            ...
            restaurantMongoId: normalizeEntityId(firstOrder.restaurantId),
            customerUserId: normalizeEntityId(firstOrder.userId),
            ...
            distance: pickupDistance,
            timeAway: pickupDistance !== 'Calculating...' ? calculateTimeAway(pickupDistance) : 'Calculating...',
            dropDistance: firstOrder.address?.location?.coordinates ? 'Calculating...' : '0 km',
            pickupDistance: pickupDistance,
    ...
    const checkBankDetails = async () => {
      try {
        ...
        if (response?.data?.success && response?.data?.data?.profile) {
          ...
          setDeliveryPartnerUserId(normalizeEntityId(profile));
          ...
          if (profile?.status && profile.status !== 'pending') {
            ...
            const isFilled = !!(bankDetails?.accountHolderName?.trim() && bankDetails?.accountNumber?.trim() && bankDetails?.ifscCode?.trim() && bankDetails?.bankName?.trim());
  ...
  const refreshSelectedRestaurantCallContext = useCallback(async () => {
    ...
    try {
      const response = await deliveryAPI.getOrderDetails(orderLookupId);
      ...
      const nextRestaurantInfo = {
        ...(selectedRestaurant || {}),
        id: order._id || selectedRestaurant?.id || orderLookupId,
        orderId: order.orderId || selectedRestaurant?.orderId || orderLookupId,
        restaurantMongoId: normalizeEntityId(order.restaurantId) || selectedRestaurant?.restaurantMongoId || "",
        customerUserId: normalizeEntityId(order.userId) || selectedRestaurant?.customerUserId || "",
        phone: order.restaurantId?.phone || selectedRestaurant?.phone || null,
        ownerPhone: order.restaurantId?.ownerPhone || selectedRestaurant?.ownerPhone || null,
        customerPhone: order.userId?.phone || selectedRestaurant?.customerPhone || null,
        customerName: order.userId?.name || order.userId?.fullName || selectedRestaurant?.customerName,
        customerAddress: order.address?.formattedAddress || selectedRestaurant?.customerAddress,
        deliveryInstructions: order.address?.deliveryInstructions || selectedRestaurant?.deliveryInstructions || "",
        customerLat: order.address?.location?.coordinates?.[1] ?? selectedRestaurant?.customerLat,
        customerLng: order.address?.location?.coordinates?.[0] ?? selectedRestaurant?.customerLng,
      };

      setSelectedRestaurant(nextRestaurantInfo);
      return nextRestaurantInfo;
    } catch (error) {
      console.error("Error refreshing order details for masked call:", error);
      return selectedRestaurant;
    }
  }, [newOrder?.orderId, newOrder?.orderMongoId, selectedRestaurant]);

  const placeDeliveryMaskedCall = useCallback(async ({ targetRole }) => {
    const isRestaurantCall = targetRole === "restaurant";
  ...
  }, [placeDeliveryMaskedCall]);

  const handleCallCustomerMasked = useCallback(() => {
    placeDeliveryMaskedCall({ targetRole: "customer" });
  }, [placeDeliveryMaskedCall]);
  ...
  useEffect(() => {
    ...
    async function initializeGoogleMap() {
      try {
        ...
        const mapTypeId = window.google?.maps?.MapTypeId?.ROADMAP !== undefined ? window.google.maps.MapTypeId.ROADMAP : 'roadmap';
        ...
        try {
          map = new window.google.maps.Map(mapContainerRef.current, {
            ...
            zoom: 18,
            minZoom: 10,
            // Minimum zoom level (city/area view)
            maxZoom: 21,
            // Maximum zoom level - allow full zoom
            mapTypeId: mapTypeId,
            tilt: 45,
            heading: 0,
        ...
        if (preservedState) {
          ...
          if (preservedState.bikeMarkerPosition && isOnlineRef.current) {
            createOrUpdateBikeMarker(preservedState.bikeMarkerPosition.lat, preservedState.bikeMarkerPosition.lng, preservedState.bikeMarkerHeading, false // Don't center when restoring from navigation
            );
          }
      ...
      const tryRoute = (travelMode, modeName) => {
        return new Promise((resolve, reject) => {
          directionsServiceRef.current.route({
            ...
            // Save API cost - don't get alternatives
            avoidHighways: false,
            avoidTolls: false,
  ...
  const updateLiveTrackingPolyline = useCallback((directionsResult, riderPosition) => {
    ...
    try {
      ...
      const path = [new window.google.maps.LatLng(riderPos.lat, riderPos.lng),
      // Start from bike position
      ...trimmedPolyline.map(point => new window.google.maps.LatLng(point.lat, point.lng))];
      ...
      if (liveTrackingPolylineRef.current) {
        ...
        liveTrackingPolylineRef.current = new window.google.maps.Polyline({
          ...
          geodesic: true,
          strokeColor: '#1E88E5',
          // Vibrant blue like Zomato (more visible than #4285F4)
          strokeOpacity: 1.0,
          strokeWeight: 6,
          // Optimal thickness for visibility
          zIndex: 1000,
          // High z-index to be above other map elements
          icons: [],
          // No icons/dots - clean solid line
          map: window.deliveryMapInstance
        });
        ...
        if (!liveTrackingPolylineShadowRef.current) {
          liveTrackingPolylineShadowRef.current = new window.google.maps.Polyline({
            ...
            strokeOpacity: 0.6,
            strokeWeight: 10,
            // Slightly thicker for shadow effect
            zIndex: 999,
            // Behind main polyline
            icons: [],
            map: window.deliveryMapInstance
          });
        } else {
  ...
  const animateRiderMarker = useCallback((newPosition, heading) => {
    ...
    const cancelAnimation = animateMarker(currentPosition, {
      lat: newLat,
      lng: newLng
    }, 500,
    ...
    const initializeDirectionsMap = async () => {
      ...
      try {
        ...
        if (routeResult) {
          ...
          const markerIcon = navigationMode === 'customer' ? `data:image/svg+xml;charset=UTF-8,${encodeURIComponent(`
  ...
  useEffect(() => {
    ...
    if (riderLocation && riderLocation.length === 2) {
      ...
      if (lastBikePositionRef.current) {
        const distance = calculateDistanceInMeters(lastBikePositionRef.current.lat, lastBikePositionRef.current.lng, newPosition.lat, newPosition.lng);
        ...
        if (distance > 50 && timeSinceLastRecalc > 30000 && selectedRestaurant) {
          ...
          calculateRouteWithDirectionsAPI([newPosition.lat, newPosition.lng], {
            lat: selectedRestaurant.lat,
            lng: selectedRestaurant.lng
          }).then(result => {
  ...
  useEffect(() => {
    ...
    if (!directionsRendererRef.current) {
      ...
      directionsRendererRef.current.setOptions({
        suppressMarkers: true,
        // Hide default markers including car icon
        suppressInfoWindows: false,
        polylineOptions: {
          strokeColor: '#4285F4',
          // Bright blue like Zomato
          strokeWeight: 0,
          // Completely hide DirectionsRenderer polyline (has dots)
          strokeOpacity: 0,
          // Hide completely
          zIndex: -1,
      ...
      try {
        ...
        const getRestoredRestaurantInfo = order => {
          ...
          return {
            ...
            restaurantMongoId: normalizeEntityId(order?.restaurantId) || savedRestaurantInfo.restaurantMongoId || "",
            customerUserId: normalizeEntityId(order?.userId) || savedRestaurantInfo.customerUserId || "",
            ...
            customerPhone: order?.userId?.phone || savedRestaurantInfo.customerPhone || null,
            ...
            phone: order?.restaurantId?.phone || savedRestaurantInfo.phone || null,
  ...
  useEffect(() => {
    ...
    const verifyOrderInterval = setInterval(async () => {
      try {
        ...
        const isOrderDelivered = order.status === 'delivered' || order.status === 'completed' || order.deliveryState?.currentPhase === 'completed' || order.deliveryState?.status === 'delivered';
  ...
  useEffect(() => {
    ...
    if ((orderReady.orderId || order?.orderId) && order && !selectedRestaurant?.orderId) {
      ...
      if (order.restaurantId?.address) {
        ...
        const addressParts = [restaurantLocation.street, restaurantLocation.area, restaurantLocation.city, restaurantLocation.state, restaurantLocation.zipCode || restaurantLocation.pincode || restaurantLocation.postalCode].filter(Boolean);
        restaurantAddress = addressParts.join(', ');
      } else if (restaurantLocation?.addressLine1) {
        const addressParts = [restaurantLocation.addressLine1, restaurantLocation.addressLine2, restaurantLocation.city, restaurantLocation.state].filter(Boolean);
        restaurantAddress = addressParts.join(', ');
      } else if (order.restaurantId?.addressLine1) {
        const addressParts = [order.restaurantId.addressLine1, order.restaurantId.addressLine2, order.restaurantId.area,
        // Zone
        order.restaurantId.city, order.restaurantId.state, order.restaurantId.pincode || order.restaurantId.zipCode || order.restaurantId.postalCode].filter(Boolean);
        restaurantAddress = addressParts.join(', ');
      } else if (order.restaurantId?.street || order.restaurantId?.city) {
        const addressParts = [order.restaurantId.street, order.restaurantId.area,
        // Zone
        order.restaurantId.city, order.restaurantId.state, order.restaurantId.pincode || order.restaurantId.zipCode || order.restaurantId.postalCode].filter(Boolean);
        restaurantAddress = addressParts.join(', ');
    ...
    const isDelivered = orderStatus === 'delivered' || deliveryPhase === 'completed' || deliveryPhase === 'delivered' || currentRestaurantInfo?.deliveryState?.status === 'delivered';
    if (isDelivered) {
      clearOrderReady();
      return;
    }
  ...
  useEffect(() => {
    ...
    if (selectedRestaurant?.address && selectedRestaurant.address !== 'Restaurant Address' && selectedRestaurant.address.length > 20) {
              ...
              try {
                ...
                if (restaurantResponse.data?.success && restaurantResponse.data.data) {
                  ...
                  if (restLocation?.formattedAddress) {
                    ...
                    const addressParts = [restLocation.street, restLocation.area, restLocation.city, restLocation.state, restLocation.zipCode || restLocation.pincode || restLocation.postalCode].filter(Boolean);
                    fetchedAddress = addressParts.join(', ');
                  } else if (restLocation?.addressLine1) {
                    const addressParts = [restLocation.addressLine1, restLocation.addressLine2, restLocation.city, restLocation.state].filter(Boolean);
                    fetchedAddress = addressParts.join(', ');
                  } else if (restaurant.street || restaurant.city) {
                    const addressParts = [restaurant.street, restaurant.area, restaurant.city, restaurant.state, restaurant.zipCode || restaurant.pincode || restaurant.postalCode].filter(Boolean);
                    fetchedAddress = addressParts.join(', ');
                  }
  ...
  useEffect(() => {
    ...
    const isDelivered = orderStatus === 'delivered' || deliveryPhase === 'completed' || deliveryPhase === 'delivered' || selectedRestaurant?.deliveryState?.status === 'delivered';
    if (isDelivered) {
    ...
    const isOrderIdConfirmed = deliveryPhase === 'en_route_to_delivery' || deliveryPhase === 'picked_up' || deliveryPhase === 'en_route_to_drop' || orderStatus === 'out_for_delivery' || selectedRestaurant?.deliveryState?.status === 'order_confirmed' || selectedRestaurant?.deliveryState?.currentPhase === 'en_route_to_delivery' || selectedRestaurant?.deliveryState?.currentPhase === 'en_route_to_drop';
  ...
  }, [riderLocation?.[0] ?? null, riderLocation?.[1] ?? null, selectedRestaurant?.lat ?? null, selectedRestaurant?.lng ?? null, selectedRestaurant?.deliveryPhase ?? selectedRestaurant?.deliveryState?.currentPhase ?? null, selectedRestaurant?.orderStatus ?? selectedRestaurant?.status ?? null, Boolean(showNewOrderPopup), Boolean(showOrderIdConfirmationPopup), Boolean(showreachedPickupPopup), Boolean(showReachedDropPopup), Boolean(showOrderDeliveredAnimation), Boolean(showCustomerReviewPopup), Boolean(showPaymentPage), selectedRestaurant?.orderStatus, selectedRestaurant?.status, selectedRestaurant?.deliveryPhase, selectedRestaurant?.deliveryState?.status, calculateDistanceInMeters]);
  ...
  useEffect(() => {
    ...
    const deliveryPhase = selectedRestaurant?.deliveryPhase || selectedRestaurant?.deliveryState?.currentPhase || '';
    const deliveryStateStatus = selectedRestaurant?.deliveryState?.status || '';
    const isDelivered = orderStatus === 'delivered' || orderStatus === 'completed' || deliveryPhase === 'completed' || deliveryPhase === 'delivered' || deliveryStateStatus === 'delivered' || showPaymentPage || showOrderDeliveredAnimation;
    if (isDelivered) {
  ...
  }, [selectedRestaurant?.orderStatus, selectedRestaurant?.status, selectedRestaurant?.deliveryPhase, selectedRestaurant?.deliveryState?.currentPhase, selectedRestaurant?.deliveryState?.status, showPaymentPage, showOrderDeliveredAnimation, showCustomerReviewPopup, showreachedPickupPopup, showOrderIdConfirmationPopup, showReachedDropPopup, clearNewOrder]);
  ...
  useEffect(() => {
    ...
    const isPickedUp = orderStatus === 'out_for_delivery' || orderStatus === 'picked_up' || deliveryPhase === 'en_route_to_delivery' || deliveryPhase === 'picked_up';
    ...
    if (isPickedUp && hasCustomerLocation && riderLocation && riderLocation.length === 2) {
      ...
      const needsCustomerRoute = !currentDirections || !currentDirections.routes || currentDirections.routes.length === 0;
  ...
  }, [selectedRestaurant?.orderStatus, selectedRestaurant?.status, selectedRestaurant?.deliveryPhase, selectedRestaurant?.deliveryState?.currentPhase, selectedRestaurant?.customerLat, selectedRestaurant?.customerLng, riderLocation, calculateRouteWithDirectionsAPI, updateLiveTrackingPolyline]);
  ...
  useEffect(() => {
    ...
    const hasCustomerCoords = selectedRestaurant?.customerLat != null && selectedRestaurant?.customerLng != null && !(selectedRestaurant.customerLat === 0 && selectedRestaurant.customerLng === 0);
    const orderId = selectedRestaurant?.orderId || selectedRestaurant?.id;
    ...
    deliveryAPI.getOrderDetails(orderId).then(res => {
  ...
  useEffect(() => {
    ...
    const isDeliveredOrCompleted = orderStatus === 'delivered' || orderStatus === 'completed' || deliveryPhase === 'completed' || deliveryPhase === 'at_delivery';
    ...
    const isOutForDelivery = !isDeliveredOrCompleted && (orderStatus === 'out_for_delivery' || deliveryPhase === 'en_route_to_delivery' || deliveryPhase === 'picked_up' || deliveryPhase === 'at_delivery' || deliveryStateStatus === 'order_confirmed' || deliveryStateStatus === 'en_route_to_delivery' || orderStatus === 'ready');
    ...
    const isInDeliveryPhase = isOutForDelivery || deliveryPhase === 'picked_up' || deliveryStateStatus === 'order_confirmed' || orderStatus === 'out_for_delivery';
  ...
  }, [riderLocation?.[0] ?? null, riderLocation?.[1] ?? null, selectedRestaurant?.customerLat ?? null, selectedRestaurant?.customerLng ?? null, selectedRestaurant?.orderStatus ?? newOrder?.status ?? null, selectedRestaurant?.deliveryPhase ?? selectedRestaurant?.deliveryState?.currentPhase ?? null, deliveryStateStatus,
  // Use memoized value to ensure consistent dependency array size
  Boolean(showNewOrderPopup), Boolean(showOrderIdConfirmationPopup), Boolean(showreachedPickupPopup), Boolean(showReachedDropPopup), Boolean(showOrderDeliveredAnimation), Boolean(showCustomerReviewPopup), Boolean(showPaymentPage), calculateDistanceInMeters]);
  ...
  const createOrUpdateBikeMarker = async (latitude, longitude, heading = null, shouldCenterMap = true) => {
    ...
    if (!bikeMarkerRef.current) {
      ...
      bikeMarkerRef.current = new window.google.maps.Marker({
        ...
        icon: bikeIcon,
        optimized: false,
        // Disable optimization for exact positioning
        animation: window.google.maps.Animation.DROP,
        // Drop animation on first appearance
  ...
  const handleSwipeBarTouchStart = e => {
    ...
    const isOnScrollableContent = target.closest('[ref="homeSectionsScrollRef"]') || target.closest('.overflow-y-auto') || homeSectionsScrollRef.current && homeSectionsScrollRef.current.contains(target);
  ...
  const drawZonesOnMap = zonesToDraw => {
    ...
    zonesToDraw.forEach((zone, index) => {
      ...
      const polygon = new window.google.maps.Polygon({
        ...
        fillOpacity: 0.3,
        // Light fill opacity for better visibility
        editable: false,
        draggable: false,
        clickable: true,
        zIndex: 1
  ...
  <div className="">
    ...
    <div className="flex gap-3 mb-6">
      <button
        ...
        disabled={isCallingRestaurant}
        className="flex-1 flex items-center justify-center gap-2 px-4 py-3 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
      >
        {isCallingRestaurant ? (
        ...
        <span className="text-gray-700 font-medium">
          {isCallingRestaurant ? 'Calling...' : 'Call'}
        </span>
      </button>
      <button onClick={() => {
