...
import { Wallet, Wifi, X, ArrowLeft } from "lucide-react";
import { Button } from "@/components/ui/button";
import { getDeliveryWalletState, calculateDeliveryBalances } from "../utils/deliveryWalletState";
...
export default function MyAccount() {
  ...
  return <div className="min-h-screen bg-[#f6e9dc] overflow-x-hidden pb-24">
      {/* Header */}
      <div className="bg-white border-b border-gray-200 px-4 py-4 md:py-6 flex items-center gap-4 rounded-b-3xl md:rounded-b-none">
        <button onClick={() => navigate("/delivery")} className="p-2 hover:bg-gray-100 rounded-lg transition-colors">
      ...
      <div className="px-4 py-6">
        ...
        <div className="bg-gray-800 rounded-xl p-4 md:p-6 mb-6 shadow-lg">
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4 mb-6">
            ...
            <div className="flex flex-col gap-2 w-full md:w-auto md:flex-shrink-0">
              <Button onClick={() => setShowConfirmDialog(true)} className="bg-[#ff8100] hover:bg-[#e67300] text-white font-semibold px-3 py-2 md:px-4 md:py-2 rounded-lg text-xs md:text-sm w-full md:w-auto">
                Adjust Payments
              </Button>
              <Button onClick={() => {
        ...
        <div className="mb-4">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => navigate("/delivery/transactions")} className="text-[#ff8100] text-sm md:text-base font-medium hover:underline">
              View All
            </button>
          </div>
      ...
      <AnimatePresence>
        {showConfirmDialog && <>
          ...
          duration: 0.2
        }} onClick={() => setShowConfirmDialog(false)} className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
              {/* Dialog Box */}
              <motion.div initial={{
            opacity: 0,
            scale: 0.9,
            ...
            stiffness: 300,
            damping: 30
          }} onClick={e => e.stopPropagation()} className="bg-white rounded-xl shadow-2xl max-w-md w-full p-6 relative">
                {/* Close Button */}
                <button onClick={() => setShowConfirmDialog(false)} className="absolute top-4 right-4 p-2 hover:bg-gray-100 rounded-full transition-colors">
                ...
                <div className="mt-2">
                  ...
                  <div className="flex gap-3">
                    <Button onClick={() => setShowConfirmDialog(false)} variant="outline" className="flex-1 border-gray-300 text-gray-700 hover:bg-gray-50">
                      Cancel
                    </Button>
                    <Button onClick={() => {
                  // Handle OK action
