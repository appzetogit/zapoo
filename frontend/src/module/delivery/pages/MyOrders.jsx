...
import { ArrowLeft, Search, Mic, MoreVertical, ChevronRight, Star, RotateCcw, AlertCircle, Loader2, Package } from "lucide-react";
...
export default function MyOrders() {
    ...
    const fetchOrders = async () => {
      try {
        ...
        const response = await deliveryAPI.getTripHistory({
          ...
          status: 'ALL TRIPS',
          // Get all trips including completed
  ...
  useEffect(() => {}, [filteredOrders, searchQuery]);
  return <div className="min-h-screen bg-gray-50 pb-10 font-sans">
      ...
      <div className="bg-white p-4 flex items-center shadow-sm sticky top-0 z-10">
        ...
        {!loading && <div className="ml-auto text-xs text-gray-500 flex flex-col items-end">
      ...
      <div className="px-4 py-2">
        {loading ? <div className="flex flex-col items-center justify-center py-12">
            ...
            </div> : filteredOrders.length === 0 ? <div className="flex flex-col items-center justify-center py-12">
            <Package className="w-16 h-16 text-gray-300 mb-4" />
            <h3 className="text-lg font-semibold text-gray-900 mb-2">
              {searchQuery ? "No orders found" : "You haven't placed any orders yet"}
            </h3>
            <p className="text-gray-600 text-sm text-center mb-6">
              {searchQuery ? "Try searching with different keywords" : "Start accepting orders to see them here"}
            </p>
          ...
          return <div key={orderKey} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <img src={restaurantImage} alt={restaurantName} className="w-full h-full object-cover" onError={e => {
                    e.target.src = "https://images.unsplash.com/photo-1555949258-eb67b1ef0ceb?auto=format&fit=crop&w=100&q=80";
                      ...
                      <div className="flex-1">
                        ...
                        <button onClick={() => navigate(`/restaurant/${order.restaurantId?.slug || order.restaurantId?._id || order.restaurantId}`)} className="text-xs text-red-500 font-medium flex items-center mt-1 hover:text-red-600">
                  ...
                  {/* Items List */}
                  <div className="px-4 py-2">
                    {order.items && order.items.map((item, idx) => <div key={item._id || item.itemId || idx} className="flex items-center gap-2 mt-1">
                        {/* Veg/Non-Veg Icon */}
                  ...
                  <div className="px-4 py-2 flex items-center justify-between">
                    <div>
                      ...
                      {!paymentFailed && isDelivered && <p className="text-xs font-medium text-gray-500 mt-1">{orderStatus}</p>}
        </div>
