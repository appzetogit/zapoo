...
export default function Conversation() {
  ...
  return <div className="min-h-screen bg-[#f6e9dc] overflow-x-hidden">
      ...
      <div className="bg-white border-b border-gray-200 px-4 py-4 md:py-3 flex items-center gap-4 rounded-b-3xl md:rounded-b-none">
        <button onClick={() => navigate("/delivery/profile")} className="p-2 hover:bg-gray-100 rounded-lg transition-colors">
      ...
      <div className="px-4 py-6 pb-24 md:pb-6">
        ...
        <div className="mb-4">
          <div className="relative">
            ...
            <input type="text" placeholder="Search conversations..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#ff8100] focus:border-transparent outline-none bg-white" />
          </div>
        </div>

        {/* Conversations List */}
        <div className="space-y-2">
          {filteredConversations.map((conversation, index) => <motion.div key={conversation.id} initial={{
          opacity: 0,
          y: 20
        }} animate={{
          opacity: 1,
          ...
          delay: index * 0.1
              ...
              <Card className="bg-white shadow-sm border border-gray-100 hover:shadow-md transition-shadow cursor-pointer" onClick={() => {}}>
                <CardContent className="px-2 md:px-4 py-1.5 md:py-3 gap-0">
                  <div className="flex items-center gap-3">
                    <div className="relative flex-shrink-0">
                      ...
                      {conversation.unread > 0 && <span className="absolute -top-1 -right-1 bg-red-500 text-white text-[10px] font-bold rounded-full w-5 h-5 flex items-center justify-center">
                          {conversation.unread}
                        </span>}
                    </div>
        ...
        {filteredConversations.length === 0 && <div className="text-center py-12">
