        ...
        <div className="px-6 py-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-4">
              ...
              {order.estimatedDeliveryTime && <div className="space-y-1">
                  <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider flex items-center gap-2">
              ...
              {order.deliveredAt && <div className="space-y-1">
                  <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider flex items-center gap-2">
                    <Clock className="w-4 h-4" />
                    Delivered At
                  </p>
            ...
            <div className="space-y-4">
              {order.orderStatus && <div className="space-y-1">
              ...
              {(order.paymentStatus || order.paymentCollectionStatus != null) && <div className="space-y-1">
                  <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider flex items-center gap-2">
                    <CreditCard className="w-4 h-4" />
                    Payment Status
                  </p>
                  <p className={`text-sm font-medium ${getPaymentStatusColor(order.paymentType === 'Cash on Delivery' || order.payment?.method === 'cash' || order.payment?.method === 'cod' ? order.paymentCollectionStatus ?? (order.status === 'delivered' ? 'Collected' : 'Not Collected') : order.paymentStatus)}`}>
                    {order.paymentType === 'Cash on Delivery' || order.payment?.method === 'cash' || order.payment?.method === 'cod' ? order.paymentCollectionStatus ?? (order.status === 'delivered' ? 'Collected' : 'Not Collected') : order.paymentStatus}
                  </p>
                </div>}
              {order.deliveryType && <div className="space-y-1">
                  <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider flex items-center gap-2">
          ...
          <div className="border-t border-slate-200 pt-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {order.customerPhone && <div className="space-y-1">
                  <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider flex items-center gap-2">
              ...
              {order.customerEmail && <div className="space-y-1">
                  <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider flex items-center gap-2">
          ...
          {order.restaurant && <div className="border-t border-slate-200 pt-4">
          ...
          {/* Order Items */}
          {order.items && Array.isArray(order.items) && order.items.length > 0 && <div className="border-t border-slate-200 pt-4">
              <h3 className="text-sm font-semibold text-slate-700 mb-4 flex items-center gap-2">
                <Package className="w-4 h-4" />
                Order Items ({order.items.length})
              </h3>
              <div className="space-y-3">
                {order.items.map((item, index) => <div key={index} className="flex items-start justify-between p-3 bg-slate-50 rounded-lg">
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        ...
                        {item.isVeg !== undefined && <span className={`text-xs px-1.5 py-0.5 rounded ${item.isVeg ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
          ...
          {(order.billImageUrl || order.billImage || order.deliveryState?.billImageUrl) && <div className="border-t border-slate-200 pt-4">
              <h3 className="text-sm font-semibold text-slate-700 mb-4 flex items-center gap-2">
              ...
              <div className="space-y-3">
                <div className="relative w-full max-w-2xl border-2 border-slate-300 rounded-xl overflow-hidden bg-white shadow-sm">
                  <img src={order.billImageUrl || order.billImage || order.deliveryState?.billImageUrl} alt="Order Bill" className="w-full h-auto object-contain max-h-[500px] mx-auto block" loading="lazy" onError={e => {
                ...
                <div className="flex items-center gap-3">
                  <a href={order.billImageUrl || order.billImage || order.deliveryState?.billImageUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-lg transition-colors shadow-sm">
                    <Eye className="w-4 h-4" />
                    View Full Size
                  </a>
                  <a href={order.billImageUrl || order.billImage || order.deliveryState?.billImageUrl} download className="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-slate-700 bg-slate-100 hover:bg-slate-200 rounded-lg transition-colors">
                    <Package className="w-4 h-4" />
                    Download
                  </a>
                </div>
          ...
          {/* Delivery Address */}
          {order.address && <div className="border-t border-slate-200 pt-4">
              <h3 className="text-sm font-semibold text-slate-700 mb-4 flex items-center gap-2">
              ...
              <div className="space-y-2 p-4 bg-slate-50 rounded-lg">
                ...
                {getCoordinates(order.address) && <p className="text-xs text-slate-500 mt-2">
                    <span className="font-medium">Coordinates:</span> {getCoordinates(order.address)}
                  </p>}
                {order.address.label && <p className="text-xs text-slate-500">
          ...
          {(order.deliveryPartnerName || order.deliveryPartnerPhone) && <div className="border-t border-slate-200 pt-4">
              <h3 className="text-sm font-semibold text-slate-700 mb-4 flex items-center gap-2">
                <Truck className="w-4 h-4" />
                Delivery Partner
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {order.deliveryPartnerName && <div className="space-y-1">
                ...
                {order.deliveryPartnerPhone && <div className="space-y-1">
          ...
          <div className="border-t border-slate-200 pt-4">
            ...
            <div className="space-y-2">
              {order.totalItemAmount !== undefined && <div className="flex justify-between text-sm">
                  <span className="text-slate-600">Subtotal</span>
                  <span className="font-medium text-slate-900">₹{order.totalItemAmount.toFixed(2)}</span>
                </div>}
              {order.itemDiscount !== undefined && order.itemDiscount > 0 && <div className="flex justify-between text-sm">
                  <span className="text-slate-600">Discount</span>
                  <span className="font-medium text-emerald-600">-₹{order.itemDiscount.toFixed(2)}</span>
                </div>}
              {order.couponDiscount !== undefined && order.couponDiscount > 0 && <div className="flex justify-between text-sm">
              ...
              {order.deliveryCharge !== undefined && <div className="flex justify-between text-sm">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="font-medium text-slate-900">
                  {order.platformFee !== undefined && order.platformFee > 0 ? `₹${order.platformFee.toFixed(2)}` : <span className="text-slate-400">₹0.00</span>}
                </span>
              </div>
              {order.vatTax !== undefined && order.vatTax > 0 && <div className="flex justify-between text-sm">
