...
export const exportTransactionReportToCSV = (transactions, filename = "transaction_report") => {
  const headers = ["SI", "Order ID", "Restaurant", "Customer Name", "Total Item Amount", "Item Discount", "Admin Coupon Discount", "Restaurant Coupon Discount", "Discounted Amount", "GST", "Delivery Charge", "Order Amount"]
  const rows = transactions.map((transaction, index) => [
    index + 1,
    transaction.orderId,
    transaction.restaurant,
    transaction.customerName,
    (transaction.totalItemAmount || 0).toFixed(2),
    (transaction.itemDiscount || 0).toFixed(2),
    (transaction.adminCouponDiscount || 0).toFixed(2),
    (transaction.restaurantCouponDiscount || 0).toFixed(2),
    (transaction.discountedAmount || 0).toFixed(2),
    (transaction.gst ?? transaction.vatTax ?? 0).toFixed(2),
    (transaction.deliveryCharge || 0).toFixed(2),
    (transaction.orderAmount || 0).toFixed(2)
  ])
        ...
        <tbody>
          ${transactions.map((transaction, index) => `
            <tr>
              ...
              <td>$${(transaction.totalItemAmount || 0).toFixed(2)}</td>
              <td>$${(transaction.itemDiscount || 0).toFixed(2)}</td>
              <td>$${(transaction.adminCouponDiscount || 0).toFixed(2)}</td>
              <td>$${(transaction.restaurantCouponDiscount || 0).toFixed(2)}</td>
              <td>$${(transaction.discountedAmount || 0).toFixed(2)}</td>
              <td>$${(transaction.gst ?? transaction.vatTax ?? 0).toFixed(2)}</td>
              <td>$${(transaction.deliveryCharge || 0).toFixed(2)}</td>
              <td>$${(transaction.orderAmount || 0).toFixed(2)}</td>
            </tr>
          `).join("")}
        </tbody>
      </table>
