...
export default function EarningAddonHistory() {
  ...
  const filteredHistory = useMemo(() => {
    ...
    return history.filter(item => item.deliveryman?.toLowerCase().includes(query) || item.deliveryId?.toLowerCase().includes(query) || item.offerTitle?.toLowerCase().includes(query));
  }, [history, searchQuery]);
  ...
  const getStatusBadge = status => {
    ...
    return <span className={`px-3 py-1 rounded-full text-xs font-medium ${config.bg} ${config.text} flex items-center gap-1`}>
  ...
  const handleCheckAllCompletions = async () => {
    try {
      ...
      const partnersResponse = await adminAPI.getDeliveryPartners({
        limit: 1000
      });
  ...
  return <div className="p-4 lg:p-6 bg-slate-50 min-h-screen">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
            ...
            <div className="flex items-center gap-3">
              <button onClick={handleCheckAllCompletions} disabled={isCheckingCompletions} className="px-4 py-2.5 text-sm font-medium rounded-lg bg-[#FF5200] text-white hover:bg-[#E64A00] disabled:bg-orange-300 disabled:cursor-not-allowed flex items-center gap-2 transition-all" title="Check all delivery partners for completed offers">
              ...
              <div className="relative flex-1 sm:flex-initial min-w-[250px]">
                <input type="text" placeholder="Ex: search delivery man or offer" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-10 pr-4 py-2.5 w-full text-sm rounded-lg border border-slate-300 bg-white focus:outline-none focus:ring-2 focus:ring-slate-400 focus:border-slate-400" />
              ...
              <table className="w-full">
                <thead className="bg-slate-50 border-b border-slate-200">
                  <tr>
                    ...
                    {visibleColumns.deliveryman && <th className="px-6 py-4 text-left text-[10px] font-bold text-slate-700 uppercase tracking-wider">
                        <div className="flex items-center gap-2">
                          <span>Deliveryman</span>
                          <ArrowUpDown className="w-3 h-3 text-slate-400 cursor-pointer hover:text-slate-600" />
                        </div>
                      </th>}
                    {visibleColumns.offerTitle && <th className="px-6 py-4 text-left text-[10px] font-bold text-slate-700 uppercase tracking-wider">
                        <div className="flex items-center gap-2">
                          <span>Offer Title</span>
                        </div>
                      </th>}
                    {visibleColumns.ordersCompleted && <th className="px-6 py-4 text-left text-[10px] font-bold text-slate-700 uppercase tracking-wider">
                        <div className="flex items-center gap-2">
                          <span>Orders</span>
                        </div>
                      </th>}
                    ...
                    {visibleColumns.date && <th className="px-6 py-4 text-left text-[10px] font-bold text-slate-700 uppercase tracking-wider">
                        <div className="flex items-center gap-2">
                          <span>Date</span>
                          <ArrowUpDown className="w-3 h-3 text-slate-400 cursor-pointer hover:text-slate-600" />
                        </div>
                ...
                <tbody className="bg-white divide-y divide-slate-100">
                  {filteredHistory.length === 0 ? <tr>
                      <td colSpan={Object.values(visibleColumns).filter(v => v).length} className="px-6 py-12 text-center">
                    ...
                    </tr> : filteredHistory.map(item => <tr key={item._id} className="hover:bg-slate-50 transition-colors">
                        ...
                        {visibleColumns.deliveryman && <td className="px-6 py-4 whitespace-nowrap">
                            <div className="flex flex-col">
                              ...
                              {item.deliveryPhone && item.deliveryPhone !== 'N/A' && <span className="text-xs text-slate-400 mt-0.5">{item.deliveryPhone}</span>}
                            </div>
                          </td>}
                        {visibleColumns.offerTitle && <td className="px-6 py-4">
                            <span className="text-sm text-slate-700">{item.offerTitle}</span>
                          </td>}
                        {visibleColumns.ordersCompleted && <td className="px-6 py-4 whitespace-nowrap">
                            <div className="flex flex-col">
                              <span className="text-sm font-medium text-slate-900">
                                {item.ordersCompleted || 0} / {item.ordersRequired || 0}
                              </span>
                              {item.ordersRequired > 0 && <span className="text-xs text-slate-500 mt-0.5">
                        ...
                        {visibleColumns.date && <td className="px-6 py-4 whitespace-nowrap">
                            <div className="flex flex-col">
                              <span className="text-sm text-slate-700">{formatDate(item.date || item.completedAt)}</span>
                              {item.completedAt && <span className="text-xs text-slate-400 mt-0.5">
                        ...
                        {visibleColumns.actions && <td className="px-6 py-4 whitespace-nowrap">
                            <div className="flex items-center gap-2">
                              {item.status === 'pending' && <button onClick={() => handleOpenCreditDialog(item)} className="px-3 py-1.5 text-xs font-medium rounded-lg bg-emerald-500 text-white hover:bg-emerald-600 transition-all" title="Credit to Wallet">
                                  Credit
                                </button>}
                              {item.status === 'pending' && <button onClick={() => handleCancel(item._id)} className="p-2 rounded-lg hover:bg-slate-100 transition-colors" title="Cancel">
      ...
      <Dialog open={isCreditDialogOpen} onOpenChange={setIsCreditDialogOpen}>
        <DialogContent className="max-w-lg bg-gradient-to-br from-white via-slate-50 to-white p-0 border-0 shadow-2xl">
          ...
          {selectedHistory && <div className="px-6 py-6 space-y-6">
              {/* Information Cards */}
              <div className="space-y-3">
                ...
                <div className="flex items-start gap-3 p-4 bg-slate-50 rounded-xl border border-slate-200">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {selectedHistory.ordersCompleted && selectedHistory.ordersRequired && <p className="text-xs text-slate-500 mt-0.5">
              ...
              <div className="space-y-2">
                ...
                <textarea value={creditNotes} onChange={e => setCreditNotes(e.target.value)} className="w-full px-4 py-3 border-2 border-slate-200 rounded-xl bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all resize-none text-sm text-slate-700 placeholder:text-slate-400" rows={4} placeholder="Add any notes about this credit transaction..." />
              ...
              setCreditNotes("");
                ...
                <button onClick={handleCredit} className="px-5 py-2.5 text-sm font-semibold rounded-xl bg-gradient-to-r from-emerald-500 to-emerald-600 text-white hover:from-emerald-600 hover:to-emerald-700 shadow-lg shadow-emerald-500/30 transition-all flex items-center gap-2">
        ...
        <DialogContent className="max-w-md bg-white">
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-200">
              <button onClick={resetColumns} className="px-4 py-2 text-sm font-medium rounded-lg border border-slate-300 bg-white text-slate-700 hover:bg-slate-50">
                Reset
              </button>
              <button onClick={() => setIsSettingsOpen(false)} className="px-4 py-2 text-sm font-medium rounded-lg bg-emerald-500 text-white hover:bg-emerald-600">
                Apply
              </button>
            </div>
          </div>
