...
const formatBonusAmount = transaction => {
  ...
  let cleaned = transaction.bonus.toString().replace(/¹/g, '') // Remove superscript 1
...
export default function DeliverymanBonus() {
  ...
  const handleSubmit = async e => {
    ...
    try {
      const response = await adminAPI.addDeliveryPartnerBonus(formData.deliveryPartnerId, formData.amount, '' // No reference
      );
      if (response?.data?.success || response?.data?.data) {
        // Refresh transactions
        const transactionsResponse = await adminAPI.getDeliveryPartnerBonusTransactions({
          limit: 1000
        });
  ...
  const handleExport = format => {
    ...
    switch (format) {
      case "excel":
        exportBonusToExcel(filteredTransactions);
        break;
      case "pdf":
        exportBonusToPDF(filteredTransactions);
        break;
      default:
        break;
    }
  ...
  return <div className="p-4 lg:p-6 bg-slate-50 min-h-screen">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 mb-6 relative">
          <button onClick={() => setIsSettingsOpen(true)} className="absolute top-6 right-6 p-2 rounded-lg bg-slate-100 hover:bg-slate-200 transition-colors">
          ...
          <form onSubmit={handleSubmit}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <input type="number" step="0.01" value={formData.amount} onChange={e => handleInputChange("amount", e.target.value)} placeholder="Enter amount" className={`w-full px-4 py-2.5 border rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-[#FF5200] text-sm ${formErrors.amount ? "border-red-500" : "border-slate-300"}`} disabled={submitting} />
            ...
            <div className="flex items-center justify-end gap-4 mt-6">
              <button type="button" onClick={handleReset} className="px-6 py-2.5 text-sm font-medium rounded-lg border border-slate-300 bg-white text-slate-700 hover:bg-slate-50 transition-all disabled:opacity-50 disabled:cursor-not-allowed" disabled={submitting}>
                Reset
              </button>
              <button type="submit" className="px-6 py-2.5 text-sm font-medium rounded-lg bg-[#FF5200] text-white hover:bg-[#E64A00] transition-all shadow-md disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2" disabled={submitting}>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
          ...
          {loading ? <div className="flex flex-col items-center justify-center py-20">
            ...
            </div> : filteredTransactions.length === 0 ? <div className="flex flex-col items-center justify-center py-20">
      ...
      <Dialog open={showSuccessDialog} onOpenChange={setShowSuccessDialog}>
        <DialogContent className="max-w-md bg-white p-0 opacity-0 data-[state=open]:opacity-100 data-[state=closed]:opacity-0 transition-opacity duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=open]:fade-in-0 data-[state=closed]:fade-out-0 data-[state=open]:scale-100 data-[state=closed]:scale-100">
          ...
          <div className="px-6 pb-6 flex items-center justify-end">
            <button onClick={() => setShowSuccessDialog(false)} className="px-4 py-2 text-sm font-medium rounded-lg bg-emerald-500 text-white hover:bg-emerald-600 transition-all shadow-md">
              OK
            </button>
          </div>
        </DialogContent>
