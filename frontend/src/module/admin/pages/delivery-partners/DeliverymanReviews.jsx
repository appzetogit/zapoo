  ...
  return <div className="p-4 lg:p-6 bg-slate-50 min-h-screen">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
            ...
            <div className="flex items-center gap-3">
              <div className="relative flex-1 sm:flex-initial min-w-[250px]">
                <input type="text" placeholder="Ex : search delivery man" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-10 pr-4 py-2.5 w-full text-sm rounded-lg border border-slate-300 bg-white focus:outline-none focus:ring-2 focus:ring-slate-400 focus:border-slate-400" />
          ...
          <div className="overflow-x-auto">
            {isLoading ? <div className="flex items-center justify-center py-12">
              ...
              </div> : filteredReviews.length === 0 ? <div className="text-center py-12">
                ...
                <thead className="bg-slate-50 border-b border-slate-200">
                  <tr>
                    ...
                    {visibleColumns.orderId && <th className="px-6 py-4 text-left text-[10px] font-bold text-slate-700 uppercase tracking-wider">
                        <div className="flex items-center gap-2">
                          <span>Order ID</span>
                          <ArrowUpDown className="w-3 h-3 text-slate-400 cursor-pointer hover:text-slate-600" />
                        </div>
                    ...
                    {visibleColumns.deliverymanId && <th className="px-6 py-4 text-left text-[10px] font-bold text-slate-700 uppercase tracking-wider">
                        <div className="flex items-center gap-2">
                          <span>Delivery Boy ID</span>
                          <ArrowUpDown className="w-3 h-3 text-slate-400 cursor-pointer hover:text-slate-600" />
                        </div>
                      </th>}
                    {visibleColumns.customer && <th className="px-6 py-4 text-left text-[10px] font-bold text-slate-700 uppercase tracking-wider">
                        <div className="flex items-center gap-2">
                          <span>Customer</span>
                          <ArrowUpDown className="w-3 h-3 text-slate-400 cursor-pointer hover:text-slate-600" />
                        </div>
                      </th>}
                    {visibleColumns.review && <th className="px-6 py-4 text-left text-[10px] font-bold text-slate-700 uppercase tracking-wider">
                        <div className="flex items-center gap-2">
                          <span>Review</span>
                          <ArrowUpDown className="w-3 h-3 text-slate-400 cursor-pointer hover:text-slate-600" />
                        </div>
                      </th>}
                    {visibleColumns.rating && <th className="px-6 py-4 text-left text-[10px] font-bold text-slate-700 uppercase tracking-wider">
                        <div className="flex items-center gap-2">
                          <span>Rating</span>
                          <ArrowUpDown className="w-3 h-3 text-slate-400 cursor-pointer hover:text-slate-600" />
                        </div>
                ...
                <tbody className="bg-white divide-y divide-slate-100">
                  {filteredReviews.map(review => <tr key={review.sl || review.orderId} className="hover:bg-slate-50 transition-colors">
                      ...
                      {visibleColumns.deliverymanId && <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      {visibleColumns.review && <td className="px-6 py-4">
                          <div className="flex items-center gap-2">
                            <span className="text-sm text-slate-700 flex-1 truncate max-w-xs">
                              {review.review || 'No review text'}
                            </span>
                            {review.review && review.review.trim() && <button onClick={() => {
                      setSelectedReview(review);
                      setIsReviewModalOpen(true);
                    }} className="p-1.5 rounded-lg hover:bg-slate-100 transition-colors text-slate-600 hover:text-slate-900 flex-shrink-0" title="View full review">
                                <Eye className="w-4 h-4" />
                              </button>}
                          </div>
                        </td>}
                      {visibleColumns.rating && <td className="px-6 py-4 whitespace-nowrap">
      ...
      <Dialog open={isReviewModalOpen} onOpenChange={setIsReviewModalOpen}>
        <DialogContent className="max-w-2xl bg-white p-0">
          ...
          {selectedReview && <div className="px-6 py-6 space-y-6">
              {/* Order & Delivery Info */}
          ...
          <DialogFooter className="px-6 pb-6 pt-4 border-t border-slate-200">
            <button onClick={() => setIsReviewModalOpen(false)} className="px-4 py-2 text-sm font-medium rounded-lg bg-slate-100 text-slate-700 hover:bg-slate-200 transition-all">
              Close
            </button>
          </DialogFooter>
        </DialogContent>
