...
export default function RestaurantsList() {
      ...
      try {
        ...
        if (response.data && response.data.success && response.data.data) {
          ...
          const mappedRestaurants = restaurantsData.map((restaurant, index) => ({
            ...
            _id: restaurant._id,
            // Preserve original _id for API calls
  ...
  const filteredRestaurants = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      result = result.filter(restaurant => restaurant.name.toLowerCase().includes(query) || restaurant.ownerName.toLowerCase().includes(query) || restaurant.ownerPhone.includes(query));
    }
    if (filters.all !== "All") {
    ...
    if (filters.cuisine) {
      result = result.filter(restaurant => restaurant.cuisine.toLowerCase().includes(filters.cuisine.toLowerCase()));
    }
    if (filters.zone) {
  ...
  const handleToggleStatus = async id => {
    try {
      ...
      const updatedRestaurants = restaurants.map(restaurant => restaurant.id === id ? {
        ...restaurant,
  ...
  const uniqueCuisines = useMemo(() => {
    const cuisines = restaurants.map(r => r.cuisine).filter(c => c && c !== "N/A").filter((value, index, self) => self.indexOf(value) === index).sort();
    return cuisines;
  }, [restaurants]);
  ...
  const confirmBanRestaurant = async () => {
    ...
    try {
      ...
      try {
        ...
        setRestaurants(prevRestaurants => prevRestaurants.map(r => r.id === restaurant.id || r._id === restaurant._id ? {
          ...r,
          status: newStatus
        } : r));
  ...
  const handleExtendSubscription = async () => {
    ...
    try {
      setExtending(true);
      const restaurantId = restaurantDetails._id || restaurantDetails.id || restaurantDetails.restaurantId;
      const response = await adminAPI.extendRestaurantSubscription(restaurantId, extensionDays);
  ...
  return <div className="p-4 lg:p-6 bg-slate-50 min-h-screen">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
            ...
            <div className="flex items-center gap-3">
              <button className="px-4 py-2.5 text-sm font-medium rounded-lg bg-[#FF5200] hover:bg-[#E64A00] text-white flex items-center gap-2 transition-all">
                <Plus className="w-4 h-4" />
                <span>Add Restaurant</span>
              </button>
              <div className="relative flex-1 sm:flex-initial min-w-[250px]">
                <input type="text" placeholder="Ex: search by Restaurant n" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-10 pr-4 py-2.5 w-full text-sm rounded-lg border border-slate-300 bg-white focus:outline-none focus:ring-2 focus:ring-[#FF5200] focus:border-[#FF5200]" />
          ...
          <div className="overflow-x-auto">
            {loading ? <div className="flex items-center justify-center py-20">
              ...
              </div> : error ? <div className="flex flex-col items-center justify-center py-20">
                ...
                <tbody className="bg-white divide-y divide-slate-100">
                  {filteredRestaurants.length === 0 ? <tr>
                      <td colSpan={7} className="px-6 py-20 text-center">
                    ...
                    </tr> : filteredRestaurants.map((restaurant, index) => <tr key={restaurant.id} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-full overflow-hidden bg-slate-100 flex items-center justify-center flex-shrink-0">
                              <img src={restaurant.logo} alt={restaurant.name} className="w-full h-full object-cover" onError={e => {
                        e.target.src = "https://via.placeholder.com/40";
                      }} />
                            </div>
                            <div className="flex flex-col min-w-0">
                              <span className="text-sm font-medium text-slate-900 truncate max-w-[150px]" title={restaurant.name}>{restaurant.name}</span>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex flex-col min-w-0">
                            <span className="text-sm font-medium text-slate-900 truncate max-w-[150px]" title={restaurant.ownerName}>{restaurant.ownerName}</span>
                        ...
                        <td className="px-4 py-3 whitespace-nowrap">
                          <button onClick={() => handleToggleStatus(restaurant.id)} className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-[#FF5200] focus:ring-offset-2 ${restaurant.status ? "bg-[#FF5200]" : "bg-slate-300"}`}>
                            <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${restaurant.status ? "translate-x-6" : "translate-x-1"}`} />
                          </button>
                        </td>
                        <td className="px-4 py-3 whitespace-nowrap text-center">
                          <div className="flex items-center justify-center gap-2">
                            <button onClick={() => handleViewDetails(restaurant)} className="p-1.5 rounded text-[#FF5200] hover:bg-orange-50 transition-colors" title="View Details">
                              <Eye className="w-4 h-4" />
                            </button>
                            <button onClick={() => handleBanRestaurant(restaurant)} className={`p-1.5 rounded transition-colors ${!restaurant.status ? "text-green-600 hover:bg-green-50" : "text-red-600 hover:bg-red-50"}`} title={!restaurant.status ? "Unban Restaurant" : "Ban Restaurant"}>
                              <ShieldX className="w-4 h-4" />
                            </button>
                            <button onClick={() => handleDeleteRestaurant(restaurant)} className="p-1.5 rounded text-red-600 hover:bg-red-50 transition-colors" title="Delete Restaurant">
      ...
      {selectedRestaurant && <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4" onClick={closeDetailsModal}>
            <div className="bg-white rounded-xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
              {/* Modal Header */}
              <div className="sticky top-0 bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between">
                <h2 className="text-2xl font-bold text-slate-900">Restaurant Details</h2>
                <button onClick={closeDetailsModal} className="p-2 rounded-lg hover:bg-slate-100 transition-colors">
              ...
              {/* Modal Content */}
              <div className="p-6">
                {loadingDetails && <div className="flex items-center justify-center py-20">
                    <Loader2 className="w-8 h-8 animate-spin text-[#FF5200]" />
                    <span className="ml-3 text-slate-600">Loading details...</span>
                  </div>}
                {!loadingDetails && (restaurantDetails || selectedRestaurant) && <div className="space-y-6">
                      {/* Restaurant Basic Info */}
                      <div className="flex items-start gap-6 pb-6 border-b border-slate-200">
                        <div className="w-24 h-24 rounded-lg overflow-hidden bg-slate-100 flex-shrink-0">
                          <img src={restaurantDetails?.profileImage?.url || restaurantDetails?.logo || selectedRestaurant?.logo || selectedRestaurant?.originalData?.profileImage?.url || "https://via.placeholder.com/96"} alt={restaurantDetails?.name || selectedRestaurant?.name || "Restaurant"} className="w-full h-full object-cover" onError={e => {
                  e.target.src = "https://via.placeholder.com/96";
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex items-center gap-4 flex-wrap">
                            {(restaurantDetails?.ratings?.average || selectedRestaurant?.originalData?.ratings?.average) && <div className="flex items-center gap-1">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                          ...
                          <div className="space-y-3">
                            {restaurantDetails?.location && <div className="flex items-start gap-3">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          ...
                          <div className="space-y-3">
                            <div>
                              ...
                              <div className="flex flex-wrap gap-2">
                                {restaurantDetails?.cuisines && Array.isArray(restaurantDetails.cuisines) && restaurantDetails.cuisines.length > 0 ? restaurantDetails.cuisines.map((cuisine, idx) => <span key={idx} className="px-3 py-1 bg-orange-100 text-[#FF5200] rounded-full text-sm font-medium">
                                      {cuisine}
                                    </span>) : <span className="px-3 py-1 bg-orange-100 text-[#FF5200] rounded-full text-sm font-medium">
                        ...
                        <div>
                          ...
                          <div className="space-y-3">
                            {restaurantDetails?.deliveryTimings && <div className="flex items-center gap-3">
                            ...
                            {restaurantDetails?.estimatedDeliveryTime && <div>
                      ...
                      <div className="pt-6 border-t border-slate-200">
                        <h4 className="text-lg font-semibold text-slate-900 mb-4 flex items-center justify-between">
                          ...
                          {!showExtendInput && <button onClick={() => setShowExtendInput(true)} className="text-sm bg-orange-600 text-white px-3 py-1.5 rounded-lg hover:bg-orange-700 transition-colors">
                              Extend Subscription
                            </button>}
                        </h4>

                        {showExtendInput && <div className="bg-orange-50 p-4 rounded-lg mb-4 border border-orange-100">
                            <h5 className="font-medium text-orange-900 mb-2">Extend Subscription Period</h5>
                            <div className="flex gap-3 items-end">
                              <div>
                                <label className="block text-xs text-orange-700 mb-1">Days to Extend</label>
                                <input type="number" value={extensionDays} onChange={e => setExtensionDays(parseInt(e.target.value) || 0)} className="w-40 px-3 py-2 border border-orange-200 rounded-md focus:outline-none focus:ring-2 focus:ring-orange-500" min="1" />
                              </div>
                              <button onClick={handleExtendSubscription} disabled={extending || extensionDays <= 0} className="px-4 py-2 bg-orange-600 text-white rounded-md hover:bg-orange-700 disabled:opacity-50 disabled:cursor-not-allowed text-sm">
                                {extending ? "Extending..." : "Confirm Extension"}
                              </button>
                              <button onClick={() => setShowExtendInput(false)} className="px-4 py-2 bg-white text-slate-600 border border-slate-300 rounded-md hover:bg-slate-50 text-sm">
                                Cancel
                              </button>
                            </div>
                          </div>}
                          ...
                          {restaurantDetails?.subscription ? <>
                              ...
                              <div>
                                ...
                                <p className="font-medium text-slate-900">
                                  {restaurantDetails.subscription.endDate ? new Date(restaurantDetails.subscription.endDate).toLocaleDateString('en-IN', {
                        day: 'numeric',
                        month: 'short',
                        year: 'numeric'
                      }) : 'N/A'}
                              ...
                              <div>
                                ...
                                <p className="font-medium text-slate-900">
                                  {restaurantDetails.subscription.startDate ? new Date(restaurantDetails.subscription.startDate).toLocaleDateString('en-IN', {
                        day: 'numeric',
                        month: 'short',
                        year: 'numeric'
                      }) : 'N/A'}
                      ...
                      {(restaurantDetails?.createdAt || restaurantDetails?.updatedAt) && <div className="pt-6 border-t border-slate-200">
                          <h4 className="text-lg font-semibold text-slate-900 mb-4">Registration Information</h4>
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                            {restaurantDetails.createdAt && <div className="flex items-center gap-3">
                        ...
                        minute: '2-digit'
                            ...
                            {restaurantDetails.updatedAt && <div className="flex items-center gap-3">
                                <Calendar className="w-5 h-5 text-slate-400" />
                                <div>
                        ...
                        minute: '2-digit'
                            ...
                            {restaurantDetails.restaurantId && <div>
                            ...
                            {restaurantDetails.signupMethod && <div>
                      ...
                      {restaurantDetails?.onboarding?.step3 && <div className="pt-6 border-t border-slate-200">
                          <h4 className="text-lg font-semibold text-slate-900 mb-4">Registration Documents</h4>
                          <div className="space-y-6">
                            {/* PAN Details */}
                            {restaurantDetails.onboarding.step3.pan && <div className="bg-slate-50 rounded-lg p-4">
                                <h5 className="font-semibold text-slate-900 mb-3 flex items-center gap-2">
                                  <FileText className="w-4 h-4" />
                                  PAN Details
                                </h5>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                                  {restaurantDetails.onboarding.step3.pan.panNumber && <div>
                                  ...
                                  {restaurantDetails.onboarding.step3.pan.nameOnPan && <div>
                                  ...
                                  {restaurantDetails.onboarding.step3.pan.image?.url && <div className="md:col-span-2">
                                      <p className="text-xs text-slate-500 mb-2">PAN Document</p>
                                      <a href={restaurantDetails.onboarding.step3.pan.image.url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-[#FF5200] hover:text-[#E64A00]">
                            ...
                            {/* GST Details */}
                            {restaurantDetails.onboarding.step3.gst && <div className="bg-slate-50 rounded-lg p-4">
                                <h5 className="font-semibold text-slate-900 mb-3 flex items-center gap-2">
                                  <FileText className="w-4 h-4" />
                                  GST Details
                                </h5>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                                  ...
                                  {restaurantDetails.onboarding.step3.gst.gstNumber && <div>
                                  ...
                                  {restaurantDetails.onboarding.step3.gst.legalName && <div>
                                  ...
                                  {restaurantDetails.onboarding.step3.gst.address && <div>
                                  ...
                                  {restaurantDetails.onboarding.step3.gst.image?.url && <div className="md:col-span-2">
                                      <p className="text-xs text-slate-500 mb-2">GST Document</p>
                                      <a href={restaurantDetails.onboarding.step3.gst.image.url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-[#FF5200] hover:text-[#E64A00]">
                                        <ImageIcon className="w-4 h-4" />
                            ...
                            {/* FSSAI Details */}
                            {restaurantDetails.onboarding.step3.fssai && <div className="bg-slate-50 rounded-lg p-4">
                                <h5 className="font-semibold text-slate-900 mb-3 flex items-center gap-2">
                                  <FileText className="w-4 h-4" />
                                  FSSAI Details
                                </h5>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                                  {restaurantDetails.onboarding.step3.fssai.registrationNumber && <div>
                                  ...
                                  {restaurantDetails.onboarding.step3.fssai.expiryDate && <div>
                          ...
                          day: 'numeric'
                                  ...
                                  {restaurantDetails.onboarding.step3.fssai.image?.url && <div className="md:col-span-2">
                                      <p className="text-xs text-slate-500 mb-2">FSSAI Document</p>
                                      <a href={restaurantDetails.onboarding.step3.fssai.image.url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-[#FF5200] hover:text-[#E64A00]">
                                        <ImageIcon className="w-4 h-4" />
                            ...
                            {/* Bank Details */}
                            {restaurantDetails.onboarding.step3.bank && <div className="bg-slate-50 rounded-lg p-4">
                                <h5 className="font-semibold text-slate-900 mb-3 flex items-center gap-2">
                                  <CreditCard className="w-4 h-4" />
                                  Bank Details
                                </h5>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                                  {restaurantDetails.onboarding.step3.bank.accountNumber && <div>
                                      <p className="text-xs text-slate-500 mb-1">Account Number</p>
                                      <p className="font-medium text-slate-900">
                                        {restaurantDetails.onboarding.step3.bank.accountNumber}
                                      </p>
                                    </div>}
                                  {restaurantDetails.onboarding.step3.bank.ifscCode && <div>
                                  ...
                                  {restaurantDetails.onboarding.step3.bank.accountHolderName && <div>
                                  ...
                                  {restaurantDetails.onboarding.step3.bank.accountType && <div>
                      ...
                      {restaurantDetails?.onboarding?.step1 && <div className="pt-6 border-t border-slate-200">
                          <h4 className="text-lg font-semibold text-slate-900 mb-4">Registration Step 1 Details</h4>
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                            {restaurantDetails.onboarding.step1.restaurantName && <div>
                            ...
                            {restaurantDetails.onboarding.step1.ownerName && <div>
                            ...
                            {restaurantDetails.onboarding.step1.ownerEmail && <div>
                            ...
                            {restaurantDetails.onboarding.step1.ownerPhone && <div>
                            ...
                            {restaurantDetails.onboarding.step1.primaryContactNumber && <div>
                            ...
                            {restaurantDetails.onboarding.step1.location && <div className="md:col-span-2">
                      ...
                      {restaurantDetails?.onboarding?.step2 && <div className="pt-6 border-t border-slate-200">
                          <h4 className="text-lg font-semibold text-slate-900 mb-4">Registration Step 2 Details</h4>
                          <div className="space-y-4">
                            {restaurantDetails.onboarding.step2.cuisines && Array.isArray(restaurantDetails.onboarding.step2.cuisines) && restaurantDetails.onboarding.step2.cuisines.length > 0 && <div>
                                <p className="text-xs text-slate-500 mb-2">Cuisines (at registration)</p>
                                <div className="flex flex-wrap gap-2">
                                  {restaurantDetails.onboarding.step2.cuisines.map((cuisine, idx) => <span key={idx} className="px-3 py-1 bg-orange-100 text-orange-700 rounded-full text-sm font-medium">
                                      {cuisine}
                                    </span>)}
                                </div>
                              </div>}
                            {restaurantDetails.onboarding.step2.deliveryTimings && <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                                <div>
                            ...
                            {restaurantDetails.onboarding.step2.openDays && Array.isArray(restaurantDetails.onboarding.step2.openDays) && restaurantDetails.onboarding.step2.openDays.length > 0 && <div>
                                <p className="text-xs text-slate-500 mb-2">Open Days (at registration)</p>
                                <div className="flex flex-wrap gap-2">
                                  {restaurantDetails.onboarding.step2.openDays.map((day, idx) => <span key={idx} className="px-3 py-1 bg-orange-100 text-orange-700 rounded-full text-sm font-medium capitalize">
                                      {day}
                                    </span>)}
                                </div>
                              </div>}
                            {restaurantDetails.onboarding.step2.profileImageUrl?.url && <div>
                                <p className="text-xs text-slate-500 mb-2">Profile Image (at registration)</p>
                                <a href={restaurantDetails.onboarding.step2.profileImageUrl.url} target="_blank" rel="noopener noreferrer" className="inline-block">
                                  <img src={restaurantDetails.onboarding.step2.profileImageUrl.url} alt="Profile" className="w-32 h-32 rounded-lg object-cover border border-slate-200 hover:border-[#FF5200] transition-colors" onError={e => {
                      ...
                      {restaurantDetails?.onboarding?.step4 && <div className="pt-6 border-t border-slate-200">
                          <h4 className="text-lg font-semibold text-slate-900 mb-4">Registration Step 4 Details</h4>
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                            {restaurantDetails.onboarding.step4.estimatedDeliveryTime && <div>
                            ...
                            {restaurantDetails.onboarding.step4.distance && <div>
                      ...
                      {(restaurantDetails?.slug || restaurantDetails?.restaurantId || restaurantDetails?.phoneVerified !== undefined || restaurantDetails?.signupMethod) && <div className="pt-6 border-t border-slate-200">
                ...
                {!loadingDetails && !restaurantDetails && !selectedRestaurant && <div className="flex flex-col items-center justify-center py-20">
      ...
      {banConfirmDialog && <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4" onClick={cancelBanRestaurant}>
            <div className="bg-white rounded-xl shadow-2xl max-w-md w-full" onClick={e => e.stopPropagation()}>
              <div className="p-6">
                ...
                <p className="text-sm text-slate-700 mb-6">
                  {banConfirmDialog.action === 'ban' ? 'Are you sure you want to ban this restaurant? They will not be able to receive orders or access their account.' : 'Are you sure you want to unban this restaurant? They will be able to receive orders and access their account again.'}
                </p>

                <div className="flex items-center gap-3">
                  <button onClick={cancelBanRestaurant} disabled={banning} className="flex-1 px-4 py-2.5 text-sm font-medium rounded-lg border border-slate-300 bg-white hover:bg-slate-50 text-slate-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                    Cancel
                  </button>
                  <button onClick={confirmBanRestaurant} disabled={banning} className={`flex-1 px-4 py-2.5 text-sm font-medium rounded-lg text-white transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${banConfirmDialog.action === 'ban' ? 'bg-red-600 hover:bg-red-700' : 'bg-green-600 hover:bg-green-700'}`}>
                    {banning ? <span className="flex items-center justify-center gap-2">
                        <Loader2 className="w-4 h-4 animate-spin" />
                        {banConfirmDialog.action === 'ban' ? 'Banning...' : 'Unbanning...'}
                      </span> : banConfirmDialog.action === 'ban' ? 'Ban Restaurant' : 'Unban Restaurant'}
                  </button>
                </div>
              </div>
            </div>
      ...
      {deleteConfirmDialog && <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4" onClick={cancelDeleteRestaurant}>
            <div className="bg-white rounded-xl shadow-2xl max-w-md w-full" onClick={e => e.stopPropagation()}>
              <div className="p-6">
                <div className="flex items-center gap-4 mb-4">
                ...
                <div className="flex items-center gap-3">
                  <button onClick={cancelDeleteRestaurant} disabled={deleting} className="flex-1 px-4 py-2.5 text-sm font-medium rounded-lg border border-slate-300 bg-white hover:bg-slate-50 text-slate-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                    Cancel
                  </button>
                  <button onClick={confirmDeleteRestaurant} disabled={deleting} className="flex-1 px-4 py-2.5 text-sm font-medium rounded-lg bg-red-600 hover:bg-red-700 text-white transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                    {deleting ? <span className="flex items-center justify-center gap-2">
                        <Loader2 className="w-4 h-4 animate-spin" />
                        Deleting...
                      </span> : "Delete Restaurant"}
                  </button>
                </div>
              </div>
            </div>
