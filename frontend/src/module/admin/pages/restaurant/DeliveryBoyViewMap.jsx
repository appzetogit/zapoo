    ...
    try {
      ...
      if (response.data?.success && response.data.data?.deliveryPartners) {
        ...
        const onlineBoys = response.data.data.deliveryPartners.filter(boy => {
          ...
          const hasLocation = coordinates && Array.isArray(coordinates) && coordinates.length >= 2 && coordinates[0] !== 0 && coordinates[1] !== 0;
          if (isOnline && hasLocation) {} else {}
  ...
  const drawDeliveryBoyMarkers = async (google, map) => {
    ...
    for (const boy of deliveryBoys) {
      ...
      if (!lat || !lng || isNaN(lat) || isNaN(lng) || lat === 0 || lng === 0) {
        console.warn("⚠️ Invalid lat/lng values:", {
          lat,
          lng
        });
        continue;
      ...
      if (lat < -90 || lat > 90 || lng < -180 || lng > 180) {
        console.warn("⚠️ Coordinates out of range:", {
          lat,
          lng
        });
        continue;
      ...
      <div className="p-4 lg:p-6">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-4">
          ...
          minHeight: "600px"
            ...
            {loading && !mapLoading && <div className="absolute inset-0 flex items-center justify-center bg-slate-100 rounded-lg">
                <div className="text-center">
                  <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600 mx-auto mb-4"></div>
            ...
            {!loading && !mapLoading && zones.length === 0 && deliveryBoys.length === 0 && <div className="absolute inset-0 flex items-center justify-center bg-slate-100 rounded-lg">
                <div className="text-center p-6">
                  <MapPin className="w-12 h-12 text-slate-400 mx-auto mb-4" />
          ...
          {/* Legend */}
          {!mapLoading && <div className="mt-4 p-4 bg-slate-50 rounded-lg border border-slate-200">
              ...
              <div className="text-xs text-slate-600 space-y-1">
                ...
                {deliveryBoys.length === 0 && <p className="text-amber-600">
