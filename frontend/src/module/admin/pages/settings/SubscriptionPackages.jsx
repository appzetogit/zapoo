...
export default function SubscriptionPackages() {
  ...
  const filtered = useMemo(() => {
    return packages.filter(pkg => pkg.name.toLowerCase().includes(searchQuery.toLowerCase().trim()));
      ...
      <div className="bg-white rounded-lg shadow-sm border border-slate-200 mb-4">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between px-4 py-3 border-b border-slate-100 gap-3">
          ...
          <div className="flex items-center gap-2 text-xs">
            ...
            return <button key={label} type="button" onClick={() => setActiveFilter(label)} className={`px-3 py-1 rounded-full font-semibold border text-xs transition-all ${active ? "bg-blue-600 text-white border-blue-600" : "bg-slate-50 text-slate-700 border-slate-200 hover:bg-slate-100"}`}>
                  {label}
                </button>;
          })}
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 px-4 py-4">
          <OverviewCard title="Basic" amount="$ 399.00" previous="$ 399.00" bgColor="bg-sky-50" />
          <OverviewCard title="Standard" amount="$ 1,598.00" previous="$ 1,598.00" bgColor="bg-amber-50" />
          <OverviewCard title="Pro" amount="$ 3,597.00" previous="$ 3,597.00" bgColor="bg-sky-50" />
        </div>
      </div>
        ...
        <div className="px-4 py-3 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
          <div className="flex-1 md:flex-none">
            <div className="relative w-full md:w-72">
              <input type="text" placeholder="Search by name" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="w-full pl-8 pr-3 py-2 text-xs border border-slate-300 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" />
          ...
          <table className="min-w-full divide-y divide-slate-100">
            <thead className="bg-slate-50 border-b border-slate-200">
              <tr>
                ...
                {visibleColumns.name && <th className="px-6 py-4 text-left text-[10px] font-bold text-slate-700 uppercase tracking-wider">
                    <div className="flex items-center gap-2">
                      <span>Package Name</span>
                      <ArrowUpDown className="w-3 h-3 text-slate-400 cursor-pointer hover:text-slate-600" />
                    </div>
                  </th>}
                {visibleColumns.pricing && <th className="px-6 py-4 text-left text-[10px] font-bold text-slate-700 uppercase tracking-wider">
                    <div className="flex items-center gap-2">
                      <span>Pricing</span>
                      <ArrowUpDown className="w-3 h-3 text-slate-400 cursor-pointer hover:text-slate-600" />
                    </div>
                  </th>}
                {visibleColumns.duration && <th className="px-6 py-4 text-left text-[10px] font-bold text-slate-700 uppercase tracking-wider">
                    <div className="flex items-center gap-2">
                      <span>Duration</span>
                      <ArrowUpDown className="w-3 h-3 text-slate-400 cursor-pointer hover:text-slate-600" />
                    </div>
                  </th>}
                {visibleColumns.subscribers && <th className="px-6 py-4 text-left text-[10px] font-bold text-slate-700 uppercase tracking-wider">
                    <div className="flex items-center gap-2">
            ...
            <tbody className="bg-white divide-y divide-slate-100">
              {filtered.length === 0 ? <tr>
                  <td colSpan={Object.values(visibleColumns).filter(v => v).length} className="px-6 py-8 text-center text-slate-500">
                ...
                </tr> : filtered.map((pkg, index) => <tr key={pkg.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    {visibleColumns.pricing && <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    {visibleColumns.subscribers && <td className="px-6 py-4 whitespace-nowrap">
...
function OverviewCard({
  ...
  return <div className={`${bgColor} rounded-xl px-6 py-5 flex flex-col items-center justify-between`}>
