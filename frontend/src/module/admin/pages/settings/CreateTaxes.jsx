...
export default function CreateTaxes() {
  ...
  const filtered = useMemo(() => {
    return taxes.filter(tax => tax.name.toLowerCase().includes(search.toLowerCase().trim()));
  }, [taxes, search]);
  ...
  const toggleStatus = id => {
    setTaxes(prev => prev.map(t => t.id === id ? {
      ...t,
      active: !t.active
    } : t));
  };
      ...
      <div className="mb-4 flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-3">
          <div className="relative flex-1 sm:flex-initial min-w-[250px]">
            <input type="text" placeholder="Ex: Tax" value={search} onChange={e => setSearch(e.target.value)} className="w-full pl-10 pr-4 py-2.5 text-sm rounded-lg border border-slate-300 bg-white focus:outline-none focus:ring-2 focus:ring-slate-400 focus:border-slate-400" />
          ...
          <table className="min-w-full divide-y divide-slate-100">
            <thead className="bg-slate-50 border-b border-slate-200">
              <tr>
                ...
                {visibleColumns.rate && <th className="px-6 py-4 text-left text-[10px] font-bold text-slate-700 uppercase tracking-wider">
                    <div className="flex items-center gap-2">
                      <span>Tax Rate</span>
                      <ArrowUpDown className="w-3 h-3 text-slate-400 cursor-pointer hover:text-slate-600" />
                    </div>
            ...
            <tbody className="bg-white divide-y divide-slate-100">
              {filtered.length === 0 ? <tr>
                  <td colSpan={Object.values(visibleColumns).filter(v => v).length} className="px-6 py-8 text-center text-slate-500">
                ...
                </tr> : filtered.map((tax, index) => <tr key={tax.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    {visibleColumns.rate && <td className="px-6 py-4 whitespace-nowrap">
                        <span className="text-sm text-slate-700">{tax.rate}</span>
                      </td>}
                    {visibleColumns.status && <td className="px-6 py-4 whitespace-nowrap">
                        <ToggleSwitch enabled={tax.active} onToggle={() => toggleStatus(tax.id)} />
                      </td>}
