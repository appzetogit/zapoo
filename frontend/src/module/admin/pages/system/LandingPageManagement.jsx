...
export default function LandingPageManagement() {
  ...
  const setErrorSafely = errorMessage => {
    ...
    if (lowerMessage.includes('token') || lowerMessage.includes('unauthorized') || lowerMessage.includes('no token') || lowerMessage.includes('authentication') || lowerMessage.includes('session expired')) {
      setError(null);
    } else {
  ...
  const handleBannerOrderChange = async (id, direction) => {
    ...
    try {
      setError(null);
      await api.patch(`/hero-banners/${id}/order`, {
        order: newOrder
      }, getAuthConfig());
      if (otherBanner) {
        await api.patch(`/hero-banners/${otherBanner._id}/order`, {
          order: banner.order
        }, getAuthConfig());
      }
      await fetchBanners();
  ...
  const handleLinkRestaurants = async () => {
    ...
    try {
      ...
      setError(null);
      setSuccess(null);
      const response = await api.patch(`/hero-banners/${selectedBannerId}/link-restaurants`, {
  ...
  const handlePendingCategoryLabelChange = (id, newLabel) => {
    setPendingCategories(prev => prev.map(item => item.id === id ? {
      ...item,
      label: newLabel
    } : item));
  };
  ...
  const handleCategoryOrderChange = async (id, direction) => {
    ...
    try {
      setError(null);
      await api.patch(`/hero-banners/landing/categories/${id}/order`, {
        order: newOrder
      }, getAuthConfig());
      if (otherCategory) {
        await api.patch(`/hero-banners/landing/categories/${otherCategory._id}/order`, {
  ...
  const handleExploreMoreOrderChange = async (id, direction) => {
    ...
    try {
      setError(null);
      await api.patch(`/hero-banners/landing/explore-more/${id}/order`, {
        order: newOrder
      }, getAuthConfig());
      if (otherItem) {
        await api.patch(`/hero-banners/landing/explore-more/${otherItem._id}/order`, {
          order: item.order
        }, getAuthConfig());
      }
      await fetchExploreMore();
  ...
  const handleUnder250BannerOrderChange = async (id, direction) => {
    ...
    try {
      setError(null);
      await api.patch(`/hero-banners/under-250/${id}/order`, {
        order: newOrder
      }, getAuthConfig());
      if (otherBanner) {
        await api.patch(`/hero-banners/under-250/${otherBanner._id}/order`, {
          order: banner.order
        }, getAuthConfig());
      }
      await fetchUnder250Banners();
  ...
  const handleGourmetOrderChange = async (id, direction) => {
    ...
    try {
      setError(null);
      await api.patch(`/hero-banners/gourmet/${id}/order`, {
        order: newOrder
      }, getAuthConfig());
      if (otherRestaurant) {
        await api.patch(`/hero-banners/gourmet/${otherRestaurant._id}/order`, {
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-2 mb-6">
          <div className="flex gap-2 overflow-x-auto">
            {tabs.map(tab => {
            const Icon = tab.icon;
            return <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex items-center gap-2 px-4 py-2 rounded-lg font-medium transition-colors whitespace-nowrap ${activeTab === tab.id ? 'bg-orange-500 text-white' : 'text-slate-600 hover:bg-slate-100'}`}>
        ...
        {success && <div className="mb-6 bg-green-50 border border-green-200 text-green-800 px-4 py-3 rounded-lg flex items-center gap-2">
            <CheckCircle2 className="w-5 h-5" />
            <span>{success}</span>
          </div>}

        {error && <div className="mb-6 bg-red-50 border border-red-200 text-red-800 px-4 py-3 rounded-lg flex items-center gap-2">
        ...
        {activeTab === 'banners' && <>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input type="text" value={bannerTitle} onChange={e => setBannerTitle(e.target.value)} placeholder="e.g. HOME FOR SALE" className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-400" />
                  </div>
                  <div>
                    ...
                    <input type="text" value={bannerSubtitle} onChange={e => setBannerSubtitle(e.target.value)} placeholder="e.g. Modern" className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-400" />
                  </div>
                  <div>
                    ...
                    <textarea value={bannerDescription} onChange={e => setBannerDescription(e.target.value)} placeholder="Short description shown under the title..." rows={3} className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-400 resize-none" />
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-sm font-semibold text-slate-700 mb-1">CTA Button Text</label>
                      <input type="text" value={bannerCtaText} onChange={e => setBannerCtaText(e.target.value)} placeholder="Order Now" className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-400" />
                    </div>
                    <div>
                      <label className="block text-sm font-semibold text-slate-700 mb-1">CTA Link</label>
                      <input type="text" value={bannerCtaLink} onChange={e => setBannerCtaLink(e.target.value)} placeholder="/user" className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-400" />
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <label htmlFor="single-banner-input" className="border-2 border-dashed border-orange-300 rounded-lg p-6 text-center bg-orange-50/30 cursor-pointer hover:border-orange-400 transition-colors flex-1 flex flex-col items-center justify-center gap-2">
                    <input id="single-banner-input" type="file" accept="image/*" onChange={handleSingleBannerFileChange} className="hidden" />
                    {bannerPreview ? <img src={bannerPreview} alt="Preview" className="w-full max-h-40 object-contain rounded-lg" /> : <>
                  ...
                  <button type="button" onClick={handleSingleBannerUpload} disabled={singleBannerUploading || !bannerFile} className="w-full py-2.5 bg-orange-500 hover:bg-orange-600 disabled:opacity-50 text-white font-semibold rounded-lg text-sm transition-colors flex items-center justify-center gap-2">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
              ...
              {bannersLoading ? <div className="flex items-center justify-center py-12">
                  <Loader2 className="w-8 h-8 text-orange-600 animate-spin" />
                </div> : banners.length === 0 ? <div className="text-center py-12 text-slate-500">
                  <ImageIcon className="w-12 h-12 mx-auto mb-3 text-slate-400" />
                  <p>No banners uploaded yet.</p>
                </div> : <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {banners.map((banner, index) => <div key={banner._id} className="border border-slate-200 rounded-lg overflow-hidden hover:shadow-md transition-shadow">
                      ...
                      setShowRestaurantModal(true);
                        ...
                        {banner.linkedRestaurants && banner.linkedRestaurants.length > 0 && <div className="mt-2 pt-2 border-t border-slate-200">
                            <p className="text-xs text-slate-600 mb-1">Linked Restaurants ({banner.linkedRestaurants.length}):</p>
                            <div className="flex flex-wrap gap-1">
                              {banner.linkedRestaurants.slice(0, 3).map(restaurant => <span key={restaurant._id || restaurant} className="px-2 py-0.5 bg-orange-50 text-orange-700 rounded text-xs">
                                  {restaurant.name || 'Restaurant'}
                                </span>)}
                              {banner.linkedRestaurants.length > 3 && <span className="px-2 py-0.5 bg-slate-100 text-slate-600 rounded text-xs">
        ...
        {activeTab === 'under-250' && <>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 mb-6">
              ...
              <div className="border-2 border-dashed border-orange-300 rounded-lg p-8 text-center bg-orange-50/30 cursor-pointer transition-colors hover:border-orange-400 hover:bg-orange-50/50" onDragOver={e => {
            e.preventDefault();
            e.stopPropagation();
          }} onDragLeave={e => {
            e.preventDefault();
            ...
            if (files.length > 0) handleUnder250BannerFileSelect({
              files
            });
          }} onClick={() => under250BannersFileInputRef.current?.click()}>
                <input ref={under250BannersFileInputRef} type="file" accept="image/*" multiple onChange={handleUnder250BannerFileSelect} className="hidden" disabled={under250BannersUploading} />
                {under250BannersUploading ? <div className="flex flex-col items-center gap-3">
                    <Loader2 className="w-8 h-8 text-orange-600 animate-spin" />
                    <p className="text-orange-600 font-medium">
                      Uploading image {under250BannersUploadProgress.current} of {under250BannersUploadProgress.total}...
                    </p>
                    {under250BannersUploadProgress.total > 0 && <div className="w-full max-w-xs">
                        <div className="w-full bg-orange-200 rounded-full h-2">
                          <div className="bg-orange-600 h-2 rounded-full transition-all duration-300" style={{
                  ...
                  e.stopPropagation();
                  under250BannersFileInputRef.current?.click();
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
              ...
              {under250BannersLoading ? <div className="flex items-center justify-center py-12">
                  <Loader2 className="w-8 h-8 text-orange-600 animate-spin" />
                </div> : under250Banners.length === 0 ? <div className="text-center py-12 text-slate-500">
                  <Tag className="w-12 h-12 mx-auto mb-3 text-slate-400" />
                  <p>No under 250 banners uploaded yet.</p>
                </div> : <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {under250Banners.map((banner, index) => <div key={banner._id} className="border border-slate-200 rounded-lg overflow-hidden hover:shadow-md transition-shadow">
                      <div className="relative aspect-video bg-slate-100">
        ...
        {activeTab === 'explore-more' && <>
            ...
            {false && <>
                <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 mb-6">
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <select id="restaurant-top10" value={selectedRestaurantTop10} onChange={e => setSelectedRestaurantTop10(e.target.value)} className="mt-1 w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500" disabled={restaurantsLoading}>
                        <option value="">Select a restaurant...</option>
                        {allRestaurants.filter(r => !top10Restaurants.some(tr => tr.restaurant?._id === r._id)).map(restaurant => <option key={restaurant._id} value={restaurant._id}>
                              {restaurant.name}
                            </option>)}
                      </select>
                    </div>
                    <div>
                      ...
                      <Input id="rank" type="number" min="1" max="10" value={selectedRank} onChange={e => setSelectedRank(e.target.value)} className="mt-1" />
                    </div>
                    <Button onClick={handleAddTop10Restaurant} disabled={!selectedRestaurantTop10 || !selectedRank} className="bg-orange-500 hover:bg-orange-600 text-white">
                      Add to Top 10
                    </Button>
                  </div>
                </div>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                  ...
                  {top10Loading ? <div className="flex items-center justify-center py-12">
                      <Loader2 className="w-8 h-8 text-orange-600 animate-spin" />
                    </div> : top10Restaurants.length === 0 ? <div className="text-center py-12 text-slate-500">
                      <Trophy className="w-12 h-12 mx-auto mb-3 text-slate-400" />
                      <p>No restaurants added to Top 10 yet.</p>
                    </div> : <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide" style={{
              scrollbarWidth: 'none',
              msOverflowStyle: 'none'
            }}>
                      {top10Restaurants.sort((a, b) => a.rank - b.rank).map((item, index) => {
                const restaurant = item.restaurant || {};
                const profileImage = restaurant.profileImage?.url || restaurant.profileImage || null;
                return <div key={item._id} className="min-w-[280px] border border-slate-200 rounded-xl p-3 flex flex-col gap-3 bg-white shadow-sm hover:shadow-md transition-shadow">
                              <div className="flex items-center gap-3">
                                ...
                                <div className="w-12 h-12 rounded-lg bg-slate-100 overflow-hidden flex-shrink-0">
                                  {profileImage ? <img src={profileImage} alt={restaurant.name} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-slate-400 text-xs">IMG</div>}
                                </div>
                              ...
                              <div className="flex items-center justify-between border-t border-slate-100 pt-3">
                                <div className="flex items-center gap-2">
                                  ...
                                  <select value={item.rank} onChange={e => handleTop10RankChange(item._id, e.target.value)} className="px-2 py-1 border border-slate-200 rounded text-xs focus:ring-1 focus:ring-orange-500 outline-none">
                                ...
                                <div className="flex items-center gap-1">
                                  ...
                                  <div className="flex gap-1">
                                    <button onClick={() => handleToggleTop10Status(item._id, item.isActive)} className={`p-1.5 rounded transition-colors ${item.isActive ? 'hover:bg-yellow-50 text-yellow-600' : 'hover:bg-green-50 text-green-600'}`} title={item.isActive ? 'Deactivate' : 'Activate'}>
                                      {item.isActive ? <X className="w-4 h-4" /> : <CheckCircle2 className="w-4 h-4" />}
                                    </button>
                                    <button onClick={() => handleDeleteTop10Restaurant(item._id)} disabled={top10Deleting === item._id} className="p-1.5 rounded hover:bg-red-50 text-red-500 disabled:opacity-30 transition-colors">
            ...
            {exploreMoreSubTab === 'gourmet' && <>
                <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 mb-6">
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <select id="restaurant-gourmet" value={selectedRestaurantGourmet} onChange={e => setSelectedRestaurantGourmet(e.target.value)} className="mt-1 w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500" disabled={restaurantsLoading}>
                        <option value="">Select a restaurant...</option>
                        {allRestaurants.filter(r => !gourmetRestaurants.some(gr => gr.restaurant?._id === r._id)).map(restaurant => <option key={restaurant._id} value={restaurant._id}>
                              {restaurant.name}
                            </option>)}
                      </select>
                    </div>
                    <Button onClick={handleAddGourmetRestaurant} disabled={!selectedRestaurantGourmet} className="bg-orange-500 hover:bg-orange-600 text-white">
                      Add to Gourmet
                    </Button>
                  </div>
                </div>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                  ...
                  {gourmetLoading ? <div className="flex items-center justify-center py-12">
                      <Loader2 className="w-8 h-8 text-orange-600 animate-spin" />
                    </div> : gourmetRestaurants.length === 0 ? <div className="text-center py-12 text-slate-500">
                      ...
                      {gourmetRestaurants.sort((a, b) => a.order - b.order).map((item, index) => {
                // Get restaurant cover image with priority: coverImages > menuImages > profileImage
                const coverImages = item.restaurant?.coverImages && item.restaurant.coverImages.length > 0 ? item.restaurant.coverImages.map(img => img.url || img).filter(Boolean) : [];
                const menuImages = item.restaurant?.menuImages && item.restaurant.menuImages.length > 0 ? item.restaurant.menuImages.map(img => img.url || img).filter(Boolean) : [];
                const restaurantImage = coverImages.length > 0 ? coverImages[0] : menuImages.length > 0 ? menuImages[0] : item.restaurant?.profileImage?.url || "https://via.placeholder.com/400";
                return <div key={item._id} className="border border-slate-200 rounded-lg overflow-hidden">
            ...
            <div className="flex-1 overflow-hidden flex flex-col">
              ...
              <div className="px-6 pt-4 pb-3 space-y-3 bg-slate-50 border-b border-slate-200">
                <div className="relative">
                  ...
                  <Input type="text" placeholder="Search restaurants by name or ID..." value={restaurantSearchQuery} onChange={e => setRestaurantSearchQuery(e.target.value)} className="pl-10 h-11 bg-white border-slate-300 focus:border-orange-500 focus:ring-orange-500" />
                </div>
                {selectedRestaurantIds.length > 0 && <div className="flex items-center gap-2">
                    ...
                    <Button variant="ghost" size="sm" onClick={() => setSelectedRestaurantIds([])} className="text-xs text-slate-600 hover:text-slate-900">
                      Clear selection
                    </Button>
                  </div>}
              </div>
              ...
              <div className="flex-1 overflow-y-auto bg-white">
                {restaurantsLoading ? <div className="flex flex-col items-center justify-center py-16">
                    <Loader2 className="w-10 h-10 text-orange-600 animate-spin mb-3" />
                    <p className="text-slate-500">Loading restaurants...</p>
                  </div> : filteredRestaurantsForModal.length === 0 ? <div className="flex flex-col items-center justify-center py-16 text-center px-6">
                  ...
                  return <div key={restaurant._id} className={`px-6 py-4 transition-all cursor-pointer ${isSelected ? 'bg-orange-50 border-l-4 border-l-orange-500' : 'hover:bg-slate-50'}`} onClick={() => toggleRestaurantSelection(restaurant._id)}>
                          <div className="flex items-center gap-4">
                            <div className="flex-shrink-0">
                              <Checkbox checked={isSelected} onCheckedChange={() => toggleRestaurantSelection(restaurant._id)} onClick={e => e.stopPropagation()} className="w-5 h-5" />
                            </div>

                            {/* Restaurant Image */}
                            <div className="flex-shrink-0">
                              {profileImageUrl ? <img src={profileImageUrl} alt={restaurant.name} className="w-16 h-16 rounded-xl object-cover border-2 border-slate-200" onError={e => {
                          e.target.style.display = 'none';
                          e.target.nextSibling.style.display = 'flex';
                        }} /> : null}
                              <div className={`w-16 h-16 rounded-xl bg-gradient-to-br from-orange-400 to-orange-600 flex items-center justify-center text-white font-bold text-lg ${profileImageUrl ? 'hidden' : 'flex'}`}>
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              {restaurant.rating && <div className="flex items-center gap-1 mt-1">
                            ...
                            {/* Selected Indicator */}
                            {isSelected && <div className="flex-shrink-0">
              ...
              <div className="flex items-center justify-between gap-3 px-6 py-4 bg-slate-50 border-t border-slate-200">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <Button onClick={handleLinkRestaurants} disabled={linkingRestaurants || selectedRestaurantIds.length === 0} className="bg-orange-600 hover:bg-orange-700 text-white px-6 min-w-[140px]">
                    {linkingRestaurants ? <>
