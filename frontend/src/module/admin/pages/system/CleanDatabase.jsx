        ...
        <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-4 mb-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="space-y-2">
              {column1.map(table => <div key={table.name} className="flex items-center gap-2 p-2 hover:bg-slate-50 rounded cursor-pointer" onClick={() => handleToggleTable(table.name)}>
                  <input type="checkbox" checked={selectedTables.has(table.name)} onChange={() => handleToggleTable(table.name)} onClick={e => e.stopPropagation()} className="w-4 h-4 text-blue-600 border-slate-300 rounded focus:ring-blue-500 cursor-pointer" />
            ...
            {/* Column 2 */}
            <div className="space-y-2">
              {column2.map(table => <div key={table.name} className="flex items-center gap-2 p-2 hover:bg-slate-50 rounded cursor-pointer" onClick={() => handleToggleTable(table.name)}>
                  <input type="checkbox" checked={selectedTables.has(table.name)} onChange={() => handleToggleTable(table.name)} onClick={e => e.stopPropagation()} className="w-4 h-4 text-blue-600 border-slate-300 rounded focus:ring-blue-500 cursor-pointer" />
                  <span className="text-xs text-slate-700 flex-1">{table.name}</span>
                  <span className="px-2 py-0.5 text-[10px] font-medium bg-slate-200 text-slate-700 rounded-full">
                    {table.count}
            ...
            {/* Column 3 */}
            <div className="space-y-2">
              {column3.map(table => <div key={table.name} className="flex items-center gap-2 p-2 hover:bg-slate-50 rounded cursor-pointer" onClick={() => handleToggleTable(table.name)}>
                  <input type="checkbox" checked={selectedTables.has(table.name)} onChange={() => handleToggleTable(table.name)} onClick={e => e.stopPropagation()} className="w-4 h-4 text-blue-600 border-slate-300 rounded focus:ring-blue-500 cursor-pointer" />
                  <span className="text-xs text-slate-700 flex-1">{table.name}</span>
                  <span className="px-2 py-0.5 text-[10px] font-medium bg-slate-200 text-slate-700 rounded-full">
                    {table.count}
