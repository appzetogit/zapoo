...
export default function PushNotification() {
  ...
  const filteredNotifications = useMemo(() => {
    ...
    return notifications.filter(notification => notification.title.toLowerCase().includes(query) || notification.description.toLowerCase().includes(query));
  ...
  const handleToggleStatus = sl => {
    setNotifications(notifications.map(notification => notification.sl === sl ? {
      ...notification,
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="bg-white divide-y divide-slate-100">
                {filteredNotifications.map(notification => <tr key={notification.sl} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      {notification.image ? <div className="w-12 h-12 rounded-lg overflow-hidden bg-slate-100">
                          <img src={notificationImages[notification.sl] || notificationImage1} alt={notification.title} className="w-full h-full object-cover" onError={e => {
                      e.target.style.display = "none";
                    }} />
                        </div> : <div className="w-12 h-12 rounded-lg bg-slate-100 flex items-center justify-center">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <button onClick={() => handleToggleStatus(notification.sl)} className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-[#FF5200] focus:ring-offset-2 ${notification.status ? "bg-[#FF5200]" : "bg-slate-300"}`}>
                        <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${notification.status ? "translate-x-6" : "translate-x-1"}`} />
                      </button>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-center">
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <button onClick={() => handleDelete(notification.sl)} className="p-1.5 rounded text-red-600 hover:bg-red-50 transition-colors" title="Delete">
