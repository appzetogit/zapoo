      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 mb-6">
          ...
          <div className="flex items-center gap-2 border-b border-slate-200">
            {languageTabs.map(tab => <button key={tab.key} type="button" onClick={() => setActiveLanguage(tab.key)} className={`px-4 py-2 text-sm font-medium border-b-2 transition-colors ${activeLanguage === tab.key ? "border-blue-600 text-blue-600" : "border-transparent text-slate-600 hover:text-slate-900"}`}>
                {tab.label}
              </button>)}
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 mb-6">
            <div>
              ...
              <input type="text" value={roleName} onChange={e => setRoleName(e.target.value)} placeholder="Role name example" className="w-full px-4 py-2.5 border border-slate-300 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm" />
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 mb-6">
            <div className="mb-4">
              ...
              <div className="flex items-center mb-6">
                <input type="checkbox" id="selectAll" checked={allSelected} onChange={e => handleSelectAll(e.target.checked)} className="w-4 h-4 text-blue-600 border-slate-300 rounded focus:ring-blue-500" />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <div className="space-y-3">
                {column1.map(permission => <div key={permission.id} className="flex items-center">
                    <input type="checkbox" id={permission.id} checked={permissions[permission.id] || false} onChange={e => handlePermissionChange(permission.id, e.target.checked)} className="w-4 h-4 text-blue-600 border-slate-300 rounded focus:ring-blue-500" />
              ...
              {/* Column 2 */}
              <div className="space-y-3">
                {column2.map(permission => <div key={permission.id} className="flex items-center">
                    <input type="checkbox" id={permission.id} checked={permissions[permission.id] || false} onChange={e => handlePermissionChange(permission.id, e.target.checked)} className="w-4 h-4 text-blue-600 border-slate-300 rounded focus:ring-blue-500" />
                    <label htmlFor={permission.id} className="ml-2 text-sm text-slate-700">
                      {permission.label}
                    </label>
              ...
              {/* Column 3 */}
              <div className="space-y-3">
                {column3.map(permission => <div key={permission.id} className="flex items-center">
                    <input type="checkbox" id={permission.id} checked={permissions[permission.id] || false} onChange={e => handlePermissionChange(permission.id, e.target.checked)} className="w-4 h-4 text-blue-600 border-slate-300 rounded focus:ring-blue-500" />
                    <label htmlFor={permission.id} className="ml-2 text-sm text-slate-700">
                      {permission.label}
                    </label>
              ...
              {/* Column 4 */}
              <div className="space-y-3">
                {column4.map(permission => <div key={permission.id} className="flex items-center">
                    <input type="checkbox" id={permission.id} checked={permissions[permission.id] || false} onChange={e => handlePermissionChange(permission.id, e.target.checked)} className="w-4 h-4 text-blue-600 border-slate-300 rounded focus:ring-blue-500" />
                    <label htmlFor={permission.id} className="ml-2 text-sm text-slate-700">
                      {permission.label}
                    </label>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
            ...
            <div className="flex items-center gap-3">
              <div className="relative flex-1 sm:flex-initial min-w-[250px]">
                <input type="text" placeholder="Search by Name" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-10 pr-4 py-2.5 w-full text-sm rounded-lg border border-slate-300 bg-white focus:outline-none focus:ring-2 focus:ring-slate-400 focus:border-slate-400" />
            ...
            <table className="w-full">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  ...
                  {visibleColumns.roleName && <th className="px-6 py-4 text-left text-[10px] font-bold text-slate-700 uppercase tracking-wider">
                      <div className="flex items-center gap-2">
                        <span>Role Name</span>
                        <ArrowUpDown className="w-3 h-3 text-slate-400 cursor-pointer hover:text-slate-600" />
                      </div>
                    </th>}
                  {visibleColumns.modules && <th className="px-6 py-4 text-left text-[10px] font-bold text-slate-700 uppercase tracking-wider">
                      <div className="flex items-center gap-2">
                        <span>Modules</span>
                        <ArrowUpDown className="w-3 h-3 text-slate-400 cursor-pointer hover:text-slate-600" />
                      </div>
                    </th>}
                  {visibleColumns.createdAt && <th className="px-6 py-4 text-left text-[10px] font-bold text-slate-700 uppercase tracking-wider">
                      <div className="flex items-center gap-2">
              ...
              <tbody className="bg-white divide-y divide-slate-100">
                {filteredRoles.length === 0 ? <tr>
                    <td colSpan={Object.values(visibleColumns).filter(v => v).length} className="px-6 py-8 text-center text-slate-500">
                      No roles found
                    </td>
                  </tr> : filteredRoles.map((role, index) => <tr key={role.id} className="hover:bg-slate-50 transition-colors">
                      {visibleColumns.si && <td className="px-6 py-4 whitespace-nowrap">
                          <span className="text-sm font-medium text-slate-700">{index + 1}</span>
                        </td>}
                      {visibleColumns.roleName && <td className="px-6 py-4">
                          <span className="text-sm font-medium text-slate-900">{role.roleName}</span>
                        </td>}
                      {visibleColumns.modules && <td className="px-6 py-4">
                          <div className="flex flex-wrap gap-1">
                            {role.modules.map((module, idx) => <span key={idx} className="inline-block px-2 py-1 text-xs bg-slate-100 text-slate-700 rounded">
                                {module}
                              </span>)}
                          </div>
                        </td>}
                      ...
                      {visibleColumns.actions && <td className="px-6 py-4 whitespace-nowrap text-center">
                          <div className="flex items-center justify-center gap-2">
                            ...
                            <button onClick={() => handleDelete(role.id)} className="p-1.5 rounded text-red-600 hover:bg-red-50 transition-colors" title="Delete">
