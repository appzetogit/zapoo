...
export default function Banners() {
  ...
  const handleToggleStatus = sl => {
    setBanners(banners.map(banner => banner.sl === sl ? {
      ...banner,
          ...
          <form onSubmit={handleSubmit}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              <div>
                ...
                <input type="text" value={formData.title} onChange={e => handleInputChange("title", e.target.value)} placeholder="New banner" className="w-full px-4 py-2.5 border border-slate-300 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm" />
              </div>
              ...
              <div>
                ...
                <select value={formData.zone} onChange={e => handleInputChange("zone", e.target.value)} className="w-full px-4 py-2.5 border border-slate-300 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm">
              ...
              <div>
                ...
                <select value={formData.bannerType} onChange={e => handleInputChange("bannerType", e.target.value)} className="w-full px-4 py-2.5 border border-slate-300 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm">
              ...
              <div>
                ...
                <select value={formData.restaurant} onChange={e => handleInputChange("restaurant", e.target.value)} className="w-full px-4 py-2.5 border border-slate-300 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm">
                  <option value="">Select</option>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
            ...
            <div className="flex items-center gap-3">
              <select value={bannerType} onChange={e => setBannerType(e.target.value)} className="px-4 py-2.5 text-sm border border-slate-300 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-slate-400">
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="bg-white divide-y divide-slate-100">
                {filteredBanners.map(banner => <tr key={banner.sl} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-16 h-16 rounded-lg overflow-hidden bg-slate-100 flex-shrink-0">
                          <img src={bannerImages[banner.sl] || bannerImage1} alt={banner.title} className="w-full h-full object-cover" onError={e => {
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <button onClick={() => handleToggleStatus(banner.sl)} className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 ${banner.status ? "bg-blue-600" : "bg-slate-300"}`}>
                        <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${banner.status ? "translate-x-6" : "translate-x-1"}`} />
                      </button>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-center">
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <button onClick={() => handleDelete(banner.sl)} className="p-1.5 rounded text-red-600 hover:bg-red-50 transition-colors" title="Delete">
