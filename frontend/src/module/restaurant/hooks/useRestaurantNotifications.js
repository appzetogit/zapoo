...
export const useRestaurantNotifications = () => {
  ...
  useEffect(() => {
    ...
    const isProductionDeployment = !isLocalhost && (window.location.protocol === 'https:' || frontendHostname.includes('.') && !frontendHostname.startsWith('192.168.') && !frontendHostname.startsWith('10.'));
    ...
    socketRef.current.on('connect_error', error => {
      ...
      if (shouldLog) {
        ...
        console.warn('Restaurant Socket:', isTransportError ? `Cannot reach backend at ${backendUrl}. Ensure the backend is running (e.g. npm run dev in backend).` : error.message);
        if (!isTransportError) {
