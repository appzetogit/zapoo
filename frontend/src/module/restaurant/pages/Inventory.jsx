...
import { Search, SlidersHorizontal, ChevronUp, ChevronDown, Loader2, Utensils, Minus, Plus, ChevronLeft, ChevronRight, X, ThumbsUp } from "lucide-react";
...
function TimePickerWheel({
  ...
  return <AnimatePresence>
      ...
      opacity: 0
    }} className="fixed inset-0 bg-black/50 z-[9999] flex items-center justify-center p-4" onClick={onClose}>
        <motion.div initial={{
        scale: 0.9,
        opacity: 0
      }} animate={{
        ...
        damping: 25,
        stiffness: 300
      }} className="bg-white rounded-lg shadow-2xl w-full max-w-xs overflow-hidden" onClick={e => e.stopPropagation()}>
          <div className="flex items-center justify-center py-8 px-4 relative">
            ...
            <div className="flex-1 flex flex-col items-center">
              <div ref={hourRef} className="w-full h-48 overflow-y-scroll time-picker-scroll snap-y snap-mandatory" style={{
              scrollSnapType: 'y mandatory',
              scrollBehavior: 'smooth',
              WebkitOverflowScrolling: 'touch'
            }} onScroll={() => handleScroll(hourRef.current, setSelectedHour, hours, 40)} onTouchEnd={() => {
              setTimeout(() => {
                if (hourRef.current) {
                  const padding = 80;
                  const itemHeight = 40;
                ...
                {hours.map(hour => <div key={hour} className="h-10 flex items-center justify-center snap-center" style={{
                minHeight: '40px'
              }}>
                    <span className={`text-lg transition-all duration-200 ${selectedHour === hour ? 'font-bold text-gray-900 text-xl' : 'font-normal text-gray-400 text-base'}`}>
                      {hour}
                    </span>
                  </div>)}
                <div className="h-20"></div>
            ...
            <div className="flex-1 flex flex-col items-center">
              <div ref={minuteRef} className="w-full h-48 overflow-y-scroll time-picker-scroll snap-y snap-mandatory" style={{
              scrollSnapType: 'y mandatory',
              scrollBehavior: 'smooth',
              WebkitOverflowScrolling: 'touch'
            }} onScroll={() => handleScroll(minuteRef.current, setSelectedMinute, minutes, 40)} onTouchEnd={() => {
              setTimeout(() => {
                if (minuteRef.current) {
                  const padding = 80;
                  const itemHeight = 40;
                ...
                <div className="h-20"></div>
                {minutes.map(minute => <div key={minute} className="h-10 flex items-center justify-center snap-center" style={{
                minHeight: '40px'
              }}>
                    <span className={`text-lg transition-all duration-200 ${selectedMinute === minute ? 'font-bold text-gray-900 text-xl' : 'font-normal text-gray-400 text-base'}`}>
            ...
            <div className="flex-1 flex flex-col items-center">
              <div ref={periodRef} className="w-full h-48 overflow-y-scroll time-picker-scroll snap-y snap-mandatory" style={{
              scrollSnapType: 'y mandatory',
              scrollBehavior: 'smooth',
              WebkitOverflowScrolling: 'touch'
            }} onScroll={() => handleScroll(periodRef.current, setSelectedPeriod, periods, 40)} onTouchEnd={() => {
              setTimeout(() => {
                if (periodRef.current) {
                  const padding = 80;
                  const itemHeight = 40;
                ...
                <div className="h-20"></div>
                {periods.map(period => <div key={period} className="h-10 flex items-center justify-center snap-center" style={{
                minHeight: '40px'
              }}>
                    <span className={`text-lg transition-all duration-200 ${selectedPeriod === period ? 'font-bold text-gray-900 text-xl' : 'font-normal text-gray-400 text-base'}`}>
                      {period}
                    </span>
                  </div>)}
                <div className="h-20"></div>
          ...
          <div className="border-t border-gray-200 px-4 py-4 flex justify-center">
            <button onClick={handleConfirm} className="text-blue-600 hover:text-blue-700 font-medium text-base transition-colors">
              Okay
            </button>
          </div>
        </motion.div>
...
function SimpleCalendar({
  ...
  return <AnimatePresence>
      ...
      opacity: 0
    }} className="fixed inset-0 bg-black/50 z-[9998] flex items-center justify-center p-4" onClick={onClose}>
        <motion.div initial={{
        scale: 0.9,
        opacity: 0
      }} animate={{
        ...
        damping: 25,
        stiffness: 300
      }} className="bg-white rounded-lg shadow-2xl w-full max-w-sm overflow-hidden" onClick={e => e.stopPropagation()} ref={calendarRef}>
          <div className="p-4">
            ...
            <div className="grid grid-cols-7 gap-1 mb-2">
              {dayNames.map(day => <div key={day} className="text-center text-xs font-medium text-gray-500 py-2">
                  {day}
                </div>)}
            </div>
            ...
            <div className="grid grid-cols-7 gap-1">
              ...
              return <button key={index} onClick={() => {
                ...
                onClose();
              }} className={`h-10 text-sm rounded transition-colors ${!isCurrent ? 'text-gray-300' : isSelectedDate ? 'bg-[#3B82F6] text-white' : isTodayDate ? 'bg-blue-50 text-[#3B82F6] font-semibold' : 'text-gray-700 hover:bg-gray-100'}`}>
                    {date.getDate()}
                  </button>;
            })}
            </div>
...
export default function Inventory() {
  ...
  const [expandedCategories, setExpandedCategories] = useState(() => mockCategories.map(c => c.id));
  ...
  const handleAddonToggle = async (addonId, isAvailable) => {
    try {
      ...
      // Update local state
      setAddons(prev => prev.map(a => a.id === addonId ? {
        ...a,
        isAvailable
      } : a));
  ...
  const totalItems = useMemo(() => categories.reduce((sum, cat) => sum + (cat.itemCount || cat.items?.length || 0), 0), [categories]);
  ...
  const filteredCategories = useMemo(() => {
    ...
    return statusFilteredCategories.map(category => {
      const items = category.items || [];
      const matchesCategory = category.name?.toLowerCase().includes(q) || (category.description || "").toLowerCase().includes(q);
      const matchingItems = items.filter(item => item.name?.toLowerCase().includes(q));
    ...
    try {
      ...
      const updatedSections = sections.map(section => {
        ...
        if (itemId === null) {
          ...
          const updatedItems = section.items.map(item => item.id === String(itemId) ? {
            ...item,
            isAvailable: isAvailable
          } : item);
          ...
          const updatedSubsections = section.subsections.map(subsection => ({
            ...subsection,
            items: subsection.items.map(item => item.id === String(itemId) ? {
              ...item,
              isAvailable: isAvailable
            } : item)
          }));
  ...
  const handleRecommendToggle = async (categoryId, itemId) => {
    ...
    setCategories(prev => prev.map(category => {
      if (category.id !== categoryId) return category;
      const updatedItems = category.items.map(item => item.id === itemId ? {
        ...item,
  ...
  return <div className="min-h-screen bg-gray-100 flex flex-col">
      ...
      <div className=" top-[50px] z-40 bg-gray-100 px-4 pt-4 pb-4">
        <div ref={tabBarRef} className="flex gap-2">
          <motion.button onClick={() => setActiveTab("all-items")} className={`px-6 py-3.5 rounded-full font-medium text-sm whitespace-nowrap relative overflow-hidden ${activeTab === "all-items" ? 'text-white' : 'bg-white text-black'}`} animate={{
          scale: activeTab === "all-items" ? 1.05 : 1
        }} transition={{
          duration: 0.2
        }}>
            {activeTab === "all-items" && <motion.div layoutId="activeTabBackground" className="absolute inset-0 bg-[#3B82F6] rounded-full -z-10" initial={false} transition={{
            type: "spring",
            stiffness: 500,
            damping: 30
          }} />}
            <span className="relative z-10 flex items-center gap-2">
              All items
              {activeTab === "all-items" && <span className="bg-white text-black px-2 py-0.5 rounded-full text-xs font-medium">
                  {totalItems}
                </span>}
            </span>
          </motion.button>

          <motion.button onClick={() => setActiveTab("add-ons")} className={`px-6 py-3.5 rounded-full font-medium text-sm whitespace-nowrap relative overflow-hidden ${activeTab === "add-ons" ? 'text-white' : 'bg-white text-black'}`} animate={{
          scale: activeTab === "add-ons" ? 1.05 : 1
        }} transition={{
          duration: 0.2
        }}>
            {activeTab === "add-ons" && <motion.div layoutId="activeTabBackground" className="absolute inset-0 bg-[#3B82F6] rounded-full -z-10" initial={false} transition={{
            type: "spring",
            stiffness: 500,
            damping: 30
          }} />}
      ...
      isSwiping.current = false;
        ...
        opacity: 1,
        y: 0
      }} onClick={() => navigate("/restaurant/hub-menu")} className="bg-blue-200/20 rounded-lg p-4 mt-4 mb-4 flex items-center justify-between">
        ...
        <div className="flex sticky top-[50px] gap-2 mb-4">
          ...
          <div className="flex-1  z-40 bg-white rounded-sm border border-gray-200">
            ...
            <input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search menu" className="w-full pl-10 pr-4 py-2.5 text-sm text-gray-900 placeholder-gray-400 focus:outline-none rounded-sm" />
          </div>

          {/* Filter Button */}
          <button onClick={() => setFilterOpen(true)} className="px-4 py-2.5 bg-white border border-gray-200 rounded-sm flex items-center justify-center hover:bg-gray-50 transition-colors relative">
            <SlidersHorizontal className="w-5 h-5 text-gray-700" />
            {/* Blue dot indicator when filter is applied */}
            {selectedFilter && <span className="absolute top-1.5 right-1.5 w-2 h-2 bg-blue-600 rounded-full" />}
          </button>
        </div>
        ...
        <div className="space-y-3 mb-6">
                  ...
                  {addons.map(addon => <div key={addon.id} className="bg-white rounded-lg border border-gray-200 p-4">
                    ...
                    e.target.style.display = 'none';
                          ...
                          <div className="flex items-center">
                            <Switch checked={addon.isAvailable !== false} onCheckedChange={checked => handleAddonToggle(addon.id, checked)} className="data-[state=checked]:bg-green-600" />
                          </div>
                        </div>
                      </div>
                    </div>)}
          ...
          return <motion.div key={category.id} initial={{
            ...
            delay: index * 0.05
          }} className="bg-white rounded-lg shadow-0 overflow-hidden relative" ref={el => {
            if (el) {
              ...
              opacity: 1
                ...
                <div className="p-4 cursor-pointer" onClick={() => toggleCategory(category.id)}>
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          {category.inStock ? <p className="text-xs text-green-600 font-medium">In stock</p> : <p className="text-xs text-red-500 font-medium">
                    ...
                    <div className="flex flex-col items-center gap-3 ml-4">
                      ...
                      <div onClick={e => e.stopPropagation()}>
                        <Switch checked={category.inStock} onCheckedChange={checked => handleToggleChange("category", category.id, null, checked)} className="data-[state=checked]:bg-green-600" />
                      </div>
                ...
                duration: 0.2
                      ...
                      <div className="px-4 pb-4 space-y-0">
                        {categoryItems.map((item, itemIndex) => <div key={item.id}>
                            {itemIndex > 0 && <div className="border-t border-dashed border-gray-200 my-3" />}
                            <div className="flex items-center justify-between py-2">
                              <div className="flex items-center gap-2 flex-1">
                                ...
                                <div>
                                  ...
                                  {item.inStock ? <p className="text-xs text-green-600 font-medium">In stock</p> : <p className="text-xs text-red-500 font-medium">No time set. Turn item in stock manually.</p>}
                                </div>
                              </div>
                        ...
                        }} className={`p-1.5 rounded-lg transition-colors ${item.isRecommended ? "bg-blue-100 text-blue-600" : "bg-gray-100 text-gray-400 hover:bg-gray-200"}`} title={item.isRecommended ? "Recommended" : "Click to recommend"}>
                                  <ThumbsUp className="w-4 h-4" />
                                </button>
                                {/* Item Toggle Switch */}
                                <div onClick={e => e.stopPropagation()}>
                                  <Switch checked={item.inStock} onCheckedChange={checked => handleToggleChange("item", category.id, item.id, checked)} className="data-[state=checked]:bg-green-600" />
                                </div>
                              </div>
                            </div>
                          </div>)}
              ...
              <div className="p-6 pb-[calc(6.5rem+env(safe-area-inset-bottom))]">
                ...
                <div className="space-y-4 mb-6">
                  <label className="flex items-center gap-3 cursor-pointer">
                    <input type="radio" name="filter" checked={selectedFilter === "out-of-stock"} onChange={() => setSelectedFilter("out-of-stock")} className="w-5 h-5 text-[#3B82F6] border-gray-300 focus:ring-[#3B82F6]" />
                    <span className="text-base text-gray-900">Out of stock items only</span>
                  </label>

                  <label className="flex items-center gap-3 cursor-pointer">
                    <input type="radio" name="filter" checked={selectedFilter === "in-stock"} onChange={() => setSelectedFilter("in-stock")} className="w-5 h-5 text-[#3B82F6] border-gray-300 focus:ring-[#3B82F6]" />
                    <span className="text-base text-gray-900">In stock items only</span>
                  </label>
                </div>

                <div className="flex gap-3">
                  {selectedFilter && <button onClick={handleFilterClear} className="flex-1 border border-gray-300 text-gray-900 py-3 rounded-lg font-medium hover:bg-gray-50 transition-colors">
                      Clear
                    </button>}
                  <button onClick={handleFilterApply} className={`${selectedFilter ? 'flex-1' : 'w-full'} bg-[#3B82F6] text-white py-3 rounded-lg font-medium hover:bg-blue-700 transition-colors`}>
                    Apply
                  </button>
                </div>
              </div>
          ...
          stiffness: 300
              ...
              return <div className="">
                ...
                <div className="space-y-0 mb-6">
                  ...
                  <label className="flex items-center justify-between py-4 cursor-pointer border-b border-gray-200">
                    <div className="flex items-center gap-3 flex-1">
                      ...
                      {selectedOption === "specific-time" && <div className="ml-auto py-3 flex items-center justify-center gap-4">
                          <button onClick={() => setHours(Math.max(1, hours - 1))} className="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center hover:bg-gray-300 transition-colors">
                            <Minus className="w-4 h-4 text-gray-700" />
                          </button>
                          <span className="text-base font-medium text-gray-900 min-w-[60px] text-center">
                            {hours} hour{hours !== 1 ? 's' : ''}
                          </span>
                          <button onClick={() => setHours(hours + 1)} className="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center hover:bg-gray-300 transition-colors">
                            <Plus className="w-4 h-4 text-gray-700" />
                          </button>
                        </div>}
                      <input type="radio" name="outOfStockOption" checked={selectedOption === "specific-time"} onChange={() => setSelectedOption("specific-time")} style={{
                    accentColor: "#3B82F6"
                  ...
                  <label className="flex items-center justify-between py-4 cursor-pointer border-b border-gray-200">
                    <div className="flex items-center gap-3 flex-1">
                      ...
                      <input type="radio" name="outOfStockOption" checked={selectedOption === "next-business-day"} onChange={() => setSelectedOption("next-business-day")} style={{
                    accentColor: "#3B82F6"
                  }} className="ml-auto w-5 h-5 !text-[#3B82F6] !border-gray-300 !bg-[#3B82F6] !focus:ring-[#3B82F6]" />
                    </div>
                  </label>
                  ...
                  <label className="flex items-center justify-between py-4 cursor-pointer border-b border-gray-200">
                    <div className="flex items-center gap-3 flex-1">
                      ...
                      <input type="radio" name="outOfStockOption" checked={selectedOption === "custom-date-time"} onChange={() => setSelectedOption("custom-date-time")} style={{
                    accentColor: "#3B82F6"
                  }} className="ml-auto w-5 h-5 text-[#3B82F6] border-gray-300 focus:ring-[#3B82F6]" />
                    </div>
                  </label>
                  {selectedOption === "custom-date-time" && <div className="ml-auto py-3 flex items-center justify-center gap-4">
                      <button onClick={() => setShowCalendar(true)} className="flex-1 px-4 py-2.5 border border-gray-300 rounded-lg text-sm text-gray-900 bg-white hover:bg-gray-50 transition-colors flex items-center justify-between">
                        <span>{selectedDate ? formatDate(selectedDate) : "15 Dec 2025"}</span>
                        <ChevronDown className="w-4 h-4 text-gray-500" />
                      </button>
                      <button onClick={() => setShowTimePicker(true)} className="flex-1 px-4 py-2.5 border border-gray-300 rounded-lg text-sm text-gray-900 bg-white hover:bg-gray-50 transition-colors flex items-center justify-between">
                  ...
                  <label className="flex items-center justify-between py-4 cursor-pointer">
                    <div className="flex flex-col gap-1 flex-1">
                      <div className="flex items-center gap-3">
                        ...
                        <input type="radio" name="outOfStockOption" checked={selectedOption === "manual"} onChange={() => setSelectedOption("manual")} style={{
                      accentColor: "#3B82F6"
                    }} className="ml-auto w-5 h-5 !text-[#3B82F6] !border-gray-300 !bg-[#3B82F6] !focus:ring-[#3B82F6]" />
                      </div>
                ...
                <div className="flex gap-3 mt-6">
                  <button onClick={() => setTogglePopupOpen(false)} className="flex-1 border border-gray-300 text-gray-900 py-3 rounded-lg font-medium hover:bg-gray-50 transition-colors">
                    Back
                  </button>
                  <button onClick={handleToggleConfirm} className="flex-1 bg-[#3B82F6] text-white py-3 rounded-lg font-medium hover:bg-blue-700 transition-colors">
                    Confirm
                  </button>
                </div>
              </div>
      ...
      {activeTab !== "add-ons" && <div className="fixed right-4 bottom-24 z-30">
            <motion.button type="button" whileTap={{
        scale: 0.96
      }} onClick={() => setIsMenuOpen(prev => !prev)} className="flex items-center gap-2 px-4 py-2 rounded-full border border-gray-800 bg-white text-sm font-medium shadow-sm">
            ...
            <AnimatePresence>
              {isMenuOpen && <>
                  {/* Backdrop */}
                  <motion.div className="fixed inset-0 bg-black/40 z-30" initial={{
            opacity: 0
          }} animate={{
            opacity: 1
          }} exit={{
