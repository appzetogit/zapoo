...
export default function ChatDetailPage() {
  ...
  return <div className="min-h-screen bg-[#f6e9dc] flex flex-col overflow-hidden">
      {/* Header */}
      <div className="bg-white border-b border-gray-200 px-4 py-4 md:py-3 flex items-center gap-4 rounded-b-3xl md:rounded-b-none fixed top-0 left-0 right-0 z-50">
        <button onClick={() => navigate("/restaurant/conversation")} className="p-2 hover:bg-gray-100 rounded-lg transition-colors">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 pb-20 md:pb-4 space-y-4">
        ...
        delay: index * 0.1
            ...
            <div className={`max-w-[75%] md:max-w-[60%] rounded-2xl px-4 py-2.5 ${msg.sender === "me" ? "bg-[#ff8100] text-white rounded-br-sm" : "bg-white text-gray-900 rounded-bl-sm shadow-sm"}`}>
      ...
      <div className="bg-white border-t border-gray-200 px-4 py-3 fixed bottom-0 left-0 right-0 z-40 md:relative md:border-t-0 md:bottom-auto">
        <div className="flex items-center gap-3">
          <Input type="text" placeholder="Type a message..." value={message} onChange={e => setMessage(e.target.value)} onKeyPress={e => {
          if (e.key === "Enter") {
            handleSend();
          }
          ...
          scale: 0.9
            ...
            <Button onClick={handleSend} disabled={!message.trim()} className="rounded-full bg-[#ff8100] hover:bg-[#e67300] text-white p-2.5 w-10 h-10 flex-shrink-0 disabled:opacity-50 disabled:cursor-not-allowed">
