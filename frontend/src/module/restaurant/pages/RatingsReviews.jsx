...
import { ArrowLeft, Star, ChevronRight, ChevronDown, ThumbsUp, ThumbsDown } from "lucide-react";
...
export default function RatingsReviews() {
  ...
  return <div className="min-h-screen bg-white overflow-x-hidden">
      ...
      <div className="relative w-full">
        <img src={restaurantReviewBanner} alt="Ratings and reviews banner" className="w-full h-auto object-cover" />
      </div>
      ...
      <div className="px-4 py-4 bg-white">
        ...
        <button onClick={() => navigate("/restaurant/feedback?tab=reviews")} className="flex items-center gap-1 text-blue-600 text-sm font-normal hover:text-blue-700 transition-colors">
        ...
        <div className="border  border-gray-100 rounded-lg p-4">
          ...
          <div className="space-y-0">
            ...
            return <div key={item.id}>
                  <button onClick={() => toggleAccordion(item.id)} className="w-full flex items-center justify-between py-3 text-left">
                    <span className="text-sm text-gray-900 font-normal pr-4">
                      {item.question}
                    </span>
                    <ChevronDown className={`w-5 h-5 text-gray-900 shrink-0 transition-transform ${isExpanded ? "rotate-180" : ""}`} />
                  </button>

                  {index < accordionItems.length - 1 && <div className="border-b border-gray-300" />}
      ...
      {/* Feedback Section */}
      {!feedbackSubmitted && <div className="px-4 py-6 pb-8">
          <p className="text-base font-semibold text-gray-900 text-center mb-4">
            Was this helpful in resolving your query?
          </p>
          <div className="flex items-center gap-3">
            <button onClick={handleThankYou} className="flex-1 flex items-center justify-center gap-2 border-2 border-blue-600 bg-white rounded-lg py-3 px-4 hover:bg-blue-50 transition-colors">
              <ThumbsUp className="w-5 h-5 text-blue-600" />
              <span className="text-blue-600 text-sm font-semibold">Yes, thank you</span>
            </button>
            <button onClick={handleNotHelpful} className="flex-1 flex items-center justify-center gap-2 border-2 border-gray-400 bg-white rounded-lg py-3 px-4 hover:bg-gray-50 transition-colors">
      ...
      {/* Thank You Popup */}
      <BottomPopup isOpen={showThankYouPopup} onClose={handleThankYouPopupClose} showHandle={true}>
      ...
      {/* Not Helpful Popup */}
      <BottomPopup isOpen={showNotHelpfulPopup} onClose={handleNotHelpfulPopupClose} showHandle={true}>
        <div className="text-center py-6">
