...
export default function ReviewsPage() {
  ...
  const filteredReviews = reviews.filter(review => {
    ...
    return review.orderId.toLowerCase().includes(searchLower) || review.productName.toLowerCase().includes(searchLower) || review.reviewerName.toLowerCase().includes(searchLower);
  });
  return <div className="min-h-screen bg-[#f6e9dc] overflow-x-hidden pb-24 md:pb-6">
      ...
      <div className="bg-white border-b border-gray-200 px-4 py-3">
        <div className="relative">
          <Input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search by order id or food name..." className="w-full pl-4 pr-10 py-2.5 rounded-lg border-gray-200 focus:ring-2 focus:ring-[#ff8100]" />
              ...
              <CardContent className="p-4 space-y-3">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-shrink-0 w-12 h-12 rounded-lg overflow-hidden bg-gray-100 border border-gray-200">
                    {failedImages.has(review.id) || !review.productImage ? <div className="w-full h-full flex items-center justify-center bg-gray-100">
                        <ImageIcon className="w-6 h-6 text-gray-400" />
                      </div> : <img src={review.productImage} alt={review.productName} className="w-full h-full object-cover" onError={() => {
                  ...
                  {/* Action Button */}
                  <Button onClick={() => {
                if (review.hasReply) {} else {
                  navigate(`/restaurant/reviews/${review.id}/reply`);
                }
              }} className={`flex-shrink-0 px-4 py-2 text-sm font-medium rounded-lg ${review.hasReply ? "bg-blue-100 hover:bg-blue-200 text-blue-700" : "bg-[#ff8100] hover:bg-[#e67300] text-white"}`}>
