...
export default function FreebiesTiming() {
  ...
  const isFormValid = customerGroup && offerDays && startDate && targetMealtime && selectedFreebies.length > 0 && selectedMinOrderValue;
  const handlePreview = () => {
  ...
  return <div className="min-h-screen bg-gray-100 flex flex-col">
        ...
        <div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
            <label className="flex items-start justify-between gap-3 cursor-pointer">
              ...
              <input type="radio" name="customerGroupFreebies" value="all" checked={customerGroup === "all"} onChange={e => setCustomerGroup(e.target.value)} className="mt-1 w-5 h-5 text-black border-gray-400 focus:ring-black" style={{
              accentColor: "#000000"
            }} />
            </label>
            <label className="flex items-start justify-between gap-3 cursor-pointer">
              ...
              <input type="radio" name="customerGroupFreebies" value="new" checked={customerGroup === "new"} onChange={e => setCustomerGroup(e.target.value)} className="mt-1 w-5 h-5 text-black border-gray-400 focus:ring-black" style={{
              accentColor: "#000000"
            }} />
            </label>
          </div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-4">
            ...
            <Popover>
              <div className="space-y-2">
                ...
                <PopoverTrigger asChild>
                  <button type="button" className="w-full px-4 py-3 border border-gray-300 rounded-lg bg-white flex items-center justify-between text-sm font-medium text-gray-900">
                    <span>{startDate}</span>
              ...
              <PopoverContent align="center" className="p-2">
                <Calendar mode="single" selected={startDateDate} onSelect={date => {
                if (!date) return;
      ...
      <AnimatePresence>
        {isMealtimeOpen && <>
          ...
          opacity: 0
        }} onClick={() => setIsMealtimeOpen(false)} className="fixed inset-0 bg-black/50 z-[9999]" />
            <motion.div initial={{
          y: "100%"
        }} animate={{
          y: 0
