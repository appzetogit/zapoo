...
export default function SwitchOutlet() {
  ...
  const visibleOutlets = showOffline ? outlets : outlets.filter(outlet => outlet.status === "online");
    ...
    ease: [0.25, 0.1, 0.25, 1]
      ...
      <div className="px-4 py-6">
        ...
        <div className="space-y-4 mb-8">
          {visibleOutlets.map((outlet, index) => <motion.div key={outlet.id} initial={{
          opacity: 0,
          y: 8
        }} animate={{
          opacity: 1,
          ...
          delay: 0.1 + index * 0.05
        }} onClick={() => handleOutletClick(outlet.id)} className="bg-blue-50 rounded-lg  cursor-pointer hover:bg-blue-100 border-blue-200 border transition-colors">
              <div className="flex items-start gap-4 p-2 pb-1  rounded-t-lg">
                ...
                <div className="w-16 h-16 bg-white rounded-lg flex items-center justify-center shrink-0 overflow-hidden shadow-sm border border-gray-200">
                  {outlet.image && outlet.image !== "/api/placeholder/80/80" ? <img src={outlet.image} alt={outlet.name} className="w-full h-full object-cover" onError={e => {
                e.target.style.display = 'none';
                if (e.target.nextSibling) {
                  e.target.nextSibling.style.display = 'flex';
                }
              }} /> : null}
                  <div className="w-full h-full bg-gray-100 flex items-center justify-center" style={{
                display: outlet.image && outlet.image !== "/api/placeholder/80/80" ? 'none' : 'flex'
              }}>
