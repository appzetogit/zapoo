...
export default function ToHub() {
  ...
  const quickLinks = [
    { id: "promoted-banners", label: "Promoted Banners", icon: Megaphone, route: "/restaurant/advertisements" },
  ...
  const calculateWeeklyChartData = (orders, startDate, endDate) => {
    ...
    const dayBuckets = daysInInterval.map(day => ({
      date: day,
      dayLabel: format(day, 'EEE'), // Mon, Tue, Wed
      fullLabel: format(day, 'dd MMM'), // 01 Jan
      orders: 0,
      sales: 0
    }))
  ...
  const calculateYearlyChartData = (orders, startDate, endDate) => {
    ...
    const monthBuckets = monthsInInterval.map(month => ({
      date: month,
      monthLabel: format(month, 'MMM'), // Jan, Feb
  ...
  const fetchOrdersAndUpdateChart = useCallback(async (rangeId) => {
    try {
      ...
      if (allOrders.length > 0) {
        ...
        const { chartData: newChartData, totalSales: newTotalSales, totalOrders: newTotalOrders } = result
                ...
                <div className="h-16 chart-shell-mini">
                  <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={0}>
                    <AreaChart data={chartData} margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
                      ...
                      <XAxis dataKey={xAxisKey} tick={{ fontSize: 10, fill: "#9ca3af" }} />
                      <YAxis hide />
                      <Tooltip />
      ...
      <>
        <div className="space-y-4">
          <div className="px-4">
            ...
            {[
              { title: "Net sales", value: `${totalSales} • 0%`, dataKey: "sales", color: "#f97316" },
              { title: "Orders delivered", value: `${totalOrders} • 0%`, dataKey: "orders", color: "#f97316" },
              {
                ...
                <div className="h-16 chart-shell-mini">
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={chartData} margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
                      ...
                      <CartesianGrid strokeDasharray="3 3" stroke="#f5f5f5" />
                      <XAxis dataKey={xAxisKey} tick={{ fontSize: 10, fill: "#9ca3af" }} hide />
                      <YAxis hide />
                      <Tooltip />
                      <Area type="monotone" dataKey={section.dataKey} stroke={section.color} fill={`url(#mini-${section.dataKey})`} />
                    </AreaChart>
          ...
          {activeTopTab === "my-feed" ? (
            ...
            <div className="p-4 bg-gray-100 min-h-screen">
              <motion.div
                ...
                className="bg-white rounded-lg p-4 flex items-center gap-4  border border-gray-200 cursor-pointer mb-4"
              >
