...
export default function RestaurantStatus() {
  ...
  useEffect(() => {
    ...
    const checkIfOpen = () => {
      const now = new Date();
      const currentDayFull = now.toLocaleDateString('en-US', {
        weekday: 'long'
      }); // "Monday", "Tuesday", etc.
      ...
      try {
        ...
        if (restaurant?.isAcceptingOrders !== undefined) {
          ...
          window.dispatchEvent(new CustomEvent('restaurantStatusChanged', {
            detail: {
              isOnline: restaurant.isAcceptingOrders
            }
          }));
        } else {
  ...
  return <div className="min-h-screen bg-gray-100 overflow-x-hidden">
      {/* Header */}
  ...
  {!isWithinTimings && restaurantData && !isDayClosed && <div className="bg-pink-50 rounded-b-lg rounded-t-none p-4 flex items-start gap-3">
