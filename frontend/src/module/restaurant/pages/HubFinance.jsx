  ...
  const getReportData = () => {
    ...
    return {
      ...
      currentCycle: {
        ...
        estimatedPayout: `₹${(currentCycle.estimatedPayout || 0).toLocaleString('en-IN', {
          minimumFractionDigits: 2,
          maximumFractionDigits: 2
        })}`,
  ...
  return <div className="min-h-screen bg-gray-100 flex flex-col">
      ...
      <div className="px-4 py-3">
        <div className="flex gap-2">
          <button onClick={() => setActiveTab("payouts")} className={`flex-1 py-3 px-4 rounded-full font-medium text-sm transition-colors ${activeTab === "payouts" ? "bg-[#3B82F6] text-white" : "bg-white text-gray-600 border border-gray-300"}`}>
            Payouts
          </button>
          <button onClick={() => setActiveTab("invoices")} className={`flex-1 py-3 px-4 rounded-full font-medium text-sm transition-colors ${activeTab === "invoices" ? "bg-[#3B82F6] text-white" : "bg-white text-gray-600 border border-gray-300"}`}>
            Invoices & Taxes
          </button>
        </div>
      </div>
                ...
                {loading || walletLoading ? <div className="py-8 text-center text-gray-500">Loading...</div> : <>
                  ...
                  maximumFractionDigits: 2
                    ...
                    {financeData?.currentCycle?.recommendedItems && <div className="mt-4 p-3 bg-blue-50 rounded-lg border border-blue-100">
                        ...
                        {financeData.currentCycle.recommendedItems.topItems?.length > 0 && <div className="mt-3">
                            <p className="text-[10px] text-gray-500 uppercase mb-2 font-bold">Top Boosted Items</p>
                            <div className="space-y-1.5">
                              {financeData.currentCycle.recommendedItems.topItems.map((item, idx) => <div key={idx} className="flex justify-between items-center text-[11px] bg-white/50 p-1.5 rounded">
              ...
              <div className="space-y-3">
                <div className="flex gap-2">
                  <div className="flex-1 relative" ref={dateRangePickerRef}>
                    <button onClick={() => setShowDateRangePicker(!showDateRangePicker)} className="w-full bg-white rounded-lg px-4 py-3 flex items-center justify-between border border-gray-200 hover:border-gray-300 transition-colors cursor-pointer">
                    ...
                    <AnimatePresence>
                      {showDateRangePicker && <motion.div initial={{
                    opacity: 0,
                    y: -10
                  }} animate={{
                    opacity: 1,
                    ...
                    y: -10
                          ...
                          return dateOptions.map((option, index) => <button key={index} onClick={() => {
                  ...
                  <div className="relative" ref={downloadMenuRef}>
                    <button onClick={() => setShowDownloadMenu(!showDownloadMenu)} className="bg-[#3B82F6] text-white rounded-lg px-4 py-3 flex items-center justify-center gap-2 hover:bg-blue-700 transition-colors">
                    ...
                    <AnimatePresence>
                      {showDownloadMenu && <motion.div initial={{
                    opacity: 0,
                    scale: 0.95,
                    y: -10
                  }} animate={{
                    ...
                    ease: "easeOut"
                          ...
                          <button onClick={downloadPDF} className="w-full flex items-center gap-3 px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 transition-colors">
                ...
                {loadingPastCycles ? <div className="bg-white rounded-lg p-4">
                    <p className="text-sm text-gray-600 text-center">Loading past cycles...</p>
                  </div> : <>
                    {/* Show past cycles orders if available */}
                    {pastCyclesData && pastCyclesData.orders && pastCyclesData.orders.length > 0 && <div className="bg-white rounded-lg p-4 space-y-3">
                        {pastCyclesData.orders.map((order, index) => <div key={order.orderId || index} className="border-b border-gray-200 pb-3 last:border-b-0 last:pb-0">
                            <div className="flex justify-between items-start">
                              ...
                              <div className="text-right ml-4">
                                <p className="text-sm font-bold text-gray-900">
                                  ₹{(order.payout || 0).toLocaleString('en-IN', {
                          minimumFractionDigits: 2,
                          maximumFractionDigits: 2
                        })}
                                </p>
                    ...
                    {(!pastCyclesData || !pastCyclesData.orders || pastCyclesData.orders.length === 0) && !loadingPastCycles && financeData?.currentCycle?.orders && financeData.currentCycle.orders.length > 0 && <div className="bg-white rounded-lg p-4 space-y-3">
                        {financeData.currentCycle.orders.map((order, index) => <div key={order.orderId || index} className="border-b border-gray-200 pb-3 last:border-b-0 last:pb-0">
                            <div className="flex justify-between items-start">
                              <div className="flex-1">
                                <p className="text-sm font-semibold text-gray-900 mb-1">
                                  Order ID: {order.orderId || 'N/A'}
      ...
      <AnimatePresence>
        {showWithdrawalModal && <>
          ...
          opacity: 0
        }} className="fixed inset-0 bg-black/50 z-50" onClick={() => setShowWithdrawalModal(false)} />
            <motion.div initial={{
          opacity: 0,
          scale: 0.95,
          y: 20
