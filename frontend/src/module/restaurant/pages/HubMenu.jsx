...
export default function HubMenu() {
  ...
  const restaurantExpertise = restaurantData?.cuisines?.length > 0 ? restaurantData.cuisines.join(", ") : "";
  ...
  useEffect(() => {
    if (isFilterOpen || isAddPopupOpen || isMenuOpen || isAvailabilityPopupOpen || isCategoryOptionsOpen || isEditCategoryOpen || isAddSubCategoryOpen || isAddCategoryPopupOpen || isSearchOpen || isAddAddonModalOpen) {
  ...
  }, [isFilterOpen, isAddPopupOpen, isMenuOpen, isAvailabilityPopupOpen, isCategoryOptionsOpen, isEditCategoryOpen, isAddSubCategoryOpen, isAddCategoryPopupOpen, isSearchOpen, isAddAddonModalOpen]);
      ...
      if (switchingOffTarget.type === 'item') {
        ...
        if (response.data && response.data.success) {
          ...
          if (response.data.data && response.data.data.menu) {
            ...
            setMenuData(prev => prev.map(g => ({
              ...g,
              items: g.items.map(i => i.id === switchingOffTarget.id ? {
                ...i,
                isAvailable: false
              } : i),
              subsections: g.subsections?.map(sub => ({
                ...sub,
                items: sub.items.map(i => i.id === switchingOffTarget.id ? {
                  ...i,
                  isAvailable: false
                } : i)
              }))
          ...
          toast.success(availabilityReason === 'manual' ? 'Item availability updated' : 'Item availability scheduled successfully');
        } else {
        ...
        setMenuData(prev => prev.map(g => g.id === switchingOffTarget.id ? {
          ...g,
          isEnabled: false
        } : g));
