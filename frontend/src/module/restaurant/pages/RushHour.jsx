...
export default function RushHour() {
  ...
  return <div className="min-h-screen bg-white flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-6">
        ...
        <div className="mb-8">
          ...
          <div className="space-y-3">
            {["Get more time to prepare food", "Show correct delivery time to customers", "Avoid crowding of riders at your restaurant"].map((benefit, index) => <div key={index} className="flex items-center gap-3">
        ...
        <div className="mb-6">
          ...
          <RadioGroup value={selectedTime} onValueChange={setSelectedTime} className="space-y-4">
            {timeOptions.map(option => <div key={option.value} className="flex items-center gap-3">
                <RadioGroupItem value={option.value} id={option.value} className="h-5 w-5" />
                <Label htmlFor={option.value} className="text-sm text-gray-900 font-normal cursor-pointer flex-1">
                  {option.label}
                </Label>
              </div>)}
          </RadioGroup>
      ...
      <div className="px-4 pb-6 pt-4 bg-white border-t border-gray-200">
        <button onClick={handleConfirm} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 px-4 rounded-lg transition-colors">
          Confirm
        </button>
      </div>
    </div>;
