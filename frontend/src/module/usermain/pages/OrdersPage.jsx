...
import { ArrowLeft, Clock, CheckCircle, Home, Heart, ShoppingBag, Menu, ChefHat, Loader2, AlertCircle } from "lucide-react";
...
export default function OrdersPage() {
      ...
      try {
        ...
        if (ordersData.length > 0) {
          ...
          const transformedOrders = ordersData.map(order => {
            ...
            const deliveredAt = order.tracking?.delivered?.timestamp ? new Date(order.tracking.delivered.timestamp) : null;
            return {
  ...
  return <div className="min-h-screen bg-[#f6e9dc] pb-20 md:pb-24">
      {/* Header */}
      ...
      {/* Orders List */}
      {loading ? <div className="px-4 py-8 text-center">
        ...
        </div> : orders.length === 0 ? <div className="px-4 py-8 text-center">
          <div className="bg-white rounded-xl p-6 md:p-8 shadow-sm">
            ...
            <button onClick={() => navigate('/usermain')} className="text-[#ff8100] text-xs md:text-sm font-semibold">
              Start Shopping
            </button>
          </div>
        </div> : <div className="px-4 py-3 md:py-4 space-y-2.5 md:space-y-4">
          {orders.map(order => {
        // Check if payment failed
        const paymentFailed = order.payment?.status === 'failed' || order.payment?.status === 'pending';
        return <div key={order.id || order.mongoId} className="bg-white rounded-xl p-3 md:p-4 shadow-sm hover:shadow-md transition-shadow">
                <div className="flex items-start justify-between mb-2 md:mb-3">
                  ...
                  <div className="flex flex-col items-end gap-1 ml-2 flex-shrink-0">
                    <div className={`px-1.5 md:px-2 py-0.5 md:py-1 rounded-full text-[10px] md:text-xs font-semibold ${order.status === "Delivered" ? "bg-green-100 text-green-700" : order.status === "Preparing" || order.status === "Ready" ? "bg-orange-100 text-orange-700" : order.status === "Out for Delivery" ? "bg-blue-100 text-blue-700" : "bg-gray-100 text-gray-700"}`}>
                      {order.status}
                    </div>
                    {paymentFailed && <div className="flex items-center gap-1 px-1.5 py-0.5 bg-red-50 rounded-full">
                ...
                {/* Order Items Preview */}
                {order.itemsList && order.itemsList.length > 0 && <div className="mb-2 md:mb-3">
                    <div className="flex flex-wrap gap-1.5">
                      {order.itemsList.slice(0, 3).map((item, idx) => <span key={idx} className="text-[10px] md:text-xs text-gray-600 bg-gray-50 px-2 py-0.5 rounded-full">
                          {item.quantity}x {item.name}
                        </span>)}
                      {order.itemsList.length > 3 && <span className="text-[10px] md:text-xs text-gray-500">
                ...
                <div className="flex items-center justify-between pt-2 md:pt-3 border-t border-gray-100">
                  ...
                  <button onClick={() => navigate(`/usermain/orders/${order.id || order.mongoId}`)} className="text-[#ff8100] text-[10px] md:text-xs font-semibold ml-2 flex-shrink-0 hover:underline">
                    View Details
                  </button>
                </div>
              </div>;
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 shadow-lg z-50">
        <div className="flex items-center justify-around py-2 px-4">
          <button onClick={() => navigate('/usermain')} className="flex flex-col items-center gap-1 p-2 text-gray-600 hover:text-[#ff8100] transition-colors">
          ...
          <button onClick={() => navigate('/usermain/orders')} className="flex flex-col items-center gap-1 p-2 text-[#ff8100]">
