...
import { calculateDistance } from '@/module/deliveryV2/hooks/proximity.utils';
...
export const useProximityCheck = () => {
  ...
  const targetLocation = useMemo(() => {
    ...
    if (['PICKED_UP', 'DELIVERING', 'REACHED_DROP'].includes(tripStatus)) {
  ...
  const actionLimit = useMemo(() => {
    ...
    if (tripStatus === 'PICKED_UP' || tripStatus === 'DELIVERING') return settings.deliveryRangeLimit || 500;
    return 500;
  }, [tripStatus, settings]);
