...
export const useOrderManager = () => {
  ...
  const resolveNumericEarning = (payload) => {
    if (!payload) return 0;
    ...
    const direct = Number(payload.riderEarning || payload.deliveryEarning || payload.earningAmount || payload.amount || payload.totalEarning);
  ...
  const completeDelivery = async (otp) => {
    ...
    try {
      ...
      if (verifyRes?.data?.success) {
        ...
        // Preserve/normalize earning fields so completion summary never drops to 0.
        const completionEarnings = completeRes?.data?.data?.earnings;
        const mergedOrder = {
          ...(activeOrder || {}),
          ...(verifyRes?.data?.data?.order || {}),
          ...(finalOrder || {})
        };
        const amountFromCompletionPayload = Number(completionEarnings?.amount || completionEarnings?.totalEarning || 0);
        const normalizedAmount =
          (Number.isFinite(amountFromCompletionPayload) && amountFromCompletionPayload > 0)
            ? amountFromCompletionPayload
            : resolveNumericEarning(mergedOrder);

        if (Number.isFinite(normalizedAmount) && normalizedAmount > 0) {
          mergedOrder.deliveryEarning = Number(mergedOrder.deliveryEarning || 0) > 0 ? mergedOrder.deliveryEarning : normalizedAmount;
          mergedOrder.riderEarning = Number(mergedOrder.riderEarning || 0) > 0 ? mergedOrder.riderEarning : normalizedAmount;
          mergedOrder.earningAmount = Number(mergedOrder.earningAmount || 0) > 0 ? mergedOrder.earningAmount : normalizedAmount;
          mergedOrder.amount = Number(mergedOrder.amount || 0) > 0 ? mergedOrder.amount : normalizedAmount;
          mergedOrder.earnings = mergedOrder.earnings || completionEarnings || { amount: normalizedAmount, totalEarning: normalizedAmount };
        }

        // Update local order state so Summary Modal shows delivered status + correct earning.
        if (mergedOrder) setActiveOrder(mergedOrder);
