...
import { getHaversineDistance, calculateETA } from '@/module/deliveryV2/utils/geo';
...
export const NewOrderModal = ({ order, onAccept, onReject, onMinimize }) => {
  ...
  const estimated = order?.estimatedEarnings || {};
  const earnings =
    Number(estimated?.totalEarning) ||
    Number(order?.earnings) ||
    Number(order?.riderEarning) ||
    Number(order?.deliveryEarning) ||
    (order?.orderAmount ? Number(order.orderAmount) * 0.1 : 0);
  const earningsBreakdown = {
    basePayout: Number(estimated?.basePayout) || 0,
    distanceCommission: Number(estimated?.distanceCommission) || 0,
    commissionPerKm: Number(estimated?.commissionPerKm) || 0,
    distance: Number(estimated?.distance) || Number(estimated?.deliveryDistanceRaw) || 0,
    minDistance: Number(estimated?.minDistance) || Number(estimated?.breakdown?.minDistance) || 0,
    maxDistance: Number(estimated?.maxDistance) || Number(estimated?.breakdown?.maxDistance) || 0,
  };
  ...
  const customerLocation =
    locationToLatLng(order.customerLocation) ||
    locationToLatLng(order.deliveryLocation) ||
    locationToLatLng(order.deliveryAddress?.location) ||
    geoCoords ||
    null;
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar">
          ...
          <div className="bg-linear-to-br from-emerald-500 via-green-500 to-emerald-600 px-6 py-5 flex justify-between items-center text-white">
            ...
            <div className="bg-gray-50/50 rounded-3xl p-5 border border-gray-100/50">
              ...
              {/* Tier-based earning breakdown */}
              <div className="bg-emerald-50/70 rounded-2xl p-4 border border-emerald-100">
                <p className="text-[10px] font-black uppercase tracking-[0.18em] text-emerald-700 mb-3">Earning Breakdown</p>
                <div className="space-y-1.5 text-[12px] font-bold text-emerald-900">
                  <div className="flex items-center justify-between">
                    <span>Base payout</span>
                    <span>₹{earningsBreakdown.basePayout.toFixed(2)}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span>Distance commission</span>
                    <span>₹{earningsBreakdown.distanceCommission.toFixed(2)}</span>
                  </div>
                  <div className="flex items-center justify-between">
                ...
                {earningsBreakdown.commissionPerKm > 0 && (
                  <p className="text-[10px] text-emerald-700/80 font-semibold mt-2">
                    Formula: ₹{earningsBreakdown.basePayout.toFixed(0)} + ({earningsBreakdown.distance.toFixed(1)} km × ₹{earningsBreakdown.commissionPerKm.toFixed(0)}/km)
                  </p>
                )}
                {earningsBreakdown.maxDistance > 0 && (
                  <p className="text-[10px] text-emerald-700/70 font-semibold mt-1">
                    Slab: {earningsBreakdown.minDistance.toFixed(1)}-{earningsBreakdown.maxDistance.toFixed(1)} km
                  </p>
                )}
              </div>
	          </div>
