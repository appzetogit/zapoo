...
export const OrderSummaryModal = ({ order, onDone }) => {
  ...
  const basePayout = Number(estimated?.basePayout) || 0;
  const distanceCommission = Number(estimated?.distanceCommission) || 0;
        ...
        >
          ...
          <p className="text-white/90 text-lg mb-12">Trip completed successfully.</p>
            ...
            <p className="text-gray-950 text-6xl font-bold mb-6 tracking-tighter">₹{Number(earnings).toFixed(2)}</p>
              ...
              <div className="mt-4 text-left bg-emerald-50 rounded-2xl p-4 border border-emerald-100">
                <p className="text-[10px] font-black uppercase tracking-widest text-emerald-700 mb-2">Breakdown</p>
                <div className="text-[12px] font-bold text-emerald-900 space-y-1">
                  <div className="flex justify-between"><span>Base payout</span><span>₹{basePayout.toFixed(2)}</span></div>
                  <div className="flex justify-between"><span>Distance commission</span><span>₹{distanceCommission.toFixed(2)}</span></div>
                </div>
              </div>
	          </div>
