...
export const ProfileDetailsV2 = () => {
  ...
  const submitBankDetails = async () => {
    ...
    try {
      ...
      if (upiQrFile) {
        const uploadRes = await uploadAPI.uploadMedia(upiQrFile, { folder: "delivery/documents" })
        const uploadData = uploadRes?.data?.data || uploadRes?.data || {}
        if (!uploadData?.url) {
          throw new Error("Failed to upload UPI QR")
        }
        formData.append("documents[bankDetails][upiQrCode]", uploadData.url)
      }
