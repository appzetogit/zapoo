...
import { useProximityCheck } from '@/module/deliveryV2/hooks/useProximityCheck';
import { useOrderManager } from '@/module/deliveryV2/hooks/useOrderManager';
...
// Components
import LiveMap from '@/module/deliveryV2/components/map/LiveMap';
import { NewOrderModal } from '@/module/deliveryV2/components/modals/NewOrderModal';
import { PickupActionModal } from '@/module/deliveryV2/components/modals/PickupActionModal';
import { DeliveryVerificationModal } from '@/module/deliveryV2/components/modals/DeliveryVerificationModal';
import { OrderSummaryModal } from '@/module/deliveryV2/components/modals/OrderSummaryModal';
import ActionSlider from '@/module/deliveryV2/components/ui/ActionSlider';

// Sub Pages
import PocketV2 from '@/module/deliveryV2/pages/PocketV2';
import HistoryV2 from '@/module/deliveryV2/pages/HistoryV2';
import ProfileV2 from '@/module/deliveryV2/pages/ProfileV2';
...
import { getHaversineDistance, calculateETA, calculateHeading } from '@/module/deliveryV2/utils/geo';
...
export default function DeliveryHomeV2({ tab = 'feed' }) {
  ...
  const lastGpsErrorCodeRef = useRef(null);
  const gpsTimeoutCountRef = useRef(0);
  ...
  const fallbackDistanceKm = useMemo(() => {
    ...
    const candidates = [
      ...
      activeOrder?.assignmentInfo?.distance,
      activeOrder?.deliveryState?.routeToDelivery?.distance,
      activeOrder?.pricing?.distanceKm,
    ];
  ...
  const distanceDisplayKm = useMemo(() => {
    ...
    if (Number.isFinite(fallbackDistanceKm) && fallbackDistanceKm > 0) {
      return Number(fallbackDistanceKm).toFixed(1);
    }
    return '--';
  }, [distanceToTarget, fallbackDistanceKm]);

  const etaDisplayMin = useMemo(() => {
    if (eta != null && Number.isFinite(Number(eta)) && Number(eta) > 0) {
      return String(eta);
    }
    const payloadEta = Number(
      activeOrder?.estimatedTime ||
      activeOrder?.duration ||
      activeOrder?.eta ||
      0
    );
    if (Number.isFinite(payloadEta) && payloadEta > 0) {
      return String(Math.ceil(payloadEta));
    }
    if (Number.isFinite(fallbackDistanceKm) && fallbackDistanceKm > 0) {
      return String(Math.max(1, Math.ceil((fallbackDistanceKm * 1000) / 8 / 60)));
    }
    return '--';
  }, [eta, activeOrder, fallbackDistanceKm]);
  ...
  useEffect(() => {
    ...
    const handleGeoError = (error) => {
      ...
      const sameAsLast = lastGpsErrorCodeRef.current === code;
      lastGpsErrorCodeRef.current = code;

      if (code === 1) {
        gpsTimeoutCountRef.current = 0;
        if (sameAsLast) return;
        toast.error('Location permission denied. Please allow location access.');
      } else if (code === 2) {
        gpsTimeoutCountRef.current = 0;
        if (sameAsLast) return;
        toast.error('Unable to read GPS location. Check network/GPS and retry.');
      } else if (code === 3) {
        gpsTimeoutCountRef.current += 1;
        // Timeout is common on weak/mobile GPS. Show toast only after repeated failures.
        if (gpsTimeoutCountRef.current >= 3 && !sameAsLast) {
          toast.error('GPS is slow. Move to open sky or keep location ON.');
        }
        // Try a lower-accuracy one-shot fallback so UI can continue with any available fix.
        ...
        console.warn('[DeliveryHomeV2][GPS] watch timeout, applied one-shot fallback');
      } else {
        gpsTimeoutCountRef.current = 0;
        if (sameAsLast) return;
        toast.error('Unable to get live location.');
      }
      console.error('[DeliveryHomeV2][GPS] watchPosition error:', {
        code,
                              ...
                              <div>
                                 ...
                                 <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full border ${isWithinRange ? 'bg-emerald-50 border-emerald-100' : 'bg-orange-50 border-orange-100'}`}>
                                   ...
                                   <span className={`text-[10px] font-black uppercase tracking-widest ${isWithinRange ? 'text-emerald-600' : 'text-orange-500'}`}>
                                     {isWithinRange ? 'Ready to Arrive' : `${distanceDisplayKm} km • ${etaDisplayMin} min`}
                                   </span>
                                 </div>
                              </div>
                            </div>
