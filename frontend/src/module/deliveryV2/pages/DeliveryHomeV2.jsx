...
export default function DeliveryHomeV2({ tab = 'feed' }) {
  ...
  const computedDistanceMeters = React.useMemo(() => {
    if (distanceToTarget != null && distanceToTarget !== Infinity && Number.isFinite(distanceToTarget)) {
      return Number(distanceToTarget);
    }
    const routeToPickupKm = Number(activeOrder?.deliveryState?.routeToPickup?.distance);
    const routeToCustomerKm = Number(activeOrder?.deliveryState?.routeToCustomer?.distance);
    if (tripStatus === 'PICKING_UP' || tripStatus === 'REACHED_PICKUP') {
      if (Number.isFinite(routeToPickupKm) && routeToPickupKm > 0) return routeToPickupKm * 1000;
    } else if (tripStatus === 'PICKED_UP' || tripStatus === 'REACHED_DROP') {
      if (Number.isFinite(routeToCustomerKm) && routeToCustomerKm > 0) return routeToCustomerKm * 1000;
    }
    return null;
  }, [distanceToTarget, activeOrder, tripStatus]);

  const computedEta = React.useMemo(() => {
    if (eta != null && eta !== '' && Number(eta) > 0) return Number(eta);
    const pickupDurationMin = Number(activeOrder?.deliveryState?.routeToPickup?.duration);
    const customerDurationMin = Number(activeOrder?.deliveryState?.routeToCustomer?.duration);
    if (tripStatus === 'PICKING_UP' || tripStatus === 'REACHED_PICKUP') {
      if (Number.isFinite(pickupDurationMin) && pickupDurationMin > 0) return Math.ceil(pickupDurationMin);
    } else if (tripStatus === 'PICKED_UP' || tripStatus === 'REACHED_DROP') {
      if (Number.isFinite(customerDurationMin) && customerDurationMin > 0) return Math.ceil(customerDurationMin);
    }
    return null;
  }, [eta, activeOrder, tripStatus]);

  useEffect(() => {
    if (tripStatus === 'COMPLETED' || tripStatus === 'IDLE' || !activeOrder) {
      setActivePolyline(null);
      setSimPath([]);
      setSimIndex(0);
      setSimProgress(0);
    }
  }, [tripStatus, activeOrder]);
  ...
  useEffect(() => {
    ...
    const watchId = navigator.geolocation.watchPosition((pos) => {
      ...
      if (distMoved >= 10 || (now - lastLocationSentAt.current >= 3000)) {
      ...
      const now = Date.now();
      if (now - lastGpsErrorToastAtRef.current < 12000) return;
      ...
      if (error?.code === 1) {
        ...
        toast.error('GPS timeout. Retrying in background.');
      } else {
    ...
    const hydrateAvailableOrder = async () => {
      try {
        ...
        const availableOrders = Array.isArray(availablePayload?.docs)
          ...
          : Array.isArray(availablePayload?.orders)
        ...
        const nextIncomingOrder = availableOrders.find((order) => {
          ...
          return (
            ...
            !['cancelled', 'delivered'].includes(deliveryStateStatus)
          );
        });
                ...
                <div className="grid grid-cols-2 gap-3 w-full">
                    ...
                    <div className="flex flex-col z-10">
                      ...
                      <div className="flex items-end gap-1">
                        <span className="text-2xl font-black text-white leading-none tracking-tighter">
                          {computedDistanceMeters && computedDistanceMeters !== Infinity ? (computedDistanceMeters / 1000).toFixed(1) : '--'}
                        </span>
                  ...
                  <div className="bg-[#10B981] rounded-2xl p-3.5 shadow-xl shadow-green-500/20 border border-green-400/50 flex items-center justify-between relative overflow-hidden group">
                    <div className="flex flex-col z-10">
                      ...
                      <div className="flex items-end gap-1">
                        <span className="text-2xl font-black text-white leading-none tracking-tighter">
                          {computedEta ? String(computedEta) : '--'}
                        </span>
              ...
              <div className="w-full pointer-events-auto relative">
                ...
                {tripStatus === 'COMPLETED' && (
                  <OrderSummaryModal
                    order={activeOrder}
                    onDone={() => {
                      const completedOrderId = activeOrder?.orderId || activeOrder?._id;
                      if (completedOrderId) {
                        writeOrderTracking(completedOrderId, { polyline: null, status: 'completed' }).catch(() => {});
                      }
                      setActivePolyline(null);
                      resetTrip();
                    }}
