...
export default function DeliveryOTP() {
  ...
  useEffect(() => {
    const isComplete = otp.every((digit) => digit !== "") && otp.length === OTP_LENGTH
    if (isComplete && !showNameInput && !pendingMessage && !isLoading && !verifyTriggeredRef.current) {
  ...
  }, [otp, showNameInput, pendingMessage, isLoading])
  ...
  const handleVerify = async (otpValue = null) => {
    ...
    try {
      ...
      const responseMessage = envelope?.message || data?.message || ""

      console.info("[DeliveryOTP] verify-otp response", {
      ...
      if (data.pendingApproval === true) {
        ...
        setPendingMessage(
          responseMessage ||
            (data.isRejected
              ? "Your profile verification was rejected. Please update details and re-apply."
      ...
      if (data.needsRegistration === true || data.needsSignup === true) {
        ...
        const targetStep = data.signupStep === "documents" ? "documents" : "details"
        setIsLoading(false); navigate(`/food/delivery/signup/${targetStep}`, { replace: true });
        return
      }
      const { accessToken, refreshToken, user } = data
      if (accessToken && user) {
              ...
              >
                {!showNameInput ? (
                  <div className="space-y-10">
                    <div className="flex justify-center gap-2 sm:gap-3 px-1 max-w-full overflow-hidden">
                      {otp.map((digit, index) => (
